...
def make_materials():
    ...
    material('teal', '2B535B', .43, .54, .16, .002)
    material('teal_light', '3F6770', .47, .46, .14, .002)
    material('teal_dark', '1C3942', .49, .56, .12, .002)
    material('steel', '35464B', .37, .87, .12, .001)
    material('steel_light', '697577', .31, .91, .14, .001)
    material('shaft', '8E9D9B', .235, .94, .09, .002)
    material('pipe', 'B7BBB0', .43, .63, .11, .001)
    ...
    material('pool_glow', '26BACA', .4, variation=0, bump=0, emission=4.5)
    ...
    for axis in ('X','Y','Z'):
        ...
        edge=nt.nodes.new('ShaderNodeMath'); edge.operation='GREATER_THAN'; edge.inputs[1].default_value=.460
    ...
    threshold=nt.nodes.new('ShaderNodeMath'); threshold.operation='GREATER_THAN'; threshold.inputs[1].default_value=.55
...
def surface_character():
    """Original packed paint maps; no downloaded textures or external dependency.

    Floor rubs are shader colour/roughness, never raised or overlapping polygons.
    Their positions follow working areas and wheels, leaving the main route quiet.
    """
    import numpy as np
    size=2048
    pixels=np.zeros((size,size,4),dtype=np.float32)
    rng=random.Random(913)
    def polygon(points,colour,opacity):
        points=[((x+11)/22*(size-1),(y+11)/22*(size-1)) for x,y in points]
        for iy in range(max(0,int(min(p[1] for p in points))),min(size,int(max(p[1] for p in points))+1)):
            ...
            hits.sort()
            for x0,x1 in zip(hits[::2],hits[1::2]):
                lo=max(0,int(math.ceil(x0))); hi=min(size,int(math.ceil(x1)))
                if hi<=lo: continue
                target=pixels[iy,lo:hi]
                old=target[:,3].copy(); new=opacity+old*(1-opacity)
                target[:,:3]=(np.array(colour)*opacity+target[:,:3]*old[:,None]*(1-opacity))/new[:,None]
                target[:,3]=new
    # A few broad worn areas surround the operator positions, not the entire hall.
    areas=[(-4.8,-6.3,1.1,.60),(-1.2,-7.1,1.05,.35),(3,-6.30,1.0,.45),
           (6.0,-2.9,.38,1.15),(6.2,4.6,.42,.68),(-7.1,5.4,.50,.70),
           (-7.7,1.3,.44,.80),(-3.5,8.0,.60,.45),(0,6.55,.85,.36),
           (3.8,7.35,.72,.30),(0,-4.45,.50,.32),(-9.9,-1.6,.45,.75)]
    for cx,cy,rx,ry in areas:
        for j in range(19):
            x=cx+rng.uniform(-rx,rx); y=cy+rng.uniform(-ry,ry)
            length=rng.uniform(.09,.39); width=rng.uniform(.028,.10)
            angle=rng.uniform(-.7,.7)+(PI/2 if ry>rx else 0)
            raw=[(-length,-width),(-length*.65,width*.6),(.35*length,width),
                 (length,width*.15),(length*.74,-width*.8),(-length*.30,-width*.65)]
            pts=[(x+a*math.cos(angle)-b*math.sin(angle),y+a*math.sin(angle)+b*math.cos(angle)) for a,b in raw]
            colour=srgb('6E736B' if j%3 else 'C3BEAD')
            polygon(pts,colour,rng.uniform(.12,.31))
    # Interrupted twin wheel rubs on the west receiving route.
    for lane in (-.32,.32):
        for j in range(26):
            x=-9.8+j*.18; y=-1.62+lane+.14*math.sin(j*.24)
            polygon([(x,y),(x+.13,y+.006),(x+.14,y+.026),(x+.02,y+.020)],srgb('555F59'),rng.uniform(.10,.24))
    img=bpy.data.images.new('PAINTED floor use map — original Art09',width=size,height=size,alpha=True)
    img.colorspace_settings.name='Non-Color'; img.pixels.foreach_set(pixels.ravel()); img.pack()
    rim=M['mineral_light'].copy(); rim.name='rim_stone'; M['rim_stone']=rim
    for name in ('floor','floor_light','rim_stone'):
        nt=M[name].node_tree; p=nt.nodes.get('Principled BSDF')
        original=p.inputs['Base Color'].links[0].from_socket
        coord=nt.nodes.new('ShaderNodeNewGeometry')
        scale=nt.nodes.new('ShaderNodeVectorMath'); scale.operation='SCALE'; scale.inputs[3].default_value=1/22
        offset=nt.nodes.new('ShaderNodeVectorMath'); offset.operation='ADD'; offset.inputs[1].default_value=(.5,.5,0)
        nt.links.new(coord.outputs['Position'],scale.inputs[0]); nt.links.new(scale.outputs[0],offset.inputs[0])
        tex=nt.nodes.new('ShaderNodeTexImage'); tex.image=img; tex.extension='CLIP'; tex.interpolation='Linear'
        nt.links.new(offset.outputs[0],tex.inputs['Vector'])
        mix=nt.nodes.new('ShaderNodeMixRGB'); nt.links.new(tex.outputs['Alpha'],mix.inputs[0])
        nt.links.new(original,mix.inputs[1]); nt.links.new(tex.outputs['Color'],mix.inputs[2]); nt.links.new(mix.outputs[0],p.inputs['Base Color'])
    # Small-scale roughness differs from the broad painted colour fields.
    for name in ('mineral','mineral_light','floor','floor_light','rim_stone','teal','teal_light','teal_dark','teal_service','steel','steel_light','pipe','fabric'):
        nt=M[name].node_tree; p=nt.nodes.get('Principled BSDF')
        co=nt.nodes.new('ShaderNodeTexCoord'); n=nt.nodes.new('ShaderNodeTexNoise')
        n.inputs['Scale'].default_value=95 if name in ('fabric','mineral','mineral_light','floor','floor_light','rim_stone') else 145
        n.inputs['Detail'].default_value=1.0; n.inputs['Roughness'].default_value=.35
        nt.links.new(co.outputs['Object'],n.inputs['Vector'])
        b=nt.nodes.new('ShaderNodeBump'); b.inputs['Strength'].default_value=.20
        b.inputs['Distance'].default_value=.0008 if name.startswith(('mineral','floor','rim')) else .00015
        if p.inputs['Normal'].links: nt.links.new(p.inputs['Normal'].links[0].from_socket,b.inputs['Normal'])
        nt.links.new(n.outputs['Fac'],b.inputs['Height']); nt.links.new(b.outputs['Normal'],p.inputs['Normal'])
        rough=nt.nodes.new('ShaderNodeMapRange')
        centre=.84 if name.startswith(('mineral','floor','rim')) else (.93 if name=='fabric' else (.30 if 'steel' in name else .44))
        rough.inputs['To Min'].default_value=max(.15,centre-.15); rough.inputs['To Max'].default_value=min(.98,centre+.14)
        nt.links.new(n.outputs['Fac'],rough.inputs['Value']); nt.links.new(rough.outputs[0],p.inputs['Roughness'])
...
def mesh(name, verts, faces, mat, bevel=0, smooth=False):
    ...
    if bevel:
        ...
        mod.width = bevel * (.60 if mat in ('teal','teal_light','teal_dark','steel','steel_light') else 1)
        mod.segments = 2
...
def plate(name, loc, size, title='', textsize=.10, mat='teal_dark'):
    ...
    if title:
        textsize=min(textsize,w*.86/(max(len(line) for line in title.split('\n'))*.64))
        label(name+'.legend',title,(x,y-d/2-.004,z),textsize)
...
def hall_floor():
    ...
    grid_floor()
    for i in range(32):
        ring('Service ring slab',(0,0,-.07),3.9,5.2,.14,'rim_stone',i*TAU/32+.001,(i+1)*TAU/32-.001,5)
    ring('Service outer inlay',(0,0,.004),5.13,5.18,.008,'steel_light')
    # Broad inlaid route markers; no raised floor trip geometry.
    for t in (0,PI):
        with at(5.7*math.cos(t),5.7*math.sin(t),.00015,t+PI/2):
            mark=mesh('Small circulation chevron',[(-.07,-.12,0),(0,-.02,0),(.07,-.12,0),(.105,-.086,0),(0,.065,0),(-.105,-.086,0)],[tuple(range(6))],'floor_wear')
...
def shell():
    ...
    for i in range(8):
        ...
        with at(mid.x,mid.y,0,angle):
            ...
            for x in ((-length*.30,length*.30) if i in (0,4) else ((0,) if i in (1,3,5,7) else ())):
                ...
                light('Warm wall wash',(x,-.63,12.55),(x,-.12,7.6),1450 if i in (0,4) else 1050,'FFD5A2',1,'SPOT',1.15)
            if i in (1,4,5,7):
...
def pool():
    ...
    for i in range(32):
        ...
        ring('Precast rim stone',(0,0,.085),3.4,3.9,.17,'rim_stone',t0,t1,5)
        for j in range(7):
    ...
    light('Cyan from deep pool',(0,0,-6.08),(0,0,1.2),1350,'25BED5',1.75)
    ...
    for x,title in [(-.94,'ACKNOWLEDGE'),(.94,'SCRAM')]:
        with at(x,gy-.06,.17):
            ...
            plate(title+'.head',(0,-.045,.93),(.37,.29,.35),'',.062)
            if x>0:
                plate('SCRAM raised enamel header',(0,-.045,1.195),(.38,.080,.14),'',.069,'steel_light')
                label('SCRAM dark header ink','SCRAM',(0,-.090,1.195),.069,'ink')
                ...
                label('Acknowledge small legend','ACK',(0,-.197,1.050),.043)
...
def banks():
    ...
    for x in (-3.5,3.5):
        box('Gantry service luminaire',(x,-.52,13.70),(.48,.42,.12),'steel',.009)
        box('Gantry service diffuser',(x,-.52,13.633),(.37,.32,.014),'lamp',.004)
        light('Actuator service key',(x,-.52,13.61),(-1.4 if x<0 else 1.4,0,10.0),370,'F5E6C9',.9)
    for index,x in enumerate((-1.4,1.4)):
        ...
        with at(x,0,0):
            ...
            # Cast split bearing face with a circular retaining register; not a cabinet door.
            cylinder(f'{bank} carriage bearing register',(0,-.654,8.66),.285,.055,'steel_light',(0,-1,0),32,.006)
            cylinder(f'{bank} recessed register',(0,-.687,8.66),.229,.012,'rubber',(0,-1,0),32,.001)
            cylinder(f'{bank} bearing cap',(0,-.698,8.66),.203,.033,'teal_dark',(0,-1,0),32,.006)
            box(f'{bank} cast cap split',(0,-.719,8.66),(.37,.008,.022),'steel_light',.002)
            for k in range(8):
                t=k*TAU/8
                cylinder(f'{bank} register cap bolt',(.255*math.cos(t),-.691,8.66+.255*math.sin(t)),.022,.026,'steel',(0,-1,0),6,.002)
            for sx in (-1,1):
                box(f'{bank} upper replaceable slide',(sx*.39,-.605,9.045),(.12,.12,.23),'steel_light',.007)
                tube(f'{bank} grease feed',[(sx*.35,-.671,8.92),(sx*.35,-.679,9.05),(sx*.15,-.679,9.05)],.016,'shaft')
            plate(f'{bank} inspection stamp',(.0,-.649,9.14),(.28,.020,.08),'A-17' if bank=='A' else 'B-22',.027,'teal_light')
...
def console(name, width=1.4, depth=.8, height=1.15, style=0):
    ...
    box(name+'.kick',(0,.04,.22),(width-.30,depth-.19,.3),'teal_dark',.012)
    w=width/2; d=depth/2
    profile=[(-d,.32),(d,.32),(d,height),(-d,height-.25)]
    verts=[(sx*w,y,z) for sx in (-1,1) for y,z in profile]
    mesh(name+'.wedge chassis',verts,[(0,3,2,1),(4,5,6,7),(0,1,5,4),(1,2,6,5),(2,3,7,6),(3,0,4,7)],'teal',.012)
    ...
    for k in range(3 if style==1 else 5):
    ...
    if style==1:
        # Demand station uses a vertical mimic and three physical load meters.
        box(name+'.mimic hood',(0,d-.045,height+.38),(width-.06,.22,.83),'teal_dark',.013)
        plate(name+'.mimic inset',(0,d-.175,height+.50),(width-.18,.04,.41),'',mat='steel')
        for k in range(3):
            x=-width*.29+k*width*.29
            gauge(name+'.phase load',(x,d-.20,height+.48),.115,.28+k*.17)
            label(name+'.meter circuit','L'+str(k+1),(x,d-.205,height+.64),.041)
            lever(name+'.load selector',(x,d-.185,height+.14),.10,'rubber')
        tube(name+'.load bus',[(-width*.32,d-.20,height+.275),(width*.32,d-.20,height+.275)],.007,'pipe')
        label(name+'.station heading','DEMAND / MW',(0,d-.20,height+.75),.070)
    else:
        box(name+'.instrument riser',(0,d-.045,height+.23),(width-.06,.22,.52),'teal_dark',.012)
        screen(name+'.CRT',(-width*.19,d-.18,height+.23),width*.35,.30)
        gauge(name+'.analog backup',(width*.29,d-.19,height+.26),.12,.42+style*.08)
        plate(name+'.asset tag',(width*.29,d-.174,height+.02),(.25,.025,.09),'CTRL / 02' if style==2 else 'REACTIVITY',.028)
    for sx in (-1,1):
        # Folded side cheeks carry the deck, with open fastener access underneath.
        pts=[(sx*(w-.015),-d+.04,.37),(sx*(w-.015),d-.05,.37),
             (sx*(w-.015),d-.05,height-.09),(sx*(w-.015),-d+.04,height-.31)]
        tube(name+'.folded cheek edge',pts+[pts[0]],.009,'steel_light')
        for y in (-d*.45,d*.45):
            cylinder(name+'.side inspection screw',(sx*(w+.006),y,.49),.016,.025,'steel_light',(sx,0,0),6,.002)
    for sx in (-1,1):
...
def stations():
    ...
    with at(4.4,8.5):
        ...
        for x in (-.74,0,.74):
            for z in (.63,1.63):
                ...
                # Every cartridge sits in two rubber-lined cast cradles.
                shelf_top=.22 if z<1 else 1.16
                for yy in (-.47,.47):
                    box('Fuel cradle bolted foot',(x,yy,shelf_top+.018),(.44,.30,.036),'steel_light',.004)
                    height=z-.263-(shelf_top+.036)
                    box('Fuel cradle bearing support',(x,yy,shelf_top+.036+height/2),(.18,.20,height),'teal_dark',.006)
                    saved=FRAME.copy()
                    # Ring plane XZ, cradle opens upwards around the sleeve.
                    globals()['FRAME']=FRAME@Matrix.Translation((x,yy,z))@Matrix.Rotation(PI/2,4,'X')
                    ring('Fuel cast half saddle',(0,0,0),.223,.263,.20,'steel',PI,TAU,20)
                    ring('Fuel saddle liner',(0,0,0),.219,.226,.18,'rubber',PI,TAU,20)
                    globals()['FRAME']=saved
    ...
    with at(9,.8,0,-PI/2):
        for circuit,x in enumerate((-1.0,0,1.0),1):
            with at(x,0): cabinet('GRID / L'+str(circuit),.95,1.1,2.03,1)
...
CAMERAS = [
    ('01_HERO',(-6.0,-10.0,8.5),(0,0,6.4),22,(1120,1400)),
    ('02_WEST_ENTRY',(-10.05,-2.4,1.7),(0,0,5.0),21,(1600,1100)),
    ('03_SOUTH_GATE',(-2.6,-6.35,1.7),(0,-2.3,1.7),24,(1600,1000)),
    ('04_TURBINE_AISLE',(5.3,-6.2,1.8),(7.5,-2.6,1.05),23,(1600,1000)),
    ('05_REVERSE_NORTH',(3.2,7,1.7),(-1,-3.5,3.0),23,(1600,1000)),
    ('06_CONTROL_ROOM',(11.25,-1.70,11.72),(0,0,6.8),17,(1600,1000)),
    ('07_COMPACT_STAIR',(11.72,4.72,1.65),(15.75,4.20,1.5),16,(1600,1000)),
    ('08_MATERIAL_SLICE',(-6,-5.9,1.7),(-3.6,-8,1.4),28,(1600,1000)),
    ('09_BANK_MECHANISMS',(-4.6,-7.2,7.9),(0,0,10.85),28,(1000,1280)),
    ('10_EAST_HIGH',(9.4,-5.9,7.0),(-1.1,0,6.4),18,(1600,1200)),
]
...
def main():
    ...
    if not ARGS.cold_start:
        ...
        make_materials(); surface_character()
