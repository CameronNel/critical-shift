...
C.setTitle('Critical Shift - Reactor / Compact Stair - Schematic Architectural Set')
...
class View:
    ...
    def q(self,p):
        q=self.ox+p[0]*self.k,self.oy+p[1]*self.k
    ...
    def line(self,a,b,color=INK,lw=.2,dash=None,layer='A-OUTLINE'):
        ...
        if self.block is not None:self.block.add_line((a[0]*1000,a[1]*1000),(b[0]*1000,b[1]*1000),dxfattribs=self.attrs(layer,dash))
    def poly(self,pts,fill=None,color=INK,lw=.2,dash=None,layer='A-OUTLINE',closed=True):
        ...
        if self.block is not None:self.block.add_lwpolyline([(x*1000,y*1000) for x,y in pts],close=closed,dxfattribs=self.attrs(layer,dash))
    ...
    def circle(self,x,y,r,color=INK,lw=.2,fill=None,layer='A-EQUIP'):
        ...
        if self.block is not None:self.block.add_circle((x*1000,y*1000),r*1000,dxfattribs={'layer':layer})
    def text(self,s,x,y,size=2.5,bold=False,color=INK,align='left',angle=0):
        ...
        if self.block is not None:
            ...
            e.set_placement((x*1000,y*1000),align={'left':TextEntityAlignment.LEFT,'center':TextEntityAlignment.CENTER,'right':TextEntityAlignment.RIGHT}[align])
...
def title(id,name,scale):
    ...
    p_text('REACTOR / COMPACT CONTROL ACCESS',20,383,3.3,color=GRAY)
    ...
    p_text('REV  P01',524,30,3.0,True)
...
def core(v,level=0,detail=False,grounddoor=True,topdoor=False):
    ...
    if grounddoor:
        ...
        v.door((x0,3.0),(x0,4.2),(x0-1.2,3.0))
    if topdoor:
...
p_text('A compact stair hidden behind the east wall; no hall catwalk.',47,344,2.8,color=GRAY)
...
'02  Direct vestibule entry from the east route.',
'03  Top landing opens into the Control room.',
'04  No stair, support or catwalk occupies the hall.',
...
u=View('A101_CONTROL_10',50,(-185,181))
...
for j,level in enumerate((0,2.5,5,7.5)):
    ...
    w=View(f'A101_STAIR_LEVEL_{j}',50,(px-12.46*20,py-2*20))
...
e.rect(-4.2,0,1.2,2.1,fill=PAPER,color=INK,lw=.3)
e.line((-3.08,.95),(-3.20,.95),INK,.3)
e.text('D01',-3.6,1.55,2.4,True,GOLD,'center')
e.text('STAIR',-3.6,2.65,2.5,True,GOLD,'center')
e.text('INNER DOOR',-3.6,2.35,2.1,color=GRAY,align='center')
e.rect(-1.5,8,5.4,2.2,fill=PALE,color=CYAN,lw=.25,layer='A-GLAZ')
e.rect(-1.5,10.3,5.4,2.6,fill=PALE,color=CYAN,lw=.25,layer='A-GLAZ')
e.rect(-1.5,10.2,5.4,.10,fill=INK,color=INK,lw=.1)
for xx in (.3,2.1):e.line((xx,8),(xx,12.9),INK,.4)
e.text('CONTROL / W01',1.2,11.65,3,True,CYAN,'center')
e.text('LOWER VIEW PANE',1.2,8.9,2.4,color=CYAN,align='center')
e.dimh(-5.6,-1.6,-.60,0,'OUTER ROUTE 4 000')
e.dimv(0,5,-6.50,-5.6,'5 000 OUTER OPENING')
e.dimh(-1.5,3.9,13.25,12.9,'5 400 W01')
e.dimv(10.3,12.9,4.55,3.9,'2 600 W01')
...
c=View('A202_WEST_LANDING_SECTION',50,(320-(-.25)*20,70))
...
p_text('D01 shown in elevation through entry axis Y3.600.',461,224,2.35)
...
d=View('A301_STAIR_DETAIL',25,(100-12.46*40,100))
core(d,0,detail=True)
d.rect(11.16,1.6,1.3,4,color=GRAY,lw=.16,dash=[2,1])
...
'D01 opens into the vestibule; D02 into Control.',
'Flight guards stay inside the tread bands.',
'Gross dimensions are nominal exact-fit chains.',
'No exposed stair or gallery is inside the hall.',
