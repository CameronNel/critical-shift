...
def make_materials():
    ...
    mix=nt.nodes.new('ShaderNodeMixShader'); mix.inputs[0].default_value=.68
...
def screen(name,loc,w=.36,h=.25):
    ...
    # Early phosphor display: sparse traces, a trend line and an actual status readout.
    for row in range(2):
        linew=w*(.40+.18*row)
        box(name+'.trace',(x-w*.40+linew/2,y-.073,z+h*.32-row*h*.14),(linew,.004,.005),'screen_line',0)
    pts=[(x-w*.4+k*w*.8/11,y-.075,z+h*(-.10+.04*math.sin(k*1.7))) for k in range(12)]
    tube(name+'.trend',pts,.0028,'screen_line')
    readout='ACK READY' if 'Alarm' in name else ('84.2  MW' if 'DEMAND' in name else ('A .82 / B .81' if 'BANK' in name else 'SYS / NORMAL'))
    label(name+'.readout',readout,(x,y-.078,z-h*.31),h*.10,'screen_line')
    for sx in (-1,1):
...
def authored_finish():
    ...
    for k in range(19):
        t=(-PI/2+.48*math.sin(k*2.83)) if k<12 else (PI/2+.30*math.sin(k*3.12))
