...
def make_materials():
    material('mineral', 'B9B8A9', .86, variation=.12, bump=.027)
    material('mineral_light', 'CFCBBB', .84, variation=.14, bump=.028)
    material('floor', '93988E', .79, variation=.16, bump=.022)
    material('floor_light', 'AEB1A2', .76, variation=.20, bump=.023)
    ...
    material('pipe', 'C2C8BC', .60, .48, .14, .012)
    ...
    material('screen', '163D42', .55, variation=0, bump=0, emission=.35)
    material('screen_line', '81BFB3', .5, variation=0, bump=0, emission=.6)
    material('lamp', 'E8F3ED', .4, variation=0, bump=0, emission=4)
    material('pool_glow', '00ACDF', .4, variation=0, bump=0, emission=7.0)
    material('pool_tile', '71979B', .44, .15, .12, .012)
    ...
    material('floor_wear', 'A5A89B', .88, variation=.15, bump=.018)
    ...
    water = material('water', 'AADDE0', .045, variation=0, bump=0)
    p = water.node_tree.nodes.get('Principled BSDF')
    p.inputs['Transmission Weight'].default_value = 1
    p.inputs['IOR'].default_value = 1.333
    nt = water.node_tree
    n = nt.nodes.new('ShaderNodeTexNoise')
    n.inputs['Scale'].default_value = 4.5
    n.inputs['Detail'].default_value = 1.0
    b = nt.nodes.new('ShaderNodeBump')
    b.inputs['Strength'].default_value = .12
    b.inputs['Distance'].default_value = .018
    ...
    for axis in ('X','Z'):
        ...
        edge=nt.nodes.new('ShaderNodeMath'); edge.operation='GREATER_THAN'; edge.inputs[1].default_value=.465
    ...
    for i,s in enumerate(masks): nt.links.new(s,maximum.inputs[i])
    ...
    threshold=nt.nodes.new('ShaderNodeMath'); threshold.operation='GREATER_THAN'; threshold.inputs[1].default_value=.60
    nt.links.new(noise.outputs['Fac'],threshold.inputs[0])
    mask=nt.nodes.new('ShaderNodeMath'); mask.operation='MULTIPLY'
    nt.links.new(maximum.outputs[0],mask.inputs[0]); nt.links.new(threshold.outputs[0],mask.inputs[1])
...
def gauge(name, loc, r=.115, needle=.65):
    ...
    for i in range(9):
        t=PI*.2+PI*1.6*i/8
        a=(x+r*.60*math.cos(t),y-.037,z+r*.60*math.sin(t))
        b=(x+r*.74*math.cos(t),y-.037,z+r*.74*math.sin(t))
        tube(name+'.tick',[a,b],.004,'ink')
    t=PI*.2+PI*1.6*needle
    tube(name+'.needle',[(x,y-.045,z),(x+r*.65*math.cos(t),y-.045,z+r*.65*math.sin(t))],.008,'red')
...
def screen(name,loc,w=.36,h=.25):
    ...
    for row in range(4):
        linew=w*(.35+.4*((row*7+3)%9)/9)
        box(name+'.trace',(x-w*.40+linew/2,y-.073,z+h*.32-row*h*.2),(linew,.004,.008),'screen_line',0)
    label(name+'.readout','82.6  MW',(x,y-.078,z-h*.30),h*.11,'screen_line')
...
def light(name,loc,target,power,color='FFFFFF',size=3,kind='AREA',spot=.9):
    ...
    if kind=='SPOT': data.spot_size=spot; data.spot_blend=.65; data.shadow_soft_size=.2
...
def shell():
        ...
        with at(mid.x,mid.y,0,angle):
            ...
            for x0,x1 in zip(xcuts,xcuts[1:]):
                for z0,z1 in zip(zcuts,zcuts[1:]):
                    ...
                    nx=max(1,math.ceil((x1-x0)/3)); nz=max(1,math.ceil((z1-z0)/3.4))
                    for xx in range(nx):
                        for zz in range(nz):
            ...
            for x in (-length*.34,length*.34):
                ...
                light('Warm wall wash',(x,-.63,12.55),(x,-.12,7.6),920,'FFD8A5',1,'SPOT',1.02)
            if i in (1,4,5,7):
                box('Station lamp bracket',(0,-.27,4.65),(.60,.55,.15),'steel',.018)
    ...
    for x,y in [(-6,-5),(0,-6),(6,-5),(-6,4),(0,6),(6,4)]:
        ...
        light('Ceiling cool white',(x,y,14.98),(x*.75,y*.75,0),640,'DAE5E8',2.1)
...
def pool():
    ...
    for i in range(32):
        ...
        for j in range(7):
            ring('Shaft lining course',(0,0,-.45-j*.88),3.4,3.53,.86,'pool_tile' if j%3 else 'pipe',t0,t1,5)
    ...
    cylinder('Deep cyan emitter',(0,0,-6.45),3.28,.035,'pool_glow',sides=80,bevel=0)
    for x in (-1.4,1.4):
        cylinder('Submerged guide body',(x,0,-4.72),.64,2.35,'teal_dark',sides=32)
    ...
    light('Cyan from deep pool',(0,0,-5.85),(0,0,1.2),1800,'12CCFF',3.3)
    # Rail endpoints maintain exactly 1.40 m between fixed gate jambs.
    r=3.69; a=math.asin(.732/r)
    ...
    for i in range(n+1):
        ...
        tube('Rail upright',[(x,y,.22),(x,y,1.10)],.032,'steel')
        cylinder('Rail collar',(x,y,.3),.048,.08,'steel_light')
        bolts('Rail anchors',[(x+dx,y+dy,.23) for dx in (-.05,.05) for dy in (-.05,.05)],.013,(0,0,1))
    gy=-math.sqrt(r*r-.732*.732)
    gate=bpy.data.objects.new('SERVICE_GATE_PIVOT',None); COL.objects.link(gate); gate.matrix_world=Matrix.Translation((-.732,gy,.2))
...
def banks():
    ...
    for index,x in enumerate((-1.4,1.4)):
        ...
        with at(x,0,0):
            ...
            plate(f'{bank} enamel bank identity',(0,-.88,11.40),(1.3,.10,.60),f'CONTROL\nBANK {bank}',.19,'teal_dark')
            ...
            hatch(f'{bank} moving cover',(0,-.651,8.64),.72,1.08)
            for sx in (-.5,.5):
...
def console(name, width=1.4, depth=.8, height=1.15, style=0):
    ...
    hatch(name+'.front access',(0,-d-.018,.59),width*.68,.43)
...
def stations():
    ...
    with at(-4.4,8.5):
        ...
        for y in (-.70,0,.7):
            ...
            for x in (-.9,.9): cylinder('Cartridge cap',(x,y,1.11),.26,.14,'yellow',(1,0,0),24,.02)
    ...
    with at(-1.2,-8.4,0,PI):
        ...
        for z in (.67,1.49):
            ...
            flange('Manifold blind service cover',(-1.06,.15,z),.225,(1,0,0),'steel_light')
...
def authored_finish():
            ...
            if i in (1,4,5,7):
                ...
                for x in (-length*.22,length*.22):
                    ...
                    for depth,scale,mat in [(-.018,1.018,'steel'),(-.083,1,'mineral_light')]:
                        ...
                        mesh('Sealed riser chamfer panel',vs,fs,mat,.018)
                    for zz in (-1.2,1.2):
    ...
    # Authored floor abrasion is sparse, flat and follows the service work zone.
    for k in range(30):
        t=TAU*k/30+.12
        r=4.34+.24*math.sin(k*4.7)
        with at(r*math.cos(t),r*math.sin(t),.0013,t+PI/2):
            sx=.035+.013*(k%4); sy=.08+.03*(k%5)
            verts=[(-sx,-sy,0),(sx*.4,-sy*.87,0),(sx,sy*.34,0),(sx*.12,sy,0),(-sx*.5,sy*.65,0)]
            mesh('Route surface scuff',verts,[tuple(range(5))],'floor_wear')
    ...
    beam('Side shelf support',(12.88,-.12,10.30),(12.88,-.12,10.81),.065,.065,'steel')
...
def stair_and_control():
    ...
    box('Control floor observation glass',(11.335,0,9.9825),(.95,5.4,.035),'glass',.001)
    ...
    for z,h in [(11.6,2.6),(9.1,2.2)]:
        box('W01 safety glass',(10.90,0,z),(.026,5.4,h),'glass',.002)
    ...
    for y in (.60,-1.50):
        ...
        with at(13.72,y,10,-PI/2):
            ...
            box('Chair seat',(0,0,.5),(.54,.49,.09),'rubber',.055)
