...
def make_materials():
    ...
    material('pipe', 'CFD5CB', .55, .3, .05, .006)
    material('rubber', '1C282B', .89, .02, .08, .002)
    material('yellow', 'DAAA38', .57, .18, .08, .004)
...
def box(name, loc, size, mat='steel', bevel=.018):
    ...
    return mesh(name, verts, [(0,4,6,2),(1,3,7,5),(0,1,5,4),(2,6,7,3),(0,2,3,1),(4,5,7,6)], mat, bevel)
...
def ring(name, loc, ri, ro, depth, mat='steel', start=0, end=TAU, steps=64):
    ...
    return mesh(name,verts,faces,mat,.004,False)
...
def lathe(name, profile, mat, sides=32, bore=0):
    ...
    return mesh(name,verts,faces,mat,.012,False)
...
def hall_floor():
    ...
    for i in range(48):
        ...
        inner=[(5.2*math.cos(t),5.2*math.sin(t),0) for t in angles]
        outer=[(limit(t)*math.cos(t),limit(t)*math.sin(t),0) for t in reversed(angles)]
...
def main():
    ...
    if not ARGS.cold_start:
        ...
        revision_source.write_text(SOURCE_TEXT)
