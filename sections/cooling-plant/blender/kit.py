...
def palette():
    ...
    material('floor',(.195,.208,.195),.82,variation=.019,bump=.002)
    material('patch',(.345,.352,.327),.85,variation=.012,bump=.002)
    material('teal',(.030,.093,.099),.57,.38,.008,.0009)
    material('teal_light',(.052,.135,.139),.60,.32,.011,.0008)
    material('steel',(.19,.23,.23),.40,.8,.012)
    material('edge',(.34,.37,.355),.29,.98,.011)
    material('dark',(.034,.049,.05),.62,.5,.008)
    ...
    material('teal_worn',(.061,.105,.099),.78,.32,.009,.001)
    material('blue',(.095,.21,.295),.54,.18,.014)
    ...
    p=MAT['lamp'].node_tree.nodes.get('Principled BSDF');p.inputs['Emission Color'].default_value=(.78,.87,.80,1);p.inputs['Emission Strength'].default_value=3
...
def flange(name,loc,r,axis=(0,1,0),mat='cream',bolts=10):
    ...
    for i in range(bolts):
        q=p+u*.168+(a*cos(2*pi*i/bolts)+b*sin(2*pi*i/bolts))*r*.81
        cyl(name+' washer',q,.036,.009,'edge',u,20,.001)
        cyl(name+' hex nut',q+u*.015,.027,.032,'dark',u,6,.002)
