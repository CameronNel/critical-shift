"""Generate original CP-A03 vector plan and validate the section-local contract.
...
INK, MUTED, GRID = "#30302e", "#666560", "#dedcd7"
OXIDE, OXIDE_LIGHT = "#954e2b", "#f0e5dc"
...
def transform(x, y):
    ...
    t=DATA['proposed_reactor_connection']['translation_xyz']
    return [t[0]+c*x-s*y, t[1]+s*x+c*y]
...
def validate():
    ...
    result = {
        ...
        "acceptance_status": "CP-A03 architectural arithmetic only; see independent saved-scene reports for as-built evidence",
        "unverified": ["Runtime cart/rescue physics and interactions", "Reactor-owned closed connector door", "Global assembled facility placement", "Remote utility continuations"],
        "status": "PASS"
    }
...
def draw():
    ...
    text(58, 104, "CP-A03  ·  DIMENSIONED LOCAL PLAN + PORTAL SECTION  ·  11 SEP 2026", 15, MUTED)
    text(58, 130, "METRES  |  entry (0,0,0)  |  +Y into room  |  +Z up  |  proposed module, connection not applied", 14, MUTED)
    line(58, 148, 1392, 148, GRID)
    for x, col, label in [(60, OXIDE_LIGHT, "Permanent cart lane"), (285, AMBER_LIGHT, "Service / working clearance"), (588, "#dfdcd6", "Permanent equipment")]:
        rect(x, 164, 22, 14, col, MUTED, .6)
        text(x+31, 176, label, 12, MUTED)

    bounds_rect(DATA["shell"]["outer_bounds"], "#c7c4bd", INK, 2)
    ...
    for keep in DATA["keep_clear_volumes"]:
        ...
        fill = OXIDE_LIGHT if keep["id"] == "KC-MAIN" else AMBER_LIGHT
        stroke = OXIDE if keep["id"] == "KC-MAIN" else AMBER
        bounds_rect(keep["bounds"], fill, stroke, 1, "5 3")
    for part in DATA["partitions"]:
        bounds_rect(part["bounds"], "#93918c", INK, 1)
    for a, b in [(-5.5, -2.5), (2.5, 5.5)]:
        bounds_rect({"min": [a, 0, 0], "max": [b, .3, 5.8]}, "#c7c4bd", INK, 1.5)
    for eq in DATA["equipment_envelopes"]:
        bounds_rect(eq["bounds"], "#dfdcd6", INK, 1.6)
    for name, cy in [("P-01", 4.15), ("P-02", 7.60)]:
        # Authored pump symbols: cast casing, shaft/coupling and aisle-facing motor.
        SVG.append(f'<circle cx="{px(-4.18):.2f}" cy="{py(cy):.2f}" r="25" fill="#ba7957" stroke="{INK}" stroke-width="1.5"/>')
        line(px(-3.73), py(cy), px(-3.05), py(cy), INK, 4)
        rect(px(-3.15), py(cy+.4), .98*SCALE, .8*SCALE, "#77777b", INK, 1.2, radius=8)
        for y in [cy-.2, cy, cy+.2]:
            line(px(-3.05), py(y), px(-2.28), py(y), OXIDE, 1)
        text(px(-3.425), py(cy+.79), f"{name}  /  3.05 × 2.10", 12, INK, "middle", "bold")
        text(px(-3.425), py(cy-.78), "SHAFT X  →  MOTOR TO AISLE", 10, MUTED, "middle")
    rect(px(2.6), py(8.8), 1.7*SCALE, 3.8*SCALE, "#606166", INK, 1.5, radius=32)
    ...
    text(px(3.45), py(6.15), "2.30 × 4.81 ENV.", 11, "#ffffff", "middle")
    ...
    text(px(0), py(6.5), "PERMANENT CART / RESCUE LANE  ·  2.20 m", 15, OXIDE, "middle", "bold", -90)
    for y, label in [(2.35, "1.40 OPERATOR LANE"), (5.85, "1.30 SHARED OPERATOR LANE"), (9.3, "1.30 ALCOVE / PUMP ACCESS")]:
    ...
    text(px(-4.1), py(9.9)-8, "CP-D02 / 1.20", 10, INK, "middle")
    ...
    line(px(4.86), py(3.15), px(5.15), py(3.15), OXIDE, 5)
    text(px(4.32), py(3.5), "CP-MINE-WATER", 9, OXIDE, "middle")
    # Fixed-open entry: erase no shell outside portal, preserve five-metre aperture.
    line(px(-2.5), py(0), px(2.5), py(0), "#fbfcfa", 5)
    line(px(-2.5), py(0), px(2.5), py(0), OXIDE, 1.5, "5 3")
    for x in [-2.5, 2.5]:
        line(px(x), py(0), px(x), py(.6), INK, 3)
    SVG.append(f'<circle cx="{px(0)}" cy="{py(0)}" r="4" fill="{OXIDE}"/>')
    text(px(0), py(0)+63, "CP-P01  /  5.00 W × 5.00 H  /  OVERHEAD DOOR FIXED OPEN", 12, INK, "middle", "bold")
    ...
    line(px(0), py(.4), px(0), py(1.2), OXIDE, 2)
    line(px(0), py(1.2), px(-.13), py(1.0), OXIDE, 2)
    line(px(0), py(1.2), px(.13), py(1.0), OXIDE, 2)
    text(px(.3), py(.95), "+Y", 11, OXIDE)

    x, y = 845, 212
    text(x, y, "ACCESS + CONSTRUCTION", 17, INK, weight="bold")
    y = paragraph(x, y+28, "Ceiling plane Z5.80m; girders descend to Z5.385m. Walls 0.30m. Floor is flush; no raised platform. Section stays at local Y ≥ 0. See saved-scene verification separately.")
    ...
    text(x, y+30, "CP-P01 / REACTOR CONNECTION", 17, INK, weight="bold")
    y = paragraph(x, y+58, "Mate at reactor-owned stub OUTER END (11.016295, −11.016295, 0); rotate −135° about Z. Reactor retains its 3.7m stub and closed doors. Proposed transform remains unapplied.")
    y = paragraph(x, y+16, "Whole-shell plan separation: Control ≥3.515m in Y; east stair ≥10.115m in Y. This is a local placement proposal, not whole-map collision certification. New turbine-room fit remains unverified.")
    # Entry section schematic at same numeric drawing scale.
    ex, fy, s = 905, 957, 36
    text(x, 720, "ENTRY ELEVATION  /  LOOKING INTO +Y", 15, INK, weight="bold")
    rect(ex, fy-5.8*s, 5.6*s, 5.8*s, "#c7c4bd", INK, 1.5)
    rect(ex+.3*s, fy-5*s, 5*s, 5*s, "#f5f6f0", INK, 1.5)
    rect(ex+.3*s, fy-5.7*s, 5*s, .7*s, "#a76a45", INK, 1)
    for n in range(1, 5):
    ...
    text(ex+2.8*s, fy-2.5*s, "5.00 × 5.00 CLEAR", 13, OXIDE, "middle", "bold")
    ...
    text(1390, 1082, "CP-A03  ·  1 / 1", 12, INK, "end", "bold")
    head = '<svg xmlns="http://www.w3.org/2000/svg" width="1450" height="1130" viewBox="0 0 1450 1130"><title>Cooling plant CP-A03 dimensioned architecture plan</title><desc>Original section-local architectural floorplan, service reserves, machinery and proposed reactor connector. Dimensions in metres. Not runtime validation.</desc><g font-family="Arial, Helvetica, sans-serif">'
