...
scene['source_sha256']=hashlib.sha256(Path(__file__).read_bytes()+ (HERE/'kit.py').read_bytes()).hexdigest()
scene['authorship']='Original factory-empty Python geometry; no imported legacy meshes or external textures.'

def floor():
    arch=before()
    box('Floor',(0,6.65,-.16),(11.6,13.3,.32),'floor',.002)
    ...
    for x in (-1.58,4.98):
        ...
        for j in range(67):box('drain flush grate',(x,2.25+j*.126,.002),(.19,.036,.008),'steel',.002)
    for x in (-1.15,1.15):
...
def rear_annex():
    ...
    box('Alcove wall west',(-5.1,10.08,1.65),(.80,.16,3.3),'cream',.012)
    box('Alcove wall east',(-2.55,10.08,1.65),(1.9,.16,3.3),'cream',.012)
    box('Alcove lintel',(-4.1,10.08,2.79),(1.2,.16,1.02),'cream',.012)
    box('Alcove side lower',(-1.68,11.57,.82),(.16,2.86,1.64),'cream',.012)
    box('Alcove side upper',(-1.68,11.57,3.0),(.16,2.86,.60),'cream',.012)
    for yy in (10.32,12.85):box('Alcove side pier',(-1.68,yy,2.17),(.16,.25,1.06),'cream',.010)
    for xx in (-4.75,-3.45):box('D02 metal jamb',(xx,10.105,1.14),(.10,.21,2.28),'dark',.012)
    box('D02 header',(-4.1,10.105,2.26),(1.42,.21,.10),'dark',.009)
    # Door open 78 degrees inward, hinge at western jamb; local mesh parent preserved.
    start=before();hinge=Vector((-4.74,10.20,0));theta=pi/2;u=Vector((cos(theta),sin(theta),0))
    ...
    dr=group('D02 swinging leaf',start,'D02 metal jamb',[[-4.75,10.18,.26],[-4.75,10.18,1.8]],(-1,0,0));dr['interaction']='hinged door';dr['hinge_m']=list(hinge);dr['open_angle_deg']=90
    ...
    box('bench rear folded upstand',(-3.7,12.8875,1.01),(2.8,.025,.20),'steel',.004)
    ...
    for z in (.65,.79):
        box('bench shallow drawer',(-4.38,12.62,z),(.88,.58,.125),'teal',.012)
    ...
    start=before()
    annulus('removed blackened seal ring',(-4.08,12.63,.971),.066,.045,.024,'rubber')
    annulus('worn seal ferrule',(-4.08,12.63,.989),.069,.046,.012,'steel')
...
def exchanger():
    ...
    for y in (4.97,8.89):
        flange('HX bolted head '+str(y),(x,y,z),.92,(0,1,0),'teal',20)
    ...
    for y,zv in ((5.8,1.70),(7.85,1.92)):
        pipe('secondary water nozzle',[(4.13,y,zv),(5.5,y,zv)],.10,'steel')
        flange('secondary wall union',(4.9,y,zv),.17,(1,0,0),'steel',8)
        cyl('secondary blue ID',(5.2,y,zv),.104,.13,'blue',(1,0,0),40,.001)
        box('secondary wall sleeve',(5.478,y,zv),(.048,.38,.38),'dark',.006)
...
def entry():
    ...
    box('reserve guarded lever',(4.86,2.13,1.40),(.05,.035,.13),'red',.010)
    text('reserve label','RESERVE\nRESTART',(4.955,2.28,1.57),.069,'paper',(pi/2,0,-pi/2))
    pipe('reserve cable upfeed',[(5.27,2.03,1.66),(5.27,2.03,3.0),(5.27,.0,3.0)],.024,'dark')
    group('reserve restart socket',start)
...
if args.stage=='full':
    ...
    practical('entry batten',0,1.55,5.10,360,(.78,.86,1))
...
CAMERAS=[
 ...
 ('C05_PUMP_A',(-.65,2.88,1.68),(-3.63,4.24,1.00),37),
 ('C06_EXCHANGER',(1.17,4.08,1.68),(3.63,7.0,1.73),32),
 ('C07_PINCH',(-.30,5.84,1.68),(-3.55,8.48,1.48),28),
 ('C08_WORKSHOP',(-3.91,10.57,1.68),(-3.69,12.59,1.11),27),
 ('C09_BUNDLE_BAY',(1.26,11.95,1.68),(3.53,8.6,2.0),30),
...
(ROOT/'production'/'build_manifest.json').write_text(json.dumps(dict(revision=args.revision,stage=args.stage,source_sha256=scene['source_sha256'],objects=len(scene.objects),materials=len(bpy.data.materials),blend=str(out),cameras=10),indent=2))
