...
def material(name,h,rough,metal=0,variation=0,bump=0,emission=0):
    ...
    return m
M={
 'wall':material('Warm mineral plaster','b4b0a3',.87,variation=.10,bump=.012),
 'concrete':material('Dense cast concrete','777e79',.9,variation=.12,bump=.015),
 'floor':material('Dry worn terrazzo concrete','898c83',.77,variation=.07,bump=.005),
 'teal':material('Aged petrol enamel','3f6a6d',.48,.22,.09,.003),
 'darkteal':material('Structural painted steel','344c51',.61,.35,.06),
 'cream':material('Thermal jacket warm grey','b0aa92',.7,.08,.05,.002),
 ...
 'cloth':material('Cotton cleaning rag','606d70',.98,variation=.07,bump=.004),
 ...
 'screen':material('Instrument phosphor','77a994',.5,emission=.25),
 'glass':material('Smoked instrument glass','536d6f',.21,.25),
}
def group(name):
...
def box(n,p,d,m,b=.005):
    ...
    bpy.ops.object.transform_apply(location=False,rotation=False,scale=True);return bevel(o,b)
...
def door(y,n):
    ...
    for side in [-1,1]:
        ...
        for z in [.6,2.5]:
            for dx in [-.44,.44]:bolt(n+' leaf fastener',(x+dx,yp-.087,z),'Y',.012)
        for dx in [-.42,.42]:
    ...
    bracket=box(n+' sign bracket',(0,y-.08,3.07),(.18,.18,.14),'darkteal',.004)
    beam(n+' sign standoff',(0,y-.08,3.18),(0,y-.305,3.18),.028,'darkteal')
    anchor(n+' sign',(0,y-.08,3.0),n+' lintel',prop=bracket.name)
...
def bench():
    ...
    cloth((-3.25,21.8,1.0));anchor('rag',(-3.05,22,1.0),'Bench top',prop='Used folded cotton rag')
...
def main():
    ...
    S['support_registry']=json.dumps(SUPPORT);S['camera_contract']=json.dumps(CAMERAS)
