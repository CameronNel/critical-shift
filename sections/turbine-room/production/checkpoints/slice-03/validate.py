...
def check_contact_islands():
    ...
    add('support.visible_contact_islands', 'FAIL' if islands else 'PASS',
        ...
        caveat='Conservative vertex/surface distance plus triangle intersections; connection is not mechanical stability or a substitute for an explicit intended-support registry')
...
def clear_volume(label, low, high):
    ...
    for name, geo in CACHE.items():
        ...
        if obj.hide_render or obj.get('clearance_ignore', False):
            continue
...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument('--output'); ap.add_argument('--expect-revision'); ap.add_argument('--no-fail-exit', action='store_true')
    ...
    check_clearance(interface)
    add('machine.shaft_alignment', 'NOT_APPLICABLE' if SCENE.get('build_phase') == 'slice' else 'NOT_PROVEN',
        'No turbine in slice' if SCENE.get('build_phase') == 'slice' else 'Machine contract audit awaits full-build geometry')
