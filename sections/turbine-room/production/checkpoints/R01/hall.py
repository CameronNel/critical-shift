...
def build_hall(a):
    ...
    def casing(n,y0,y1,r0,r1):
        ...
        for upper in [False,True]:
            ...
            a.mesh(n+(' upper removable' if upper else ' lower casting'),verts,faces,'cream',.009)
        for side in [-1,1]:
    ...
    text('Oil cabinet title','LUBE / 01',(2.525,12.72,1.34),.105,face='W',align='CENTER')
    for z in [.61,.70]:
    ...
    for y in [20,23]:
        box('Bus wall bracket',(-3.57,y,4.26),(.85,.15,.08),'steel')
    ...
    for y,label,value in [(9.08,'SPEED','SYNC'),(9.53,'HEALTH','READY'),(10.12,'OUTPUT','MATCH'),(10.57,'DEMAND','STEADY'),(11.18,'RESERVE','SHARED'),(11.62,'SAFETY','ARMED')]:
        ...
        text('Instrument heading '+label,label,(-2.802,y,1.70),.073,face='E',align='CENTER')
        text('Instrument readout '+label,value,(-2.804,y,1.42),.065,mat='screen',face='E',align='CENTER')
        for dz in [1.33,1.36]:box('Readout scale',(-2.801,y,dz),(.005,.24,.006),'white',0)
    for y,label in [(9.30,'THROTTLE'),(10.35,'LOAD')]:
        ...
        text(label+' label',label,(-2.645,y-.27,1.09),.064,mat='white',face='UP')
    cyl('Emergency trip mushroom',(-2.71,11.4,1.16),.066,.10,'red')
    text('Emergency trip label','TRIP',(-2.67,11.13,1.09),.085,face='UP')
    ...
    for y,title in [(10.25,'TURBINE CONTROL'),(21.3,'MAINTENANCE / 02')]:
        box('West title backing',(-3.958,y,2.85),(.06,3.1,.44),'darkteal')
        text('West title '+title,title,(-3.915,y,2.76),.18,face='E',align='CENTER')
    for y in [9.3,10.35,11.4]:
    ...
    for x in [4.31,4.89]:
        for y in [19.37,19.83]:cyl('Hoist trolley wheel',(x,y,5.72),.09,.08,'steel','X')
