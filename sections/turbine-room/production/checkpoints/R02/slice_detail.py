...
def details(a):
    ...
    text('Bearing batch marking','T01  /  6208',(-3.3,21.192,1.473),.019,'ink')
    ...
    anchor('table spanner',(x+.039,y-.15,1.0),'Bench top',prop=sockets[0],members=['Forged spanner shank',sockets[1]])
    ...
    box('Oil grade plate',(-2.95,18.4375,.8),(.3,.025,.11),'darkteal',.003)
    text('Oil stand grade','ISO 46',(-3.075,18.422,.776),.063,'white')
    ...
    for y in [18.51,18.99]:bolt('Tank cradle fixing',(-3.21,y,.71),r=.015)
    ...
    for z in [.94,1.02,1.10]:box('Oil sight index',(-2.975,18.419,z),(.027,.005,.002),'white',0)
    ...
    for y in [24.35,24.85]:
        box('Door return construction seam',(-1.202,y,1.4),(.004,.009,2.68),'concrete',0)
        box('Threshold tile seam',(0,y,.002),(2.4,.012,.003),'dark',0)
    box('Return wall bumper',(-1.228,24.6,.78),(.045,1.16,.14),'darkteal',.004)
    for y in [24.16,25.04]:bolt('Return bumper anchor',(-1.212,y,.78),'X',.015)
    ...
    a.light('Door downlight pool',(0,23.68,3.34),(0,23.4,.2),90,(1,.81,.54),.45)
    body=box('Door downlight body',(0,23.71,3.43),(.5,.18,.15),'darkteal',.008)
    box('Door downlight lens',(0,23.69,3.349),(.4,.12,.012),'lamp',.003)
    bracket=box('Door lamp bracket',(0,23.85,3.43),(.12,.3,.1),'darkteal',.003)
    anchor('door lamp',(0,24,3.43),'Overdoor wall.001' if a.args.phase=='full' else 'Overdoor wall',(0,1,0),prop=bracket.name,members=[body.name,'Door downlight lens'])
