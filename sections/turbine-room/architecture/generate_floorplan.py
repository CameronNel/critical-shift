...
TEAL, ROUTE, SERVICE = "#397a7b", "#d1eae0", "#fff0ca"
...
class Sheet:
    def __init__(self):
        self.svg = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}">',
                    '<title>Turbine Room — metric architectural design A</title>',
...
def draw_plan(s):
    ...
    s.poly([pt(*p) for p in points],"#94b8ae",TEAL,2)
    for y in [7,8.7,10.6,12]: s.line(pt(2.95,y),pt(6.25,y),TEAL,2)
    plan_rect(s,[3.0,14.4],[6.2,18.7],"#8ba9aa",TEAL,2)
    ...
    for x,y in [(0,3),(0,21.7),(8.1,3),(8.1,21.7)]:
        plan_rect(s,[x-.4,y-1.1],[x+.4,y+1.1],"#edf7f0",TEAL,2)
    ...
    s.circle(*pt(2.15,12.9),7,"#ebc568",INK,1)
    ...
    s.text(706,192,"U03  z4.60",14,BUS,anchor="middle")
    s.line((716,199),pt(8.4,24),BUS)
...
def draw_sections(s):
    ...
    r(0,7.2,24,.25,WALL);r(5.5,0,14.5,.45,"#bbc5be")
    r(6,.45,6.6,3.35,"#94b8ae",TEAL);r(14.4,.45,4.3,3.35,"#8ba9aa",TEAL)
    ...
    r(2.5,0,4.2,.45,"#bbc5be")
    s.circle(*p(4.6,2),1.8*ss,"#94b8ae",TEAL,2)
    ...
    s.lines(977,1142,["7.20 m clear shell. Route portals set minimum clear height at 2.70 m.","No mezzanine or main-route steps in baseline A. Floor datum ±0.00.","Floor condensate stays east of aisle; any branch needs flush cover.","Final meshes, opened guards and support undersides remain untested."],16)


def finish(s,checks):
    s.text(80,61,"CRITICAL SHIFT  /  TURBINE ROOM",35,INK,True)
    s.text(80,100,"ARCHITECTURAL DESIGN A  ·  metric interface evidence  ·  08 SEP 2026",18,MUTED)
