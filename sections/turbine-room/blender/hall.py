...
def build_hall(a):
    ...
    for y,title in [(10.25,'TURBINE CONTROL'),(21.3,'MAINTENANCE / 02')]:
        ...
        text('West title '+title,title,(-3.458,y,2.76),.18,face='E',align='CENTER')
    ...
    # Quiet broad painted variation; industrial surfaces retain distinct response at eye height.
    for key,strength in [('floor',.16),('cream',.12),('concrete',.18),('wall',.08)]:
        mat=a.M[key];nd=mat.node_tree.nodes;lk=mat.node_tree.links;bs=nd.get('Principled BSDF')
        tex=nd.new('ShaderNodeTexNoise');tex.inputs['Scale'].default_value=13 if key=='floor' else 5;tex.inputs['Detail'].default_value=1.0
        co=nd.new('ShaderNodeTexCoord');lk.new(co.outputs['Generated'],tex.inputs['Vector'])
        ramp=nd.new('ShaderNodeValToRGB');base=list(mat.diffuse_color)
        ramp.color_ramp.elements[0].position=.20;ramp.color_ramp.elements[0].color=tuple([c*(1-strength) for c in base[:3]]+[1])
        ramp.color_ramp.elements[1].position=.80;ramp.color_ramp.elements[1].color=tuple([min(1,c*(1+strength*.6)) for c in base[:3]]+[1])
        lk.new(tex.outputs['Fac'],ramp.inputs[0]);lk.new(ramp.outputs['Color'],bs.inputs['Base Color'])
    bpy.data.objects['Hall broad daylight'].data.energy=1250
    bpy.data.objects['Entry broad fill'].data.energy=600
    # One coherent panel frame unifies the control bank; no extra booth footprint.
    box('Control bank continuous top',(-2.925,10.35,1.80),(.20,3.10,.08),'darkteal',.012)
    for ob in list(a.S.objects):
        if ob.name.startswith('Readout scale'):bpy.data.objects.remove(ob,do_unlink=True)
        elif ob.name.startswith('Instrument readout'):ob.location.z=1.48;ob.data.size=.055
        elif ob.name.startswith('Instrument status lens'):ob.location.z=1.60
    for y in [9.08,9.53,10.12,10.57,11.18,11.62]:
        cyl('Readback dial',(-2.807,y,1.36),.080,.008,'cream','X',48,.001)
        for i in range(7):
            t=math.pi*.15+math.pi*.7*i/6
            beam('Readback tick',(-2.802,y+.058*math.cos(t),1.36+.058*math.sin(t)),(-2.802,y+.068*math.cos(t),1.36+.068*math.sin(t)),.0018,'ink')
        beam('Readback pointer',(-2.800,y,1.36),(-2.800,y+.039,1.402),.0026,'ink')
        cyl('Readback hub',(-2.798,y,1.36),.007,.006,'ink','X',16,.001)
    # Reachable valve operation: enclosed geared shaft drives the existing valve bonnet.
    for ob in list(a.S.objects):
        if ob.name.startswith(('Steam valve stem','Steam isolation handwheel','Valve wheel spoke')):bpy.data.objects.remove(ob,do_unlink=True)
    box('Steam operator tower foot',(6.85,7.2,.055),(.25,.34,.11),'darkteal',.009)
    box('Steam operator tower',(6.85,7.2,.69),(.12,.16,1.30),'darkteal',.012)
    box('Valve vertical gear cover',(6.85,7.2,1.99),(.12,.18,1.5),'darkteal',.018)
    beam('Valve drive shaft',(6.05,7.2,2.72),(6.85,7.2,2.72),.035,'steel')
    box('Valve bevel gearbox',(6.85,7.2,2.72),(.21,.25,.22),'darkteal',.025)
    beam('Reachable handwheel spindle',(6.85,7.2,1.30),(6.98,7.2,1.30),.026,'steel')
    torus('Reachable isolation handwheel',(6.97,7.2,1.3),.18,.020,'red','X')
    for t in [0,math.pi/2,math.pi,math.pi*1.5]:beam('Reachable wheel spoke',(6.97,7.2,1.3),(6.97,7.2+.18*math.cos(t),1.3+.18*math.sin(t)),.013,'red')
    box('Isolation instruction plate',(6.92,7.2,.82),(.02,.32,.20),'cream',.005)
    text('Isolation instruction','STEAM\nISOLATE',(6.932,7.2,.86),.047,mat='ink',face='E',align='CENTER')
    marker('INTERACT_STEAM_ISOLATION',(6.99,7.2,1.3),'geared handwheel at reachable service height, engine action pending')
    # Actual small service fittings make the oil cabinet usable scenery, not a blank decorative box.
    for y in [12.35,13.09]:
        for z in [.73,1.28]:box('Oil door hinge',(2.526,y,z),(.03,.075,.10),'steel',.005)
    box('Oil service door pull',(2.53,12.41,1.02),(.035,.035,.20),'dark',.005)
    cyl('Oil pressure bezel',(2.523,12.70,1.13),.109,.022,'steel','X',48,.002)
    cyl('Oil pressure dial',(2.510,12.70,1.13),.092,.004,'cream','X',48,.001)
    beam('Oil pressure pointer',(2.507,12.70,1.13),(2.507,12.74,1.18),.002,'ink')
    box('Oil level backing',(2.523,12.98,.99),(.015,.12,.40),'cream',.004)
    box('Oil level sight',(2.513,12.98,.99),(.009,.032,.29),'yellow',.003)
    for z in [.87,.99,1.11]:box('Oil level mark',(2.513,13.02,z),(.006,.035,.006),'ink',0)
