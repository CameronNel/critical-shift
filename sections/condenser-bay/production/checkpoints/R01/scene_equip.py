...
def condenser():
    ...
    k.shell_along_x("CD shell", x0, x1, cy, body_c_z, ry, rz, "cream", 32, 0.016)
    for i, x in enumerate((x0 + 0.35, cx, x1 - 0.35)):
        k.torus("CD stiffener", (x, cy, body_c_z), ry + 0.02, 0.035, "struct", "X")
        k.box("CD strap", (x, cy, body_c_z), (0.06, CD_WID_Y + 0.08, CD_BODY_Z1 - CD_BODY_Z0 + 0.08), "struct", 0.004)
    ...
    # Steam chest — rectangular, matches 2.5 x 1.5 exhaust
    k.box("CD steam chest", (EXH_C[0], EXH_C[1], 4.95), (EXH_X + 0.35, EXH_Y + 0.35, 0.70), "cream", 0.02)
    k.box("CD chest flange", (EXH_C[0], EXH_C[1], 5.28), (EXH_X + 0.55, EXH_Y + 0.55, 0.10), "struct", 0.01)
    ...
    # Neck + expansion bellows up to U04
    k.box("CD neck lower", (EXH_C[0], EXH_C[1], 5.48), (EXH_X + 0.12, EXH_Y + 0.12, 0.28), "cream", 0.012)
    for i, z in enumerate((5.62, 5.70, 5.78, 5.86)):
        k.box("bellow convolution", (EXH_C[0], EXH_C[1], z), (EXH_X + 0.22, EXH_Y + 0.22, 0.045), "steel", 0.006)
    k.box("CD neck upper", (EXH_C[0], EXH_C[1], 5.96), (EXH_X + 0.08, EXH_Y + 0.08, 0.12), "cream", 0.01)
    k.box("CD mating flange", (EXH_C[0], EXH_C[1], 5.995), (EXH_X + 0.28, EXH_Y + 0.28, 0.06), "struct", 0.008)
...
def _waterbox(side, x_face, sign, cx, cy, cz, ry, rz):
    ...
    for dy in (-0.7, -0.25, 0.25, 0.7):
        for dz in (-0.7, 0.0, 0.7):
            k.bolt(f"WB {side} cover bolt", (x + sign * (WB_DEPTH / 2 + 0.07), cy + dy, cz + dz), "X", 0.016)
    # Manway
...
def condensate_pumps():
    ...
    k.rod("riser hang A", (6.4, 7.6, 5.5), (6.4, 7.6, 5.95), 0.012, "steel")
    k.rod("riser hang B", (IF_CONDENSATE_HANDOFF[0], 1.4, 5.5), (IF_CONDENSATE_HANDOFF[0], 1.4, 5.95), 0.012, "steel")
    k.anchor("riser hang A", (6.4, 7.6, 5.95), "Ceiling slab", (0, 0, 1))
    k.anchor("riser hang B", (IF_CONDENSATE_HANDOFF[0], 1.4, 5.95), "Ceiling slab", (0, 0, 1))
...
def cooling_water():
    ...
    k.pipe(
        ...
        [
            ...
            (5.55, 3.20, 3.15),
            (5.55, CD_C[1], 3.15),
            (CD_C[0] + CD_LEN_X / 2 + WB_DEPTH + 0.02, CD_C[1], 3.15),
        ],
        0.09,
        "cream",
        0.18,
    ...
    k.pipe(
        ...
        [
            ...
            (8.6, 4.90, 3.15),
            (5.55, 4.90, 3.15),
            (5.55, CD_C[1] + 0.55, 3.15),
            (CD_C[0] - CD_LEN_X / 2 - WB_DEPTH - 0.02, CD_C[1] + 0.55, 3.15),
        ],
        0.09,
        "cream",
        0.18,
    ...
    for x, y in ((8.6, 3.20), (8.6, 4.90), (5.55, 3.20), (5.55, 4.90)):
        ...
        k.rod("CW hang", (x, y, 3.28), (x, y, 5.92), 0.012, "steel")
        k.anchor("CW hang " + f"{x:.1f}_{y:.1f}", (x, y, 5.92), "Ceiling slab", (0, 0, 1))
...
def operator_station():
    ...
    k.box("OP desk", (X0 + 0.55, y, 0.92), (0.42, 0.70, 0.06), "wood", 0.008)
    k.box("OP desk leg L", (X0 + 0.48, y - 0.28, 0.46), (0.06, 0.06, 0.88), "dark", 0.003)
    k.box("OP desk leg R", (X0 + 0.48, y + 0.28, 0.46), (0.06, 0.06, 0.88), "dark", 0.003)
    # Analog instruments
    labels = [("VAC", 0.35), ("LEVEL", 0.12), ("CW IN", -0.12), ("FLOW", -0.35)]
    for i, (lab, dy) in enumerate(labels):
        k.gauge("OP " + lab, (X0 + 0.42, y + dy, 1.85), "X", 0.11)
        k.text("OP " + lab + " t", lab, (X0 + 0.40, y + dy, 1.68), 0.055, "white", "E")
    # Guarded switches
    for i, dy in enumerate((0.28, 0.08, -0.08, -0.28)):
        k.cyl("OP switch body", (X0 + 0.40, y + dy, 1.22), 0.035, 0.05, "steel", "X", 12)
        k.cyl("OP switch knob", (X0 + 0.44, y + dy, 1.22), 0.025, 0.04, "yellow" if i == 0 else "dark", "X", 10)
        k.box("OP switch guard", (X0 + 0.46, y + dy, 1.22), (0.02, 0.08, 0.10), "yellow", 0.002)
    k.box("OP trip", (X0 + 0.40, y, 1.02), (0.08, 0.10, 0.08), "red", 0.004)
    k.box("OP trip guard", (X0 + 0.46, y, 1.02), (0.04, 0.14, 0.12), "yellow", 0.003)
    ...
    k.box("OP mug", (X0 + 0.62, y - 0.18, 0.98), (0.07, 0.07, 0.09), "burgundy", 0.004)
    k.cyl("OP mug handle", (X0 + 0.67, y - 0.18, 0.98), 0.018, 0.05, "burgundy", "Y", 10)
...
def drainage_and_hoist():
    ...
    k.rod("hoist hang L", (5.3, 4.05, 5.48), (5.3, 4.05, 5.95), 0.014, "steel")
    k.rod("hoist hang R", (8.9, 4.05, 5.48), (8.9, 4.05, 5.95), 0.014, "steel")
    k.anchor("hoist L", (5.3, 4.05, 5.95), "Ceiling slab", (0, 0, 1))
    k.anchor("hoist R", (8.9, 4.05, 5.95), "Ceiling slab", (0, 0, 1))
