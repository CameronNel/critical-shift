...
def palette():
    ...
    M = {
        ...
        "oxide": material("Oxide orange enamel", "B56D38", 0.50, 0.20, 0.08, 0.0025),
        "oxide_light": material("Light oxide enamel", "C4844C", 0.55, 0.14, 0.07, 0.002),
        "struct": material("Structural painted steel", "41444A", 0.62, 0.35, 0.05),
        "cream": material("Thermal jacket ivory", "C8C5B9", 0.70, 0.08, 0.045, 0.0018),
        "steel": material("Machined steel", "7C878A", 0.42, 0.88, 0.05),
        "dark": material("Oiled iron", "30383C", 0.64, 0.62, 0.07),
        ...
        "lamp": material("Warm diffusing glass", "FFE2B1", 0.40, emission=2.4),
        "glass": material("Smoked instrument glass", "5A5962", 0.18, 0.08, trans=0.22),
        "water": material("Sight-glass condensate", "8A8478", 0.12, trans=0.55),
    }
    return M
...
def bolt(n, p, axis="Z", r=0.018):
    return cyl(n, p, r, 0.022, "steel", axis, 6, 0.002)
...
def gauge(n, p, axis="X", r=0.11):
    cyl(n + " bezel", p, r, 0.055, "dark", axis, 32, 0.004)
    u = Vector((1, 0, 0) if axis == "X" else ((0, 1, 0) if axis == "Y" else (0, 0, 1)))
    face = Vector(p) + u * 0.028
    cyl(n + " dial", face, r * 0.82, 0.008, "paper", axis, 32)
    cyl(n + " glass", face + u * 0.012, r * 0.80, 0.006, "glass", axis, 32)
    cyl(n + " pivot", face + u * 0.006, 0.012, 0.016, "dark", axis, 12)
    return face


def light(n, p, target, power, color=(1.0, 0.86, 0.68), size=1.2, shape="DISK", size_y=None):
