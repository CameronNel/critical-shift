...
def materials():
    ...
    k.M['water'] = k.material('Astra warm neutral liquid column', '9E834E', .22, .03)
    k.M['glass'] = k.material('Astra clear thick gauge glass Eevee', 'DDE0DD', .12, .0)
    m=k.M['glass']; n=m.node_tree.nodes; l=m.node_tree.links
    bs=n.get('Principled BSDF'); out=n.get('Material Output')
    trans=n.new('ShaderNodeBsdfTransparent')
    mix=n.new('ShaderNodeMixShader'); layer=n.new('ShaderNodeLayerWeight')
    edge=n.new('ShaderNodeMapRange')
    edge.inputs['To Min'].default_value=.035
    edge.inputs['To Max'].default_value=.24
    l.new(layer.outputs['Fresnel'],edge.inputs['Value'])
    l.new(edge.outputs['Result'],mix.inputs[0]); l.new(trans.outputs[0],mix.inputs[1])
    l.new(bs.outputs[0],mix.inputs[2]); l.new(mix.outputs[0],out.inputs['Surface'])
...
def sight_glasses():
    ...
    for side in [-1,1]:
        ...
        for index,x in enumerate([2.38,3.62]):
            ...
            k.torus(tag+' meniscus',(x,gy,water_top),.046,.003,'steel')
...
def secondary_construction():
    ...
    for iy in range(9):
        for ix in range(9):
            u=ix/8;v=iy/8
            verts.append((-1.14+.24*u,4.34+.17*v,.978+.012*math.sin(u*math.pi*3)*math.sin(v*math.pi)))
...
def gallery_repairs():
    ...
    bpy.context.view_layer.update()
    for o in list(k.S.objects):
        if o.name.startswith('stair2'): o.matrix_world=change@o.matrix_world
    ...
    grate('gallery west',.10,1.00,1.805,6.295)
    grate('gallery north',1.00,5.45,5.395,6.295)
    segments=[((6.35,3.95),(6.35,6.295)),((6.35,6.295),(.10,6.295)),
              ((.10,6.295),(.10,1.805)),((.10,1.805),(1.,1.805)),
              ((1.,1.805),(1.,5.395)),((1.,5.395),(5.45,5.395)),((5.45,5.395),(5.45,4.10))]
    for a,b in segments:
    ...
    for x,y in [(6.35,4.1),(6.35,6.25),(.1,2.1),(.1,5.9),(3.2,6.295)]:
