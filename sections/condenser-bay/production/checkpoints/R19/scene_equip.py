...
def condenser():
    ...
    # Level glasses on south and north hotwell
    ...
    _rect_frame("CD mating gasket", 5.94, 0.05, 0.20, "rubber")
    _rect_frame("CD mating flange", 5.99, 0.12, 0.24, "struct")
    k.box("CD gasket strip S", (EXH_C[0], y_s - 0.22, 5.93), (EXH_X + 0.50, 0.08, 0.08), "rubber", 0.004)
    k.box("CD flange plate S", (EXH_C[0], y_s - 0.26, 5.99), (EXH_X + 0.55, 0.08, 0.16), "struct", 0.006)
    for x in [EXH_C[0] + d * (EXH_X / 2 + 0.06) for d in (-1, -0.75, -0.5, -0.25, 0, 0.25, 0.5, 0.75, 1)]:
        k.cyl("mate bolt S", (x, y_s - 0.32, 5.99), 0.028, 0.05, "steel", "Y", 6, 0.002)
    k.text("U04 face", "U04  2.5 x 1.5", (EXH_C[0], y_s - 0.30, 5.72), 0.10, "yellow", "S")
    for x in [EXH_C[0] + d * (EXH_X / 2 + 0.18) for d in (-1, -0.5, 0, 0.5, 1)]:
        for y in (y_s - 0.20, y_n + 0.20):
            k.bolt("mate bolt Z", (x, y, 6.06), "Z", 0.016)
...
def _cep(name, x, y):
    ...
    k.flange(name + " HW nozzle", (hx, CD_C[1] + 0.58, HW_Z0 + 0.12), 0.13, "Y", "steel", 8, 0.05)
    k.cyl(name + " HW boss", (hx, CD_C[1] + 0.48, HW_Z0 + 0.12), 0.09, 0.12, "cream", "Y", 20, 0.006)
...
def cooling_water():
    ...
    for x, y in ((8.55, 3.60), (8.55, 4.40), (8.55, 5.55), (ex, ny + 0.22), (wx, ny + 0.22)):
        ...
        k.rod("CW hang", (x, y, 3.32), (x, y, 5.92), 0.012, "steel")
        k.box("CW hang plate", (x, y, 5.94), (0.22, 0.22, 0.05), "steel", 0.003)
        k.cyl("CW hang nut", (x, y, 5.90), 0.025, 0.04, "dark", "Z", 6)
...
def operator_station():
    ...
    for i, dy in enumerate((0.48, 0.16, -0.16, -0.48)):
        k.cyl("OP rotary bezel", (X0 + 0.46, y + dy, 1.38), 0.055, 0.03, "dark", "X", 24, 0.004)
        k.cyl("OP rotary body", (X0 + 0.50, y + dy, 1.38), 0.032, 0.05, "steel", "X", 20, 0.004)
        k.cyl("OP rotary knob", (X0 + 0.55, y + dy, 1.38), 0.024, 0.04, "yellow" if i == 0 else "dark", "X", 16)
        k.box("OP rotary pointer", (X0 + 0.56, y + dy, 1.42), (0.02, 0.008, 0.04), "yellow", 0.002)
        k.cyl("OP guard ring", (X0 + 0.50, y + dy, 1.38), 0.07, 0.012, "yellow", "X", 16, 0.002)
