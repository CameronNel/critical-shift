...
def _waterbox(side, x_face, sign, cx, cy, cz, ry, rz):
    ...
    k.flange(f"WB {side} manway fl", (x + sign * (WB_DEPTH / 2 + 0.07), cy, cz + 0.15), 0.24, "X", "steel", 10, 0.04)
    # Davit for cover
...
def cooling_water():
    ...
    for y, tag in ((3.20, "SUPPLY"), (4.90, "RETURN")):
        ...
        k.cyl("CW isol body " + tag, (8.15, y, 3.15), 0.12, 0.22, "steel", "X", 24, 0.008)
        k.cyl("CW isol stem " + tag, (8.15, y + 0.14, 3.15), 0.025, 0.16, "steel", "Y", 12)
        k.wheel("CW isol " + tag, (8.15, y + 0.28, 3.15), 0.15, "Y", "yellow")
