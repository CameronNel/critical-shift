...
def condenser():
    ...
    _rect_frame("CD neck lower", 5.22, 0.12, 0.09, "cream")
    # Accordion bellows: alternating steel folds that read as a steam expansion joint from the side.
    for i in range(6):
        z = 5.36 + i * 0.09
        lip = 0.14 if i % 2 == 0 else 0.08
        _rect_frame("bellow " + str(i), z, 0.05, lip, "steel" if i % 2 == 0 else "iron")
    _rect_frame("CD neck upper", 5.88, 0.08, 0.09, "cream")
    _rect_frame("CD mating flange", 5.97, 0.07, 0.16, "struct")
    _rect_frame("CD mating gasket", 5.93, 0.03, 0.13, "rubber")
    for x in (EXH_C[0] - EXH_X / 2 - 0.14, EXH_C[0] + EXH_X / 2 + 0.14):
        for y in (EXH_C[1] - EXH_Y / 2 - 0.10, EXH_C[1] + EXH_Y / 2 + 0.10):
            k.bolt("mate bolt", (x, y, 6.01), "Z", 0.016)
...
def cooling_water():
    ...
    for x, y in ((8.55, 3.60), (8.55, 4.40), (8.55, 5.55), (ex, ny + 0.22), (wx, ny + 0.22)):
        ...
        k.box("CW hang plate", (x, y, 5.94), (0.14, 0.14, 0.04), "steel", 0.003)
