...
Run only through the shared GPU gate. S07 is explicitly labelled a cutaway.
"""
...
views={
 'S01_LOCAL_U02':((7.45,2.0,5.0),(7.9,.10,5.42),32),
...
for name in selection.split(','):
    ...
    width=36/lens
    font.size=.015*width; font.align_x='LEFT';font.materials.append(material)
    label=bpy.data.objects.new('Evidence caption',font);k.S.collection.objects.link(label)
    label.parent=cam;label.location=(-.45*width,.235*width,-1.0)
    render_set(ROOT,rev,name,64)
