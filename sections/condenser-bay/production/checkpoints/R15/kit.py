...
def gauge(n, p, axis="X", r=0.11):
    ...
    b = u.cross(a).normalized()
    pv = Vector(p)
    cyl(n + " bezel", p, r, 0.05, "dark", axis, 32, 0.004)
    face = pv + u * 0.022
    cyl(n + " dial", face, r * 0.84, 0.01, "paper", axis, 32)
    for i in range(11):
        ang = math.pi * 0.15 + math.pi * 1.7 * i / 10
        d0, d1 = r * 0.62, r * 0.78
        aa = face + (a * math.cos(ang) + b * math.sin(ang)) * d0 + u * 0.008
        bb = face + (a * math.cos(ang) + b * math.sin(ang)) * d1 + u * 0.008
        rod(n + f" tick{i}", aa, bb, 0.0025, "ink")
    tip = face + (a * math.cos(math.pi * 0.55) + b * math.sin(math.pi * 0.55)) * r * 0.55 + u * 0.01
    rod(n + " needle", face + u * 0.01, tip, 0.004, "burgundy")
    cyl(n + " pivot", face + u * 0.01, 0.012, 0.014, "dark", axis, 12)
    return face
