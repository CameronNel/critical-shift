...
def condenser():
    ...
    for i in range(5):
        z = 5.38 + i * 0.10
        _rect_frame("bellow steel " + str(i), z, 0.045, 0.16 if i % 2 == 0 else 0.11, "steel")
        _rect_frame("bellow rubber " + str(i), z + 0.048, 0.03, 0.13, "rubber")
    _rect_frame("CD neck upper", 5.86, 0.08, 0.11, "cream")
    _rect_frame("CD mating gasket", 5.91, 0.04, 0.18, "rubber")
    _rect_frame("CD mating flange", 5.96, 0.10, 0.22, "struct")
    xs = [EXH_C[0] + d * (EXH_X / 2 + 0.18) for d in (-1, -0.5, 0, 0.5, 1)]
    ys = [EXH_C[1] - EXH_Y / 2 - 0.18, EXH_C[1] + EXH_Y / 2 + 0.18]
    for x in xs:
        for y in ys:
            k.bolt("mate bolt", (x, y, 6.02), "Z", 0.018)
    for y in [EXH_C[1] + d * (EXH_Y / 2 * 0.4) for d in (-1, 1)]:
        for x in (EXH_C[0] - EXH_X / 2 - 0.18, EXH_C[0] + EXH_X / 2 + 0.18):
            k.bolt("mate bolt b", (x, y, 6.02), "Z", 0.018)
