...
def condenser():
    ...
    for gy in (cy - CD_WID_Y / 2 - 0.05, cy + CD_WID_Y / 2 + 0.05):
        for x in (cx - 0.62, cx + 0.62):
            zc = (HW_Z0 + HW_Z1) / 2
            k.cyl("level glass tube", (x, gy, zc), 0.038, 0.58, "glass", "Z", 24, 0.001)
            k.cyl("level water column", (x, gy, zc - 0.06), 0.028, 0.34, "water", "Z", 20, 0.001)
...
def _cep(name, x, y):
    ...
    k.pipe(
        ...
        [
            ...
            (CD_C[0] + (0.4 if "A" in name else -0.4), CD_C[1] + 0.82, HW_Z0 + 0.12),
        ],
        0.075,
        "cream",
        0.14,
...
def cooling_water():
    ...
    for x, y in ((8.55, 3.60), (8.55, 4.40), (8.55, 5.55), (ex, ny + 0.22), (wx, ny + 0.22)):
        ...
        k.rod("CW hang", (x, y, 3.32), (x, y, 5.58), 0.014, "steel")
        k.box("CW hang clevis", (x, y, 5.62), (0.08, 0.08, 0.10), "dark", 0.003)
        k.box("CW hang plate", (x, y, 5.70), (0.28, 0.22, 0.04), "steel", 0.003)
        k.bolt("CW hang bolt L", (x - 0.08, y, 5.74), "Z", 0.014)
        k.bolt("CW hang bolt R", (x + 0.08, y, 5.74), "Z", 0.014)
