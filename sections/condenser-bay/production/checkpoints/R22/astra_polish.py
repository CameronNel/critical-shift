...
def return_connections():
    ...
    for name,x,y in [('CEP-A',P1[0],P1[1]),('CEP-B',P2[0],P2[1])]:
        ...
        k.pipe(name+' suction line',[(cx,y,.35),(cx,y,.52),(hx,y,.52),(hx,face+.38,.52),
...
def instrument_faces():
    ...
    for can in list(k.S.objects):
        ...
        prefix=can.name[:-4]
        axis=can.rotation_euler.to_matrix()@Vector((0,0,1))
        for o in list(k.S.objects):
...
def exhaust_fasteners():
    ...
    for side in [-1,1]:
        for i in range(12):
            ...
            k.cyl('U04 underside washer',(x,y,5.887),.029,.009,'steel','Z',24,.001)
            k.cyl('U04 underside nut',(x,y,5.87),.022,.030,'steel','Z',6,.001)
        for i in range(7):
            x=3+side*1.52; y=3.37+i*1.36/6
            k.cyl('U04 side underside washer',(x,y,5.887),.029,.009,'steel','Z',24,.001)
            k.cyl('U04 side underside nut',(x,y,5.87),.022,.030,'steel','Z',6,.001)
