...
ckpt=ROOT/'production/checkpoints'/rev
assert not (ckpt/'build_manifest.json').exists(), 'Never overwrite a completed revision'
ckpt.mkdir(parents=True,exist_ok=True)
hashes={}
for p in sorted(HERE.glob('*.py')):
    content=p.read_bytes();(ckpt/p.name).write_bytes(content)
...
module=ast.parse((ckpt/'kit.py').read_text())
...
manifest={'revision':rev,'baseline_sha256':expected,'blend_sha256':sha,'source_sha256':hashes,
          'render_engine':S.render.engine,'render_device':S.cycles.device,'resource_policy':S.get('astra_resource_policy'),
