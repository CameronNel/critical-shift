...
The authorized resource guard and shared GPU gate are mandatory for GPU work. S07 is explicitly labelled a cutaway.
"""
...
views={
 ...
 'S06_OPERATOR_WORK':((.10,3.10,1.40),(-1.05,4.12,1.10),40),
...
for name in selection.split(','):
    ...
    if name=='S07_U04_CUTAWAY':
        prefixes=('CD chest','CD shell','bellow','U04 lower','U04 upper','U04 visible','U04 slab',
    ...
    caption_width=36/lens
    font.size=.015*caption_width*.05; font.align_x='LEFT';font.materials.append(material)
    label=bpy.data.objects.new('Evidence caption',font);k.S.collection.objects.link(label)
    cam.data.clip_start=.01
    label.parent=cam;label.location=(-.45*caption_width*.05,.235*caption_width*.05,-.05)
