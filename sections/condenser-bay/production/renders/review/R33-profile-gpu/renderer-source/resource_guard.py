...
def configure_scene(scene,samples=32):
    ...
    if hasattr(scene.cycles,'denoising_use_gpu'):scene.cycles.denoising_use_gpu=MODE=='GPU'
    ...
    scene['astra_resource_policy']=('CPU only; 2 threads and 2 logical processors; IDLE priority; GPU denoising disabled' if MODE=='CPU' else 'User authorized full GPU speed; HIP hardware ray tracing; GPU denoising; normal priority; all CPU threads; shared GPU gate; one worker')
