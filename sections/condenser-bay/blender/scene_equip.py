...
def condenser():
    ...
    # Level glasses: outer glass, inner water, four guard rods — not an opaque steel sleeve
    for gy in (cy - CD_WID_Y / 2 - 0.05, cy + CD_WID_Y / 2 + 0.05):
        for x in (cx - 0.62, cx + 0.62):
            zc = (HW_Z0 + HW_Z1) / 2
            k.cyl("level glass tube", (x, gy, zc), 0.048, 0.62, "glass", "Z", 28, 0.001)
            k.cyl("level water column", (x, gy, HW_Z0 + 0.28), 0.034, 0.28, "water", "Z", 24, 0.001)
            k.cyl("level glass top cock", (x, gy, HW_Z1 + 0.12), 0.05, 0.10, "dark", "Z", 16, 0.003)
            k.cyl("level glass bot cock", (x, gy, HW_Z0 + 0.04), 0.05, 0.08, "dark", "Z", 16, 0.003)
            k.cyl("level glass top gland", (x, gy, HW_Z1 + 0.04), 0.042, 0.05, "steel", "Z", 16, 0.002)
            k.cyl("level glass bot gland", (x, gy, HW_Z0 + 0.12), 0.042, 0.05, "steel", "Z", 16, 0.002)
            for i in range(4):
                a = i * math.tau / 4 + math.pi / 4
                k.rod(
                    "level guard rod",
                    (x + 0.055 * math.cos(a), gy + 0.055 * math.sin(a), HW_Z0 + 0.08),
                    (x + 0.055 * math.cos(a), gy + 0.055 * math.sin(a), HW_Z1 + 0.08),
                    0.006,
                    "steel",
                )
    ...
    k.box("CD chest wall X+", (EXH_C[0] + EXH_X / 2 + t / 2, EXH_C[1], zc), (t, EXH_Y, h), "cream", 0.010)
    k.box("CD chest flange Y-", (EXH_C[0], EXH_C[1] - EXH_Y / 2 - 0.14, 5.34), (EXH_X + 0.48, 0.12, 0.08), "struct", 0.006)
    k.box("CD chest flange Y+", (EXH_C[0], EXH_C[1] + EXH_Y / 2 + 0.14, 5.34), (EXH_X + 0.48, 0.12, 0.08), "struct", 0.006)
    k.box("CD chest flange X-", (EXH_C[0] - EXH_X / 2 - 0.14, EXH_C[1], 5.34), (0.12, EXH_Y + 0.24, 0.08), "struct", 0.006)
    k.box("CD chest flange X+", (EXH_C[0] + EXH_X / 2 + 0.14, EXH_C[1], 5.34), (0.12, EXH_Y + 0.24, 0.08), "struct", 0.006)
    ...
    _rect_frame("CD neck lower", 5.22, 0.14, 0.10, "cream")

    def _corrugated_y(prefix, y_face, sign, z0, z1, folds, out, mat):
        x0, x1 = EXH_C[0] - EXH_X / 2 - 0.04, EXH_C[0] + EXH_X / 2 + 0.04
        n = folds
        vs = []
        for x in (x0, x1):
            for i in range(n + 1):
                z = z0 + (z1 - z0) * i / n
                y = y_face + sign * (out if i % 2 else 0.02)
                vs.append((x, y, z))
        faces = [(i, i + 1, n + 2 + i, n + 1 + i) for i in range(n)]
        k.mesh(prefix, vs, faces, mat, 0.004, True)

    def _corrugated_x(prefix, x_face, sign, z0, z1, folds, out, mat):
        y0, y1 = EXH_C[1] - EXH_Y / 2 - 0.04, EXH_C[1] + EXH_Y / 2 + 0.04
        n = folds
        vs = []
        for y in (y0, y1):
            for i in range(n + 1):
                z = z0 + (z1 - z0) * i / n
                x = x_face + sign * (out if i % 2 else 0.02)
                vs.append((x, y, z))
        faces = [(i, i + 1, n + 2 + i, n + 1 + i) for i in range(n)]
        k.mesh(prefix, vs, faces, mat, 0.004, True)
    ...
    x_w = EXH_C[0] - EXH_X / 2
    x_e = EXH_C[0] + EXH_X / 2
    _corrugated_y("bellow S", y_s, -1, 5.36, 5.88, 8, 0.12, "steel")
    _corrugated_y("bellow N", y_n, 1, 5.36, 5.88, 8, 0.12, "steel")
    _corrugated_x("bellow W", x_w, -1, 5.36, 5.88, 8, 0.12, "steel")
    _corrugated_x("bellow E", x_e, 1, 5.36, 5.88, 8, 0.12, "steel")
    _rect_frame("CD neck upper", 5.90, 0.08, 0.12, "cream")
    # Thick rectangular gasket + steel flange so C04 reads a sealed slab landing, not a stencil.
    _rect_frame("CD mating gasket", 5.92, 0.08, 0.22, "rubber")
    _rect_frame("CD mating flange", 5.99, 0.14, 0.28, "struct")
    k.box("CD gasket strip S", (EXH_C[0], y_s - 0.24, 5.93), (EXH_X + 0.56, 0.10, 0.10), "rubber", 0.004)
    k.box("CD flange plate S", (EXH_C[0], y_s - 0.30, 5.99), (EXH_X + 0.64, 0.12, 0.18), "struct", 0.006)
    k.box("CD flange plate N", (EXH_C[0], y_n + 0.30, 5.99), (EXH_X + 0.64, 0.12, 0.18), "struct", 0.006)
    k.box("CD flange plate W", (x_w - 0.30, EXH_C[1], 5.99), (0.12, EXH_Y + 0.64, 0.18), "struct", 0.006)
    k.box("CD flange plate E", (x_e + 0.30, EXH_C[1], 5.99), (0.12, EXH_Y + 0.64, 0.18), "struct", 0.006)
    # Continuous bolt pitch around the 2.5 x 1.5 opening.
    pitch = 0.22
    xs = []
    x = EXH_C[0] - EXH_X / 2 - 0.14
    while x <= EXH_C[0] + EXH_X / 2 + 0.14 + 1e-6:
        xs.append(x)
        x += pitch
    ys = []
    y = EXH_C[1] - EXH_Y / 2 - 0.14
    while y <= EXH_C[1] + EXH_Y / 2 + 0.14 + 1e-6:
        ys.append(y)
        y += pitch
    for x in xs:
        k.bolt("mate bolt S", (x, y_s - 0.30, 5.99), "Z", 0.018)
        k.bolt("mate bolt N", (x, y_n + 0.30, 5.99), "Z", 0.018)
    for y in ys[1:-1]:
        k.bolt("mate bolt W", (x_w - 0.30, y, 5.99), "Z", 0.018)
        k.bolt("mate bolt E", (x_e + 0.30, y, 5.99), "Z", 0.018)
    k.text("U04 face", "U04  2.5 x 1.5", (EXH_C[0], y_s - 0.38, 5.70), 0.11, "yellow", "S")
...
def _cep(name, x, y):
    ...
    k.pipe(
        ...
        [
            ...
            (CD_C[0] + (0.4 if "A" in name else -0.4), CD_C[1] + 0.58, HW_Z0 + 0.12),
            (CD_C[0] + (0.4 if "A" in name else -0.4), CD_C[1] + 0.42, HW_Z0 + 0.12),
        ],
        0.075,
        "cream",
        0.14,
    )
    hx = CD_C[0] + (0.4 if "A" in name else -0.4)
    # Closed nozzle: pipe enters a cream boss, then a bolted flange — not a grey ring.
    k.cyl(name + " HW boss", (hx, CD_C[1] + 0.42, HW_Z0 + 0.12), 0.11, 0.22, "cream", "Y", 24, 0.008)
    k.cyl(name + " HW nipple", (hx, CD_C[1] + 0.58, HW_Z0 + 0.12), 0.075, 0.14, "steel", "Y", 24, 0.004)
    k.flange(name + " HW nozzle", (hx, CD_C[1] + 0.68, HW_Z0 + 0.12), 0.14, "Y", "steel", 10, 0.06)
    k.cyl(name + " HW throat", (hx, CD_C[1] + 0.78, HW_Z0 + 0.12), 0.07, 0.08, "cream", "Y", 20, 0.003)
    ...
    k.pipe(name + " gauge tap", [(cx, y, 1.88), (cx + 0.22, y, 1.88), (cx + 0.22, y - 0.16, 1.88)], 0.012, "steel", 0.04)
    k.gauge(name + " out gauge", (cx + 0.22, y - 0.22, 1.88), "Y", 0.08)
...
def cooling_water():
    ...
    k.cyl("CW supply cap", (X1 - 0.10, 3.20, 3.15), 0.16, 0.05, "dark", "X", 28)
    k.cyl("CW return cap", (X1 - 0.10, 4.90, 3.15), 0.16, 0.05, "dark", "X", 28)
    ...
    # Local roof beam so hangers land in a frame, not on a floating plate.
    k.i_beam("CW hang beam", (8.55, 3.05, 5.72), (8.55, 6.05, 5.72), 0.16, 0.22, "steel")
    k.i_beam("CW hang beam N", (wx - 0.15, ny + 0.22, 5.72), (ex + 0.15, ny + 0.22, 5.72), 0.16, 0.22, "steel")
    for x, y in ((8.55, 3.60), (8.55, 4.40), (8.55, 5.55), (ex, ny + 0.22), (wx, ny + 0.22)):
        axis = "Y" if abs(x - 8.55) < 0.2 else "X"
        k.cyl("CW saddle", (x, y, 3.15), 0.13, 0.10, "steel", axis, 24, 0.004)
        k.rod("CW hang", (x, y, 3.28), (x, y, 5.58), 0.016, "steel")
        k.box("CW hang clevis", (x, y, 5.62), (0.10, 0.10, 0.12), "dark", 0.003)
        k.box("CW hang plate", (x, y, 5.70), (0.32, 0.24, 0.05), "steel", 0.003)
        k.bolt("CW hang bolt L", (x - 0.09, y, 5.74), "Z", 0.014)
        k.bolt("CW hang bolt R", (x + 0.09, y, 5.74), "Z", 0.014)
...
def operator_station():
    ...
    for lab, dy in labels:
        k.gauge("OP " + lab, (X0 + 0.48, y + dy, 2.08), "X", 0.13)
        k.text("OP " + lab + " t", lab, (X0 + 0.44, y + dy, 1.86), 0.055, "white", "E")
    for i, dy in enumerate((0.48, 0.16, -0.16, -0.48)):
        k.rotary("OP rotary", (X0 + 0.50, y + dy, 1.36), "X", 0.048, "yellow" if i == 0 else "dark")
    ...
    k.box("OP binder cover", (X0 + 0.92, y + 0.32, 1.03), (0.16, 0.22, 0.012), "burgundy", 0.003)
    k.box("OP binder pages", (X0 + 0.92, y + 0.32, 1.00), (0.14, 0.20, 0.04), "paper", 0.002)
    k.box("OP binder spine", (X0 + 0.84, y + 0.32, 1.01), (0.02, 0.22, 0.06), "dark", 0.002)
    ...
    k.cyl("OP desk lamp post", (X0 + 0.72, y - 0.48, 1.15), 0.012, 0.36, "steel", "Z", 8)
    k.cyl("OP desk lamp", (X0 + 0.82, y - 0.48, 1.34), 0.05, 0.04, "lamp", "Z", 12)
