...
Cycles glass uses physical transmission through an actual annular wall.
"""
import math
import bpy
...
def tube(name, p, outer, inner, length, material, axis='Z'):
    ...
    faces=[]
    # Separate rim vertices prevent annular end-cap normals bending the long
    # cylindrical walls into an hourglass under physical glass transmission.
    verts += [v.copy() for v in verts]
    for i in range(n):
        j=(i+1)%n
        faces += [(i,j,n+j,n+i), (2*n+i,3*n+i,3*n+j,2*n+j),
                  (4*n+i,6*n+i,6*n+j,4*n+j), (5*n+i,5*n+j,7*n+j,7*n+i)]
    return k.mesh(name,verts,faces,material,0,True)


def materials():
    k.M['cream'] = k.material('Astra ivory painted steel', 'B6B4AB', .58, .16, .025, .00006)
    paint=k.M['cream']; geometry=paint.node_tree.nodes.new('ShaderNodeNewGeometry')
    for noise in [n for n in paint.node_tree.nodes if n.type=='TEX_NOISE']:
        for link in list(noise.inputs['Vector'].links):paint.node_tree.links.remove(link)
        paint.node_tree.links.new(geometry.outputs['Position'],noise.inputs['Vector'])
        if noise.inputs['Scale'].default_value>100:noise.inputs['Scale'].default_value=85
    for bump in [n for n in paint.node_tree.nodes if n.type=='BUMP']:
...
def open_shell_inlet():
    # R31 labelled section exposed the old Boolean's pocket floor. Remove only
    # its cutter faces, preserving the original oval skin and end silhouettes.
    import bmesh
    o=k.S.objects['CD shell'];bm=bmesh.new();bm.from_mesh(o.data)
    faces=[]
    for f in bm.faces:
        p=f.calc_center_median()
        if abs(p.x-3)<=1.1901 and abs(p.y-4.05)<=.6901 and 4.3449<=p.z<=4.8951:
            faces.append(f)
    assert 3<=len(faces)<=12, f'Unexpected inlet pocket topology: {len(faces)} faces'
    k.S['astra_inlet_pocket_faces_removed']=len(faces)
    bmesh.ops.delete(bm,geom=faces,context='FACES');bm.to_mesh(o.data);bm.free()
    for m in list(o.modifiers):o.modifiers.remove(m)
    wall=o.modifiers.new('Actual oval shell wall','SOLIDIFY')
    wall.thickness=.025;wall.offset=-1;wall.use_even_offset=True
    k.bevel(o,.005)
    k.S['astra_shell_wall_thickness_m']=.025


def condenser_joints():
    k.group('10 Condenser CD-01')
    remove_prefix('CD stiffener')
    cx,cy,_=CD_C;cz=(CD_BODY_Z0+CD_BODY_Z1)/2
    ry=CD_WID_Y/2;rz=(CD_BODY_Z1-CD_BODY_Z0)/2
    for index,x in enumerate([cx-CD_LEN_X/2+.35,cx,cx+CD_LEN_X/2-.35]):
        # Oval stiffeners follow the vessel. The centre band terminates into the
        # chest sidewalls, leaving the actual steam inlet unobstructed.
        partial=index==1;n=96
        cut=math.acos(.85/(ry+.025))
        start,end=(math.pi-cut,math.tau+cut) if partial else (0,math.tau)
        count=n+1 if partial else n;verts=[]
        for i in range(count):
            a=start+(end-start)*i/n
            for xx,yy,zz in [(x-.045,ry+.025,rz+.025),(x+.045,ry+.025,rz+.025),
                              (x+.045,ry-.003,rz-.003),(x-.045,ry-.003,rz-.003)]:
                verts.append((xx,cy+yy*math.cos(a),cz+zz*math.sin(a)))
        faces=[]
        for i in range(n):
            nxt=(i+1)%count
            for j in range(4):faces.append((4*i+j,4*nxt+j,4*nxt+(j+1)%4,4*i+(j+1)%4))
        if partial:faces += [(3,2,1,0),tuple(4*n+j for j in range(4))]
        k.mesh('CD oval stiffener',verts,faces,'cream',.001,False)
        k.box('CD band splice',(x,cy-ry-.044,cz),(.12,.035,.11),'steel',.002)
        for dx in [-.035,.035]:k.bolt('CD band splice bolt',(x+dx,cy-ry-.070,cz),'Y',.010)
    k.S.objects['CD-01 paint'].location.x=cx+.68
    # The old flat-bottom chest began at the oval's crown, leaving its side
    # skirts above the curved shell. Extend only these four existing walls.
    for o in k.S.objects:
        if o.name.startswith('CD chest wall'):
            o.location.z=(4.18+5.36)/2;o.dimensions.z=5.36-4.18
    def crown(y):return cz+rz*math.sqrt(max(0,1-((y-cy)/ry)**2))
    for side in [-1,1]:
        y=cy+side*.854
        k.pipe('CD chest shell weld',[(cx-1.35,y,crown(y)),(cx+1.35,y,crown(y))],.007,'cream')
        points=[(cx+side*1.354,cy-.85+i*1.7/32,crown(cy-.85+i*1.7/32)) for i in range(33)]
        k.pipe('CD chest curved weld',points,.007,'cream')
    # Visible mounts make the existing flat nameplate stand-off deliberate.
    z=cz-.55;skin_y=cy-ry*math.sqrt(1-((z-cz)/rz)**2)
    for x in [cx-.20,cx+.20]:
        k.rod('CD nameplate mounting post',(x,skin_y+.005,z),(x,cy-ry-.020,z),.014,'steel')
...
def exhaust_fasteners():
    k.group('10 Condenser CD-01')
    # R33 C09 exposed legacy chest fasteners about 60mm below the flange.
    # Seat the existing washer/head/shank stacks on its actual lower face.
    for o in k.S.objects:
        if not o.name.startswith('chest bolt'):continue
        if 'washer' in o.name:o.location.z=5.3003-o.dimensions.z/2
        elif 'head' in o.name:o.location.z=5.3003-.00512-o.dimensions.z/2
        elif 'shank' in o.name:o.location.z=5.303
...
def secondary_construction():
    ...
    k.S.objects['pull bay wall t'].data.body='BUNDLE SERVICE / KEEP CLEAR'
