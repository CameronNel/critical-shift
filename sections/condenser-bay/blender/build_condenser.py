...
CAMERAS = [
    ...
    # East gallery standing, looking west into the 2.5 x 1.5 slab rectangle.
...
WALK = [
    ...
    ("W05_SE", (8.50, 1.58, 1.62), (6.15, 5.25, 2.05), 24),
...
def write_interface():
    data = {
        ...
        "envelopes": {
            ...
            "floor_z": 0,
            "ceiling_z": Z1,
            "justification": "Preserved R21 11.4 x 9.4 x 6.0 m room around condenser, west service space, east staging reserve and south stairs. Saved audits sample 2.2 m floor routes and approximately 2.0 m gallery/stair standing height; broad planning zones are not certified clear volumes.",
        },
        "portals": [
            {
                "id": "D01",
    ...
    data['circulation']['west_cart_lane']={'min':[-.35,.05,0],'max':[.45,6.25,2.2],
        'note':'Measured usable strip within the broader west equipment aisle; straight approach only.'}
    data['circulation']['gallery_deck_z']=float(S.get('astra_gallery_deck_z',4.18))
...
def main():
    ...
    cameras()
    settings()
    # Apply Grok's baseline shadow flags before adding Astra construction,
    # matching the immutable R21 continuation path.
    ...
    configure_scene(S,args.samples)
    ...
    if validate_main():
        raise RuntimeError('CPU validation failed; inspect revision report')
    if args.render:
