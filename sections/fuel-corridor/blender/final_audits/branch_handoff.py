"""Independent frozen eng11 branch audit. Read-only scene; CPU; no render/save."""
...
START=time.time()
BASE=Path(__file__).resolve().parents[2]/'production/critics'
SNAP=BASE.parent/'checkpoints/final-F10/blender'
OUT=BASE.parent/'evidence/final-pass/F10-branch-handoff.json'
contract=json.loads((SNAP/'interface.json').read_text())
previous_contract=json.loads((BASE.parent/'checkpoints/eng08/interface.json').read_text())
def json_diff(a,b,path='$'):
    ...
    if isinstance(a,dict):
        ...
        for key in sorted(set(a)|set(b)):
            ...
            else:out.extend(json_diff(a[key],b[key],path+'.'+key))
        return out
    if isinstance(a,list):
        if len(a)!=len(b):return [{'path':path,'before':a,'after':b}]
        return [v for i,(x,y) in enumerate(zip(a,b)) for v in json_diff(x,y,path+'['+str(i)+']')]
...
report['interface_semantic_changes_vs_eng08']=json_diff(previous_contract,contract)
report['unchanged_contract_fields_vs_eng08']={k:contract[k]==previous_contract[k] for k in ['ports','floor_cells','route_centerlines','handling','travel']}
...
report['local_sign_underpasses']=[]
for label,start,end in [('Plant_blade_underpass',(0,15.5,0),(0,16.5,0)),('Clean_blade_underpass',(4.4,19.2,0),(5.5,19.2,0))]:
    a,b=Vector(start),Vector(end);n=(b-a).normalized();length=(b-a).length;t=Vector((-n.y,n.x,0));width=2.0;height=2.2
    corners=[q+t*u+Vector((0,0,z)) for q in [a,b] for u in [-1,1] for z in [0,height]];roi=bounds(corners)
    gs=[g for g in geoms.values() if not g['decal'] and g['type']!='FONT' and all(g['b'][1][i]>=roi[0][i]-.1 and g['b'][0][i]<=roi[1][i]+.1 for i in range(3))];pack=combined(gs)
    counts=collections.Counter();hits=[];total=0
    for z in steps(.005,2.195,.05):
        for u in steps(-.998,.998,.05):
            total+=1;hit=cast(pack,a+t*u+Vector((0,0,z)),n,length)
            if hit:counts[hit['object']]+=1;hits.append(hit)
        for distance in steps(0,length,.05):
            total+=1;hit=cast(pack,a+n*distance-t*.998+Vector((0,0,z)),t,1.996)
            if hit:counts[hit['object']]+=1;hits.append(hit)
    floor_fail=[]
    for distance in steps(0,length,.10):
        for u in [-.9,0,.9]:
            hit=cast(pack,a+n*distance+t*u+Vector((0,0,.10)),Vector((0,0,-1)),.30)
            if not hit or abs(hit['point'][2])>.005:floor_fail.append({'distance':distance,'u':u,'hit':hit})
    report['local_sign_underpasses'].append({'name':label,'start':start,'end':end,'width':2.0,'height':2.2,'rays':total,'blocked':sum(counts.values()),'objects':dict(counts),'sample_hits':hits[:15],'floor_failures':floor_fail,'scope':'Independent 50mm crosshatch through local sign region; not full bypass, mount integrity, or engine collision proof.'})
for p in ports[:2]:
