...
SAVED=SECTION/'production/checkpoints/final-F10/blender/Fuel_Corridor.blend'
EXPECTED='a8e742a10cf5691098adabc0c2518384db3d82444517637bd67e362af4146403'
...
assert scene.get('stage')=='full' and scene.get('revision')=='final-F10'
...
report={'blend':str(SAVED),'blend_sha256':EXPECTED,'revision':scene['revision'],'stage':scene['stage'],
        ...
        'provenance':'Builder replay of retained independent eng03 algorithm on final-F10; not a new independent reviewer report','no_render_no_save_no_scene_transform_mutation':True,'elapsed_seconds':time.perf_counter()-start}
assert hashlib.sha256(SAVED.read_bytes()).hexdigest()==EXPECTED
(SECTION/'production/evidence/final-pass/F10-engineering-motion.json').write_text(json.dumps(report,indent=2))
