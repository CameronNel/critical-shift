...
def gate_drive_detail():
    ...
    rod('Gate_motor',(1.28,.030,3.96),(1.54,.030,3.96),.078,'motor_stator',48)
    for side,profile in enumerate([[(1.26,.073),(1.264,.087),(1.277,.090),(1.291,.086)],
                    [(1.526,.086),(1.542,.090),(1.556,.084),(1.56,.073)]]):
        bell=lathe_x('Motor_end_bell',profile,.03,3.96,'motor_coat',48)
        bell.data.materials.append(M['steel']);bell.data.materials.append(M['ochre'])
        for p in bell.data.polygons[2:]:
            band=(p.index-2)//48
            if band==0:p.material_index=1
            elif side==0 and band==1:p.material_index=2
        # Split normals at manufactured profile rings, retaining smooth circular
        # surfaces between them; the end bells should read as cut/cast metal.
        for edge in bell.data.edges:
            if edge.vertices[0]//48==edge.vertices[1]//48:edge.use_edge_sharp=True
    for i in range(18):
        angle=math.tau*i/18
        if abs(angle-math.pi/2)<.24:continue
        cy,sz=math.cos(angle),math.sin(angle)
        # Tangential thickness 5 mm; radial limits 77.5–100 mm.
        ob=box('Motor_cooling_fin',(1.409,.03+cy*.08875,3.96+sz*.08875),(.232,.0225,.005),'motor_stator',.0007)
        ob.rotation_euler.x=angle
        if i in [6,8,10]:
            u=1.32+(i%3)*.042
            ob=mesh('Motor_fin_edge_wear',[(u,.03+cy*.1001,3.96+sz*.1001),(u+.043,.03+cy*.1001,3.96+sz*.1001),(u+.031,.03+cy*.1001-sz*.0024,3.96+sz*.1001+cy*.0024),(u+.005,.03+cy*.1001-sz*.0024,3.96+sz*.1001+cy*.0024)],[(0,1,2,3)],'steel');ob['surface_decal']=True
    for a in [2.65,3.12,3.51]:
        verts=[]
        for xx,aa in [(1.531,a),(1.539,a+.018),(1.538,a+.075),(1.532,a+.063)]:
            rr=.086+(xx-1.526)*.25+.00015
            verts.append((xx,.03+math.cos(aa)*rr,3.96+math.sin(aa)*rr))
        ob=mesh('Motor_bell_edge_wear',verts,[(0,1,2,3)],'steel');ob['surface_decal']=True
    ...
    for xx in [1.33,1.395]:
        rod('Motor_mount_anchor',(xx,.104,3.74),(xx,.177,3.74),.004,'steel',24)
        rod('Motor_mount_anchor_head',(xx,.102,3.74),(xx,.111,3.74),.006,'steel',6)
    box('Motor_reducer_guard',(1.192,.03,3.884),(.136,.154,.254),'teal_light',.008)
    # Wear remains on the guard's flat exposed face; exclude the orange band
    # and beveled right edge rather than leave floating/partly buried patches.
    for xx,zz in [(1.136,3.980),(1.139,3.895),(1.144,3.781)]:
        ob=mesh('Motor_guard_edge_wear',[(xx,-.04715,zz),(xx+.012,-.04715,zz+.002),(xx+.009,-.04715,zz+.004),(xx-.001,-.04715,zz+.003)],[(0,1,2,3)],'steel');ob['surface_decal']=True
    ...
    cable=[Vector(p) for p in [(1.479,.03,4.079),(1.502,.03,4.079),(1.523,.045,4.087),(1.534,.071,4.111),(1.534,.108,4.153),(1.534,.15,4.178),(1.534,.213,4.184),(1.534,.247,4.184)]]
    rounded=[cable[0]]
    for a,p,b in zip(cable,cable[1:],cable[2:]):
        reach=min(.012,(p-a).length/3,(b-p).length/3)
        lo=p+(a-p).normalized()*reach;hi=p+(b-p).normalized()*reach
        rounded.extend((1-t)**2*lo+2*(1-t)*t*p+t*t*hi for t in [i/6 for i in range(7)])
    rounded.append(cable[-1]);tube('Motor_supply_cable',rounded,.007,'rubber')
    ...
    if A.stage=='full':
        ...
        area('Motor_task_pool',(5.77,9.03,4.124),(6.0,8.72,3.80),10,(1,.83,.65),.16,.09)
...
def detail_dressing(stage):
    ...
    route_sign('Service_bypass_sign','SERVICE / BYPASS',(0,13.10),0,3.37,1.85)
    route_sign('Freight_advance_sign','F02 / REACTOR',(2.16,13.2),0,2.73,1.55)
    bpy.data.objects['Freight_advance_sign_panel'].data.materials[0]=M['ochre']
    label=bpy.data.objects['Freight_advance_sign_letters'];label.data.materials[0]=M['ink'];label.data.size=.14;label.data.offset=.002
    for ob in bpy.data.objects:
        if ob.name.startswith('Freight_advance_sign_direction_chevron'):ob.data.materials[0]=M['ink']
    ...
    if stage=='full':
        ...
        for name,origin,angle,z in [('Entry_wall_key',(-2.2,4.8),math.pi/2,2.72),('Cross_wall_key',(9.1,12.2),0,2.72),('East_wall_key',(16.4,9.92),-math.pi/2,2.94),('Delivery_wall_key',(12,17.35),math.pi/2,2.72),('Service_corner_key',(-1.5,20.6),math.pi/2,2.36),('Medical_side_key',(8.5,21),0,2.34)]:wall_light(name,origin,angle,z,105)
        wall_vent('Entry_extract',(2.2,4.6),-math.pi/2,3.35,1.02)
        wall_vent('East_extract',(13.35,7),math.pi,3.35,1.12)
        wall_vent('Service_extract',(10.73,21),0,2.26,.40)
...
def first_aid_pack(name,origin,heading=0):
    ...
    # One continuous stamped cross; overlapping bars had ambiguous coplanar shading.
    outline=[(-.0175,-.068),(.0175,-.068),(.0175,-.0175),(.0565,-.0175),(.0565,.0175),(.0175,.0175),(.0175,.068),(-.0175,.068),(-.0175,.0175),(-.0565,.0175),(-.0565,-.0175),(-.0175,-.0175)]
    cross=prism(name+'_cross',[(x,z+.044+zz) for x,zz in outline],.089,.093,'ochre')
    cross.rotation_euler.x=math.pi/2
