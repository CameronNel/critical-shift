...
mat('teal',(.035,.048,.067),.49,.28,.07,.0007)
mat('teal_light',(.08,.105,.14),.60,.18,.055,.0008)
mat('steel',(.21,.255,.31),.31,.83,.04,.0004)
mat('darksteel',(.021,.030,.045),.57,.40,.055,.0008)
...
def shell(stage):
    ...
    root('Bay_steelwork','wall',[(-2.040,7.25,3.64),(-2.2,12.95,3.64)],(-1,0,0))
    ...
    for yy in [7.25,12.95]:
        ...
        for xx in [-2.13,4.33]:
            ...
            box('Bearing_plate',(xx+(.16 if yy==7.25 and xx<0 else 0),yy,3.64),(.14,.35,.5),'steel')
    pipework()
    if stage=='full':
