...
def folded_cloth(name,x,y,z,w=.24,d=.19):
    ...
    for j in range(7):
        for i in range(11):
            u,v=i/10,j/6
            vv.append((x-w/2+u*w+.003*math.sin(v*8.3),y-d/2+v*d+.002*math.sin(u*9),z+.004+.006*math.sin(math.pi*u)*math.sin(math.pi*v)+.0015*math.sin(u*17+v*5)))
    for j in range(6):
...
def laid_glove(name,x,y,mirror=1,angle=0):
    ...
    outline=[xy(u,v) for u,v in points]
    if mirror<0:outline.reverse()
    ob=prism(name+'_sewn_shell',outline,.906,.919,'glove',.003)
    # The leather palm reinforcement sits flush, with a shaped edge and a stitched cuff.
    patch=[(-.034,-.039),(.035,-.041),(.040,-.014),(.022,.044),(-.029,.042),(-.043,.017)]
    poly=[xy(u,v) for u,v in patch]
    if mirror<0:poly.reverse()
    prism(name+'_palm_reinforcement',poly,.918,.9205,'rubber',.001)
    for side in [-1,1]:
        for j in range(7):
            u=side*.034;v=-.034+j*.010
            aa=xy(u,v);bb=xy(u,v+.0035);rod(name+'_sewn_stitch',(*aa,.921),(*bb,.921),.0005,'cloth',6)
    for j in range(4):
        v=-.063+j*.005;aa=xy(-.040,v);bb=xy(.040,v)
        rod(name+'_cuff_weave',(*aa,.920),(*bb,.920),.0009,'rubber',6)
    for u,v,w in [(-.030,.070,.013),(-.002,.083,.014),(.029,.074,.012),(-.058,.059,.011)]:
        aa=xy(u-w/2,v);bb=xy(u+w/2,v+.003);rod(name+'_finger_fold',(*aa,.920),(*bb,.920),.0007,'rubber',6)
...
def utility_station(name,origin,heading=0,inlet_z=2.25):
    ...
    box(name+'_regulator_identity',(.07,-.238,1.837),(.073,.002,.045),'teal_light',.002)
...
def tool_board():
    ...
    for i in range(4):
        ...
        box('Spanner_recessed_flute',(xx,-.1275,zz-.046-length*.46),(.008*factor,.001,length*.57),'darksteel',.002)
