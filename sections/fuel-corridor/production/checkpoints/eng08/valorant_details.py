...
def gate_drive_detail():
    ...
    box('Motor_vertical_mount',(1.40,.171,3.815),(.22,.048,.19),'steel',.003)
    for xx in [1.33,1.47]:rod('Motor_mount_anchor',(xx,.146,3.79),(xx,.164,3.79),.007,'steel',6)
    box('Motor_reducer_guard',(1.192,.03,3.884),(.136,.154,.254),'teal',.016)
    ...
    box('Motor_terminal_box',(1.412,.03,4.076),(.086,.070,.052),'structural_coat',.004)
    ...
    box('Motor_cable_clamp_back',(1.534,.243,4.184),(.038,.014,.049),'steel',.002)
    box('Motor_cable_retainer',(1.534,.220,4.184),(.025,.010,.026),'teal',.002)
    rod('Motor_cable_clamp_stud',(1.534,.220,4.202),(1.534,.249,4.202),.004,'steel',6)
    ...
    if A.stage=='full':
        ...
        box('Motor_task_arm',(0,-.065,4.19),(.05,.086,.02),'steel',.002)
        box('Motor_task_hood',(0,-.11,4.20),(.24,.15,.055),'teal',.005)
        box('Motor_task_diffuser',(0,-.11,4.169),(.20,.12,.007),'lamp',.002)
        for xx in [-.045,.045]:rod('Motor_task_anchor',(xx,-.024,4.16),(xx,-.006,4.16),.005,'steel',6)
        place_wall_group(lamp,(6.22,9.03),-math.pi/2)
        area('Motor_task_pool',(6.11,9.03,4.164),(6.0,8.67,3.95),8,(1,.78,.53),.19,.10)
