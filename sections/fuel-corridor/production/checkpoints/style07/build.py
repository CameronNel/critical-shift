...
def mat(name,color,rough=.6,metal=0,variation=.05,bump=.02,emission=0):
    ...
    if variation:
        ...
        for i,(pos,v) in enumerate([(.30,-1),(.40,-.5),(.475,0),(.55,.5),(.65,1)]):
            e=ramp.color_ramp.elements[0] if i==0 else ramp.color_ramp.elements.new(pos);e.position=pos;e.color=(*[min(1,max(0,c*(1+variation*v))) for c in color],1)
...
mat('steel',(.21,.255,.31),.31,.83,.04,.0004)
mat('darksteel',(.043,.052,.066),.57,.40,.075,.0008)
mat('rubber',(.025,.03,.029),.93,.0,.04,.008)
mat('ochre',(.80,.115,.004),.51,.12,.055,.0005)
mat('ivory',(.46,.49,.53),.61,.10,.05,.0006)
...
mat('wall_patch',(.27,.30,.335),.86,variation=.025,bump=.0003)
mat('wall_light',(.375,.40,.43),.83,variation=.025,bump=.0003)
mat('brass',(.43,.27,.09),.35,.72,.025,.0004)
mat('floor_stroke',(.218,.235,.256),.82,variation=.04,bump=.001)
...
def practical(name,x,y,z=4.4,power=320,cool=False,rotate=0):
    ...
    area(name+'_light',(x,y,z-.35),(x,y,0),power,(.78,.88,1) if cool else (1,.67,.36),1.35,.22)
