...
def mat(name,color,rough=.6,metal=0,variation=.05,bump=.02,emission=0):
    ...
    if variation:
        ...
        rr=n.new('ShaderNodeMapRange');rr.inputs['From Min'].default_value=.32;rr.inputs['From Max'].default_value=.68;rr.inputs['To Min'].default_value=max(.1,rough-.12);rr.inputs['To Max'].default_value=min(1,rough+.09);l.new(tex.outputs['Fac'],rr.inputs[0]);l.new(rr.outputs[0],p.inputs['Roughness'])
        if bump:
...
mat('steel',(.21,.255,.31),.42,.70,.09,.0008)
mat('darksteel',(.043,.052,.066),.57,.40,.075,.0008)
        ...
        for za,zb in [(1.325,h*.61-.008),(h*.61+.008,h-.35)]:
            ...
            for t in [pa+.055,pb-.055]:
                for zz in [za+.055,zb-.055]:
                    q=a+d*t-out*.004+Vector((0,0,zz));rod(name+'_panel_fastener',q+out*.007,q-out*.002,.007,'steel',6)
    for i in range(divisions+1):
        along=max(.045,min(L-.045,L*i/divisions));p=a+d*along
        o=box(name+'_vertical_web',p-out*.080+Vector((0,0,h/2)),(.07,.16,h),'darksteel',.003);o.rotation_euler.z=math.atan2(d.y,d.x)
        for off in [.014,.146]:
            o=box(name+'_vertical_flange',p-out*off+Vector((0,0,h/2)),(.21,.028,h),'teal',.004);o.rotation_euler.z=math.atan2(d.y,d.x)
        if L>.45:
...
def door(name,center,w,h,heading=0,closed=False):
    ...
    PAR=parent
    if name=='FREIGHT_GATE':
        parent['assembly_role']='floor';parent['support_anchors']=json.dumps([pos(s*(w/2+.09),0,0) for s in [-1,1]]);parent['support_direction']=[0,0,-1]
        # Suspended leaves have a real load path through the overhead track.
        for s in [-1,1]:bx('_leaf_carriage_hanger',(s*(w/2+.20),.12,h-.01),(.085,.08,.22),'steel')
...
def practical(name,x,y,z=4.4,power=320,cool=False,rotate=0):
    ...
    area(name+'_light',(x,y,z-.35),(x,y,0),power,(.78,.88,1) if cool else (1,.78,.55),1.35,.22)
...
def shell(stage):
    ...
    for c in cells:
        ...
        ceil=box('Ceiling_'+c['id'],((x0+x1)/2,(y0+y1)/2,h+.118),(x1-x0+.02,y1-y0+.02,.2),'darksteel',0);ceil['support_surface']=True
        ...
        for ix in range(nx):
            for iy in range(ny):
                ...
                if (ix+iy)%3==1:
                    wx=xb-xa;wy=yb-ya
                    coords=[(xa,ya+.13*wy),(xa+.26*wx,ya+.21*wy),(xa+.34*wx,ya+.42*wy),(xa+.61*wx,ya+.47*wy),(xb,ya+.91*wy),(xa+.83*wx,yb),(xa+.39*wx,ya+.71*wy),(xa+.16*wx,ya+.66*wy)]
                    # Alternate a few broad clipped strokes rather than repeat a stamp.
                    cxp=(xa+xb)/2;cyp=(ya+yb)/2
                    coords=[(cxp+(px-cxp)*random.uniform(.72,1),cyp+(py-cyp)*random.uniform(.55,1)) for px,py in coords]
                    if ix%2:coords=[(2*cxp-px,py) for px,py in reversed(coords)]
                    prism('Broad_concrete_brush',coords,.0013,.0016,'floor_stroke',0)
    ...
    pipework()
    if stage=='full':
        root('Longitudinal_tray','ceiling',[(xx,8.14,4.4) for xx in [5.3,8.5,11.8,15.1]]+[(15.85,yy,4.4) for yy in [14.5,18.5]],(0,0,1))
        for yy in [8.0,8.28]:
        ...
        for i in range(3):tube('Power_cable',[(4,8.04+i*.055,3.762)]+[(4.6+j*.5,8.04+i*.055,3.762) for j in range(22)]+[(15.3,8.04+i*.055,3.762),(15.80+i*.055,8.6,3.762)]+[(15.80+i*.055,8.9+j*.5,3.762) for j in range(24)],.021,'rubber')
        ...
        for xx in [15.71,15.99]:box('Delivery_tray_rail',(xx,14.39,3.78),(.035,12.5,.12),'darksteel')
        for i in range(26):box('Delivery_tray_rung',(15.85,8.14+i*.48,3.73),(.28,.024,.026),'steel')
        for yy in [14.5,18.5]:
            rod('Delivery_tray_drop',(15.85,yy,4.4),(15.85,yy,3.69),.012,'steel')
            box('Delivery_tray_saddle',(15.85,yy,3.704),(.34,.09,.035),'steel')
...
practical('Staging_warm_key',.15,11.78,4.4,220)
practical('Gate_cool_fill',3.32,8.45,4.4,105,True,math.pi/2)
practical('Staging_front_practical',1.10,7.65,4.4,180)
if A.stage=='full':
...
(out/'build_manifest.json').write_text(json.dumps({'revision':A.revision,'stage':A.stage,'source_sha256':S['source_sha256'],'detail_source_sha256':S['detail_source_sha256'],'interface_sha256':S['interface_sha256'],'blend_sha256':hashlib.sha256(blend.read_bytes()).hexdigest(),'objects':len(S.objects),'cameras':CAMERAS,'blend':str(blend),'samples':A.samples,'width':A.width},indent=2))
if A.render:
