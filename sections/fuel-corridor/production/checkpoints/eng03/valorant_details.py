...
def wall_light(name,origin,heading=0,z=2.55,power=95):
    ...
    place_wall_group(par,origin,heading)
    c,s=math.cos(heading),math.sin(heading)
    area(name+'_pool',(origin[0]+s*.26,origin[1]-c*.26,z-.052),(origin[0]+s*.20,origin[1]-c*.20,.90),power*.52,(1,.75,.48),.20,.10)
...
def route_sign(name,label,origin,heading=0,z=2.82,width=1.70):
    ...
    for xx in [width*.5-.22,width*.5-.13]:
        ob=mesh(name+'_direction_chevron',[(xx-.035,-.0395,z-.052),(xx+.005,-.0395,z),(xx-.035,-.0395,z+.052),(xx,-.0395,z+.052),(xx+.04,-.0395,z),(xx,-.0395,z-.052)],[(0,1,2,3,4,5)],'ochre')
...
def detail_dressing(stage):
    ...
    if stage=='full':
        utility_station('Service_air_station',(-1.65,15.00),math.pi/2,2.14)
        for name,origin,angle,z in [('Entry_wall_key',(-2.2,4.8),math.pi/2,2.72),('Cross_wall_key',(9.1,12.2),0,2.72),('East_wall_key',(16.4,11.45),-math.pi/2,2.72),('Delivery_wall_key',(12,17.35),math.pi/2,2.72),('Service_corner_key',(-1.5,20.35),math.pi/2,2.36),('Medical_side_key',(8.5,21),0,2.34)]:wall_light(name,origin,angle,z,105)
...
def distribution_box(name,origin,heading=0,z=1.62,compact=False):
    ...
    for xx,radius in [(-w*.21,.012),(w*.10,.019)]:
        tube(name+'_conduit',[(xx,-.055,z+h/2),(xx,-.055,z+h/2+.36),(xx+.045,-.055,z+h/2+.405),(xx+.045,-.055,z+h/2+.61)],radius,'steel')
        for zz in [z+h/2+.11,z+h/2+.47]:
            box(name+'_clamp_standoff',(xx+(0 if zz<z+h/2+.4 else .045),-.027,zz),(.052,.054,.04),'darksteel',.003)
        box(name+'_termination',(xx+.045,-.032,z+h/2+.64),(.08,.064,.08),'teal',.004)
    if not compact:
...
def first_aid_pack(name,origin,heading=0):
    z=1.79;par=local_wall_root(name,origin,heading,z)
...
def route_storytelling():
    ...
    painted_route('East_reactor_route',(16.4,10.12),-math.pi/2,'04 / REACTOR','FUEL TRANSFER',-1,1.38,1.96)
    painted_route('Cross_service_identity',(7.35,12.2),0,'FUEL ROUTE','KEEP TURN CLEAR',1,1.00,1.85)
    painted_route('Bypass_clean_route',(-.53,21),0,'SERVICE / 02','MEDICAL / CLEAN',1,1.18,1.86)
    ...
    distribution_box('Bypass_isolation',(1.2,15.08),-math.pi/2,1.53,True)
    first_aid_pack('Bypass_first_aid',(1.2,16.17),-math.pi/2)
    permit_holder('Bypass_work_permit',(1.2,16.73),-math.pi/2,1.80)
    permit_holder('Plant_work_permit',(-3.15,16.2),math.pi,1.67)
    distribution_box('Clean_distribution',(9.7,18),math.pi,1.60,True)
    for i,(p,hd) in enumerate([((16.4,9.6),-math.pi/2),((16.4,11.8),-math.pi/2),((8.0,12.2),0),((12,14.3),math.pi/2),((12,18.4),math.pi/2),((1.2,14.6),-math.pi/2),((-1.5,20.2),math.pi/2),((-.55,21),0),((3.2,18),math.pi),((8.8,18),math.pi),((-3.4,18.6),0)]):low_guide('Route_guide_%02d'%i,p,hd)
    # A warm turn light shows the existing northward continuation from the main crossing.
    wall_light('Reactor_turn_key',(16.4,12.4),-math.pi/2,2.72,180)
    wall_light('Refinery_approach_key',(0,.685),math.pi,3.41,160)
    wall_light('Reactor_left_key',(11.4,22.8),math.pi/2,2.55,180)
    wall_light('Reactor_right_key',(17.0,22.8),-math.pi/2,2.55,180)
