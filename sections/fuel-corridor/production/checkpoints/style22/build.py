...
def door(name,center,w,h,heading=0,closed=False):
    ...
    bx('_lintel',(0,.05 if A.stage=='full' else 0,h+.13),(w+.40,.52 if A.stage=='full' else .32,.26),'darksteel')
...
def shell(stage):
    ...
    if stage=='full':
        ...
        tube('Recess_air_branch',[(.91,14.55,2.67),(-1.06,14.55,2.67),(-1.06,14.55,2.16),(-1.49,14.55,2.14)],.025,'steel')
        rod('Service_branch_union',(-1.38,14.55,2.146),(-1.45,14.55,2.142),.046,'steel')
...
if A.stage=='full':
    for n,x,y,h,p,co,r in [('Entry',.0,4.0,4.4,270,False,0),('Cross',8.2,10,4.4,310,False,math.pi/2),('East',14.5,10.8,4.4,330,True,0),('Delivery',14.2,17,4.4,310,False,0),('Reactor',14.2,22.8,5.9,550,True,0),('Bypass',0,16.5,3,155,False,0),('North_service',4.6,19.8,3,155,True,math.pi/2),('North_service_2',10,19.8,3,130,False,math.pi/2),('Plant',-3.6,17.4,3,135,False,math.pi/2)]:practical(n,x,y,h,p,co,r)
else:
...
if A.stage=='full':
    ...
    handoff['incident_hooks']=[marker(name,'incident',position,{'event':event,'target_assembly':target,'runtime_handler':'pending'}) for name,position,event,target in [('FC_HOOK_AIR_LEAK',(-1.49,14.55,2.14),'air_leak','Service_air_station'),('FC_HOOK_GATE_JAM',(6.35,10,3.7),'freight_gate_jam','FREIGHT_GATE'),('FC_HOOK_RESTRAINT',(2.65,12.32,.76),'carrier_restraint_fault','Long_cask_carrier')]]
    ...
    for obj in S.objects:
        if obj.get('component_role')=='sliding_leaf_carriage':handoff['doors'].append({'carriage':obj.name,'pose':obj['current_pose'],'closed_to_open_section_vector_m':list(obj['closed_to_open_translation_m']),'members':sorted(c.name for c in obj.children),'controller_and_motion_validation':'see technical evidence; engine controller pending'})
    registry=[]
    for obj in S.objects:
