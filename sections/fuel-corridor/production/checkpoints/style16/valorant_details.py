...
def small_case(name,loc,dimensions=(.38,.24,.18),material='teal'):
    ...
    box(name+'_identity_plate',(x,y-d/2-.008,z+h*.42),(.17,.005,.040),'paper',.002)
    text_obj(name+'_kit_type','TOOLS / 04',(x,y-d/2-.011,z+h*.395),.014,'ink')
    for side in [-1,1]:
...
def utility_station(name,origin,heading=0,inlet_z=2.25):
    ...
    bowl=annular_x(name+'_hollow_glass_bowl',1.51,1.68,.07,-.16,.047,.041,'glass',48)
    for v in bowl.data.vertices:v.co=(v.co.y,v.co.z,v.co.x)
    rod(name+'_filter_core',(.07,-.16,1.515),(.07,-.16,1.66),.031,'steel',24)
    for i in range(12):
        a=i*math.tau/12;rod(name+'_filter_pleat',(.07+math.cos(a)*.031,-.16+math.sin(a)*.031,1.528),(.07+math.cos(a)*.031,-.16+math.sin(a)*.031,1.65),.0025,'darksteel',8)
    for i in range(12):
        a=i*math.tau/12
...
def workbench():
    ...
    for side in [-1,1]:
        ...
        outline=[(gx+u,gy+v) for u,v in [(-.043,-.056),(.043,-.056),(.050,-.016),(.041,.041),(.022,.054),(-.033,.051),(-.049,.013)]]
        prism('Work_glove_palm',outline,.907,.927,'glove',.006)
        for i in range(4):
            xx=gx-.037+i*.024
            ellipsoid('Glove_finger',(xx,gy+.074-(i%3)*.005,.915),(.023,.089-(i%3)*.010,.019),'glove')
        ellipsoid('Glove_thumb',(gx+side*.047,gy+.006,.916),(.050,.054,.021),'glove')
        box('Glove_cuff',(gx,gy-.07,.916),(.096,.050,.020),'rubber',.009)
        for sx in [-1,1]:
            for i in range(5):rod('Glove_seam_stitch',(gx+sx*.037,gy-.035+i*.011,.9275),(gx+sx*.037,gy-.030+i*.011,.9275),.0008,'cloth',6)
        box('Glove_cuff_woven_tag',(gx+.024,gy-.063,.926),(.018,.015,.0018),'ochre',0)
...
def tool_board():
    ...
    for i in range(4):
        ...
        box('Spanner_shank',(xx,-.119,zz-.032-length/2),(.028*factor,.015,length),'steel',.003)
