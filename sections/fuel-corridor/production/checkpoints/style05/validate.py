...
def routes_audit(scene, geometries, interface):
    ...
    surfaces = SurfaceSet([g for g in geometries.values() if not g.obj.hide_render and not cap_member(g.obj)])
...
def main():
    ...
    report = {
        ...
        'current_source_sha256': hashlib.sha256((ROOT / 'blender/build.py').read_bytes()).hexdigest(),
        'saved_detail_source_sha256': scene.get('detail_source_sha256'),
        'current_detail_source_sha256': hashlib.sha256((ROOT / 'blender/valorant_details.py').read_bytes()).hexdigest(),
    ...
    report['source_matches_saved_scene'] = (report['saved_source_sha256'] == report['current_source_sha256'] and report['saved_detail_source_sha256'] == report['current_detail_source_sha256'])
    gates = ['camera_and_render_settings', 'geometry', 'dependencies', 'support_contact', 'internal_routes']
