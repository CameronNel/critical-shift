...
def apply_wayfinding():
    ...
    specs=[
        ...
        ('Bypass_clean_route',(-.42,21),0,'S02','CLEAN','MEDICAL / COMPLIANCE',1),
        ('Cross_service_identity',(8,12.2),0,'F02','REACTOR','FREIGHT ROUTE',1),
        ...
        ('Waste_marker',(16.4,18.3),-math.pi/2,'S03','WASTE','WASTE TRANSFER',1),
    ...
    for name,origin,heading,code,title,subtitle,direction in specs:
        ...
        def text(suffix,body,x,z,size):
            cu=bpy.data.curves.new(name+suffix,'FONT');cu.body=body;cu.size=size;cu.align_x='CENTER';cu.offset=.0005
        ...
        for prefix,(code,title) in codes.items():
            ...
            if '_leaf_identity_text' in ob.name and ob.data.body in ['FUEL','PLANT','SERVICES','CLEAN','MEDICAL','WASTE','TRANSFER','REFINERY','REACTOR']:
                ...
                if ob.name in [prefix+'_leaf_identity_text',prefix+'_leaf_identity_text.002']:
                    ...
                    ob.data.body=code if is_left else title;ob.data.size=.245;ob.data.offset=.0005
