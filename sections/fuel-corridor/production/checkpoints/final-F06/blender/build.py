...
mat('motor_coat',(.16,.18,.205),.68,.16,.13,.0003)
mat('motor_stator',(.075,.09,.11),.63,.32,.16,.00022)
mat('motor_diffuser',(.83,.68,.45),.7,0,0,0,.45)
...
mat('header_ink',(.003,.004,.005),1,variation=0,bump=0)
M['header_ink'].node_tree.nodes['Principled BSDF'].inputs['Specular IOR Level'].default_value=0
...
mat('sign_coat',(.075,.088,.105),.88,0,.025,.0003)
mat('sign_letters',(.82,.80,.73),.91,0,0,0)
mat('reactor_door_paint',(.105,.13,.16),.68,.20,.18,.0004)
for node in M['reactor_door_paint'].node_tree.nodes:
    if node.type=='TEX_NOISE' and node.inputs['Scale'].default_value==19:node.inputs['Scale'].default_value=3.0
...
def wall(name,a,b,h=4.4):
    ...
    for i in range(1 if A.stage=='full' and L<.27 else divisions+1):
        ...
        # Rear flange stands 4mm proud of lower lining; coincident faces flicker on cold render.
        for off in [.018,.146]:
...
def door(name,center,w,h,heading=0,closed=False):
    ...
    for s in [-1,1]:
        ...
        leaf=bx('_sliding_leaf',(x,.12,h/2),(w/2-.025,.10,h-.06),'reactor_door_paint' if name=='REACTOR_BOUNDARY' else 'teal_light',.006);leaf['animation_axis']='carriage vector';leaf['travel_m']=w/2+.15;leaf['closed_state']=closed
        # Recessed rigidifying channels, a kick panel and seam tell sheet-metal construction.
        for zz in [.23,2.48 if name=='REACTOR_BOUNDARY' else h*.45,h-.29]:bx('_leaf_stiffener',(x,.057,zz),(w/2-.19,.025,.055),'teal')
        ...
        for e in [-1,1]:
            seam_low,seam_high=(2.54,h*.915) if name=='REACTOR_BOUNDARY' else (h*.465,h*.915)
            bx('_leaf_panel_seam',(x+e*(w/4-.18),.066 if A.stage=='full' else .049,(seam_low+seam_high)/2),(.008,.008,seam_high-seam_low),'darksteel',0)
            for zz in [.18,h-.18]:
        ...
        if external:
            ...
            field_w=w/2-.18;field_z=1.88 if h>4 else .91;field_h=1.10 if h>4 else .50
            ...
            title_x=x
            if name=='REACTOR_BOUNDARY':font=.43 if s<0 else .345;title_x=x+(.19 if s<0 else -.10)
            title=text_obj(name+'_leaf_identity_text',word,pos(title_x,.0455,field_z+.01),font,'teal',(math.pi/2,0,heading))
            if name=='REACTOR_BOUNDARY':title.data.offset=.007
            sub='TRANSFER' if name in ['REFINERY_BOUNDARY','REACTOR_BOUNDARY'] else 'AUTHORIZED ACCESS'
            text_obj(name+'_leaf_identity_text',sub,pos(title_x,.0455,field_z-.20 if h>4 else field_z-.16),.14 if h>4 else min(.095,field_w/13), 'teal',(math.pi/2,0,heading))
            if name=='REACTOR_BOUNDARY':
                for i in range(2 if s<0 else 1):
                    u=(-.90+i*.13) if s<0 else .98
                    ob=mesh(name+'_leaf_identity_mark',[pos(x+u+dx,.045,field_z+dz) for dx,dz in [(-.08,-.03),(.015,-.03),(.17,.31),(.075,.31)]],[(0,1,2,3)],'ochre');ob['surface_decal']=True
            for edge in [-1,1]:
            ...
            access_z=3.28 if h>4 else 1.89;access_w=min(1.05 if h>4 else .72,field_w*.65);access_h=.82 if h>4 else .49
            bx('_leaf_access_panel',(x,.055,access_z),(access_w,.030,access_h),'teal',.006)
            for edge in [-1,1]:
                for zz in [access_z-access_h/2+.05,access_z+access_h/2-.05]:bolt(name+'_leaf_access_fastener',pos(x+edge*(access_w/2-.05),.032,zz),(math.sin(heading),-math.cos(heading),0),.018 if h>4 else .012)
            ...
            if name=='REACTOR_BOUNDARY':
                # Chamfered pressed border stiffens each tall sheet around its
                # service hatch; a distinct construction detail at reactor scale.
                outer=[(-.90,2.70),(-.76,2.56),(.76,2.56),(.90,2.70),(.90,4.39),(.76,4.53),(-.76,4.53),(-.90,4.39)]
                inner=[(-.868,2.714),(-.746,2.592),(.746,2.592),(.868,2.714),(.868,4.376),(.746,4.498),(-.746,4.498),(-.868,4.376)]
                verts=[pos(x+u,yy,zz) for contour,yy in [(outer,.047),(inner,.047),(inner,.070),(outer,.070)] for u,zz in contour]
                faces=[(j*8+i,j*8+(i+1)%8,((j+1)%4)*8+(i+1)%8,((j+1)%4)*8+i) for j in range(4) for i in range(8)]
                mesh(name+'_leaf_pressed_border',verts,faces,'teal')
                for dx in [-.69,.69]:
                    for zz in [2.70,4.39]:bolt(name+'_leaf_pressed_fastener',pos(x+dx,.063,zz),(math.sin(heading),-math.cos(heading),0),.011)
                service_pull=x-s*.67
                bx('_leaf_pull_back',(service_pull,.052,3.13),(.094,.036,.35),'steel',.009)
                bx('_leaf_pull_shadow',(service_pull,.031,3.13),(.055,.006,.29),'rubber',.007)
                for edge in [-1,1]:bx('_leaf_pull_rail',(service_pull+edge*.039,.023,3.13),(.016,.024,.31),'steel',.004)
                for zz in [2.972,3.288]:bx('_leaf_pull_end',(service_pull,.023,zz),(.090,.024,.016),'steel',.004)
                for zz in [.76,1.24]:bx('_leaf_stiffener',(x,.056,zz),(1.75,.028,.035),'old_teal',.003)
                for edge in [-1,1]:bx('_leaf_stiffener',(x+edge*.875,.056,1.0),(.035,.028,.515),'old_teal',.003)
                for i in range(16):
                    # Small edge-bound primer losses at lower handling/contact zones.
                    u=-.79+i*.105;zz=.785+(.018 if i%3==0 else 0);span=.012+(i%4)*.005
                    ob=mesh(name+'_leaf_handling_wear',[pos(x+u+du,.0694,zz+dz) for du,dz in [(0,0),(span,.002),(span*.80,.009),(.004,.006)]],[(0,1,2,3)],'primer' if i%3 else 'steel');ob['surface_decal']=True
                for edge in [-1,1]:
                    for zz in [2.991,3.569]:
                        ob=mesh(name+'_leaf_handling_wear',[pos(x+edge*.33+du,.0394,zz+dz) for du,dz in [(-.014,0),(.019,.002),(.011,.007),(-.008,.005)]],[(0,1,2,3)],'steel');ob['surface_decal']=True
                # Purposeful pallet-height coating losses on the exposed lower
                # sheet, clear of kick plate, stiffeners and identity insert.
                for i in range(26):
                    u=-.93+i*.071;z=.645+(i%4)*.020;span=.025+(i%5)*.008
                    ob=mesh(name+'_lower_contact_wear',[pos(x+u+du,.0696,z+dz) for du,dz in [(0,0),(span,.003),(span*.74,.013),(.005,.007)]],[(0,1,2,3)],'primer' if i%3 else 'steel');ob['surface_decal']=True
            pull_x=x-s*(w/4-.16)
    ...
    bx('_direction_panel',(0,-.23 if A.stage=='full' else -.178,h+.13 if A.stage=='full' else h+.16),(min(w,2.4),.040,.20 if A.stage=='full' else .30),'sign_coat' if external else 'teal')
    ...
    text_obj(name+'_type',label,pos(0,-.251 if A.stage=='full' else -.199,h+.065 if A.stage=='full' else h+.07),.115,'sign_letters' if external else 'ivory',(math.pi/2,0,heading))
    PAR=parent
...
EXTRA_CAMERAS=[
 ...
 ('D05_GATE_MECHANISM',(5.1,8.45,3.98),(6.01,8.85,3.87),35,'supplement: exposed gate motor, bracket and transmission housing; enclosed rollers verified geometrically'),
...
def shell(stage):
    ...
    # Cells describe functional volumes. The widened bypass overlaps the plant
    # volume; trim its physical slab so the common finish has no coplanar faces.
    for c in cells:
        ...
        if c['id']=='plant_header' and stage=='full':
            outline=[(x0,y0),(x1,y0),(x1,18.0),(-1.5,18.0),(-1.5,y1),(x0,y1)]
            center=Vector(((x0+x1)/2,(y0+y1)/2,-.12))
            vertices=[Vector((x,y,z))-center for z in [-.24,0] for x,y in outline]
            faces=[tuple(reversed(range(6))),tuple(range(6,12))]+[(i,(i+1)%6,(i+1)%6+6,i+6) for i in range(6)]
            floor.data.clear_geometry();floor.data.from_pydata(vertices,[],faces);floor.data.update()
            floor['junction_trim']='Remove duplicate 0.30 x 0.60 m top face at bypass junction; union footprint unchanged.'
    ...
    if stage=='full':
        ...
        tube('Recess_air_branch',[(.91,14.8,2.67),(-1.06,14.8,2.67),(-1.06,14.8,2.16),(-1.29,14.8,2.14)],.025,'steel')
        rod('Service_branch_union',(-1.18,14.8,2.149565),(-1.25,14.8,2.143478),.046,'steel')
...
if A.stage=='full':
    ...
    # Crown terminates against the existing ceiling crossmember at X6.5.
    # Its infill bears against the beam underside, with no unsupported gap.
    root('Freight_gate_crown','wall',[(6.5,10,4.345)],(1,0,0))
    box('Gate_crown_beam',(6.3125,10,4.345),(.375,4.4,.11),'darksteel')
    box('Gate_crown_infill',(6.35,10.32,4.135),(.26,3.76,.31),'mineral')
...
if A.stage=='full':
    import runpy
    wayfinding_source=Path(__file__).resolve().with_name('wayfinding.py')
    if not wayfinding_source.exists():wayfinding_source=ROOT/'blender/wayfinding.py'
    runpy.run_path(str(wayfinding_source))['apply_wayfinding']()
    ...
    handoff['incident_hooks']=[marker(name,'incident',position,{'event':event,'target_assembly':target,'runtime_handler':'pending'}) for name,position,event,target in [('FC_HOOK_AIR_LEAK',(-1.29,14.8,2.14),'air_leak','Service_air_station'),('FC_HOOK_GATE_JAM',(6.35,10,3.7),'freight_gate_jam','FREIGHT_GATE'),('FC_HOOK_RESTRAINT',(2.65,12.32,.76),'carrier_restraint_fault','Long_cask_carrier')]]
    ...
    handoff['wayfinding']={'revision':S['wayfinding_revision'],'source_sha256':S['wayfinding_source_sha256'],'codes':{'F01':'REFINERY','F02':'REACTOR','S01':'PLANT','S02':'CLEAN','S03':'WASTE','FG01':'INTERNAL FREIGHT GATE'},'base_geometry_revision':A.revision}
