...
def trolley(x=2.65,y=12.32):
    ...
    for sy in [-1,1]:
        box('Chassis_channel_web',(x,y+sy*.40,.404),(1.54,.045,.125),'teal')
        for z in [.346,.462]:box('Chassis_folded_flange',(x,y+sy*.368,z),(1.54,.105,.020),'teal')
        box('Carrier_rubber_edge',(x,y+sy*.438,.402),(1.60,.024,.087),'rubber',.008)
    ...
    for side in [-1,1]:
        xx=x+side*.605
        rod('Current_cask_end_closure',(xx-.0175,y,z),(xx+.0175,y,z),.170,'steel',64)
        ...
        for i in range(8):
            a=i*math.tau/8
            rod('Closure_bolt',(xx+side*.011,y+math.cos(a)*.133,z+math.sin(a)*.133),(xx+side*.017,y+math.cos(a)*.133,z+math.sin(a)*.133),.008,'steel',6)
    for xx in [-.43,.43]:
...
def pipework():
    ...
    tube('Bay_air_header',[(1.30,12.94,3.24),(4.19,12.94,3.24),(4.26,12.94,3.17),(4.26,12.94,2.63)],.05,'steel')
    for xx in [1.75,3.75]:
...
def utility_station(name,origin,heading=0):
    ...
    tube(name+'_service_pipe',[(-.40,-.16,2.25),(-.40,-.16,1.88),(-.32,-.16,1.80),(.04,-.16,1.80),(.34,-.16,1.80),(.39,-.16,1.75),(.39,-.16,1.53)],.042,'steel')
    for xx in [-.36,.34]:
...
def task_light():
    wall_light('Cask_task_light',(2.78,13.2),0,2.68,140)
...
def wall_light(name,origin,heading=0,z=2.55,power=95):
    ...
    place_wall_group(par,origin,heading)
    c,s=math.cos(heading),math.sin(heading)
    area(name+'_pool',(origin[0]+s*.26,origin[1]-c*.26,z-.052),(origin[0]+s*.75,origin[1]-c*.75,.75),power,(1,.83,.61),.34,.16)
...
def detail_dressing(stage):
    ...
    wall_vent('Staging_vent',(2.72,13.2),0,3.63,1.10)
    ...
    if stage=='full':
        utility_station('Service_air_station',(-1.2,14.95),math.pi/2)
