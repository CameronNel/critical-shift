...
mat('glove',(.23,.17,.105),.97,variation=.04,bump=.0015)
...
rough=nn.new('ShaderNodeMapRange');rough.inputs['From Min'].default_value=.10;rough.inputs['From Max'].default_value=.36;rough.inputs['To Min'].default_value=.73;rough.inputs['To Max'].default_value=.86;ll.new(it.outputs['Color'],rough.inputs[0]);ll.new(rough.outputs[0],pb.inputs['Roughness'])
...
wc=wn.new('ShaderNodeNewGeometry');ws=wn.new('ShaderNodeVectorMath');ws.operation='SCALE';ws.inputs[3].default_value=.4;wl.new(wc.outputs['Position'],ws.inputs[0])
wi=wn.new('ShaderNodeTexImage');wi.image=wall_image;wi.projection='BOX';wi.projection_blend=.12;wl.new(ws.outputs[0],wi.inputs['Vector'])
...
def door(name,center,w,h,heading=0,closed=False):
    ...
    bx('_direction_panel',(0,-.178,h+.16),(min(w,2.4),.040,.30),'teal')
    ...
    text_obj(name+'_type',label,pos(0,-.199,h+.07),.115,'ivory',(math.pi/2,0,heading))
    PAR=parent
