...
def workbench():
    ...
    for side in [-1,1]:
        ...
        ellipsoid('Work_glove_palm',(gx,gy,.920),(.107,.122,.032),'glove')
        for i in range(4):
            xx=gx-.037+i*.024
            ellipsoid('Glove_finger',(xx,gy+.074-(i%3)*.005,.916),(.025,.089-(i%3)*.010,.023),'glove')
        ellipsoid('Glove_thumb',(gx+side*.054,gy+.006,.917),(.059,.054,.025),'glove')
        box('Glove_cuff',(gx,gy-.07,.916),(.096,.050,.020),'rubber',.009)
        ellipsoid('Glove_palm_reinforcement',(gx,gy-.004,.934),(.066,.066,.009),'cloth')
        for i in range(4):rod('Glove_seam_stitch',(gx-.025+i*.016,gy-.019,.938),(gx-.021+i*.016,gy-.019,.938),.0013,'glove',6)
...
def tool_board():
    ...
    for i in range(4):
        ...
        mesh('Open_jaw_spanner',vv,[tuple(reversed(range(nn))),tuple(range(nn,nn*2))]+[(j,(j+1)%nn,(j+1)%nn+nn,j+nn) for j in range(nn)],'steel')
