...
def apply_wayfinding():
    ...
    for name,origin,heading,code,title,subtitle,direction in specs:
        ...
        root['wayfinding_code']=code;root['wayfinding_revision']='final-pass'
    ...
    for ob in scene.objects:
        ...
        for prefix,(code,title) in codes.items():
            ...
            if ob.name==prefix+'_type':ob.data.body=code+' / '+title;ob.data.size=.18;ob.data.offset=.002
        ...
        if ob.name=='FREIGHT_GATE_type':ob.data.body='FG01 / FREIGHT';ob.data.size=.18;ob.data.offset=.002
        if ob.name=='REACTOR_BOUNDARY_type':
            ob.data.materials.clear();ob.data.materials.append(bpy.data.materials['header_ink']);ob.data.offset=.002
    ...
    scene['wayfinding_revision']='final-pass'
    scene['wayfinding_source_sha256']=hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
    bpy.context.view_layer.update()
    register_panel_supports()
    print(json.dumps({'wayfinding_revision':'final-pass','replaced_panels':len(specs),'objects':len(scene.objects)}))
...
def register_panel_supports():
    ...
    for panel in panels:
        ...
        for foot in sorted((o for o in panel.children if '_mount_foot' in o.name),key=lambda o:o.name):
            ...
            # Begin in front of the full 160mm upright, never inside its section.
            hit,_,index,distance=tree.ray_cast(center+normal*.30,-normal,.40)
