...
def shell(stage):
    ...
    if stage=='full':
        ...
        tube('Recess_air_branch',[(.91,14.68,2.67),(-1.06,14.68,2.67),(-1.06,14.68,2.16),(-1.49,14.68,2.14)],.025,'steel')
        rod('Service_branch_union',(-1.38,14.68,2.146),(-1.45,14.68,2.142),.046,'steel')
...
if A.stage=='full':
    ...
    handoff['incident_hooks']=[marker(name,'incident',position,{'event':event,'target_assembly':target,'runtime_handler':'pending'}) for name,position,event,target in [('FC_HOOK_AIR_LEAK',(-1.49,14.68,2.14),'air_leak','Service_air_station'),('FC_HOOK_GATE_JAM',(6.35,10,3.7),'freight_gate_jam','FREIGHT_GATE'),('FC_HOOK_RESTRAINT',(2.65,12.32,.76),'carrier_restraint_fault','Long_cask_carrier')]]
