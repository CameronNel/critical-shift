...
DIAGNOSTIC_CAMERAS=[
    ...
    ('D02_WORKBENCH',(-.20,8.75,1.65),(-1.80,10.58,1.26),40),
    ('D03_UTILITY',(3.80,11.0,1.72),(3.80,13.15,1.72),46),
...
CAP_SUFFIXES = ('_sliding_leaf', '_leaf_stiffener', '_folded_return', '_kick_plate', '_leaf_panel_seam', '_leaf_gusset', '_gusset_rivet', '_localized_kick_scuff', '_leaf_carriage_hanger', '_leaf_carriage_roller', '_leaf_carriage_axle', '_leaf_identity_field', '_leaf_identity_text', '_leaf_identity_fastener', '_leaf_orange_strip', '_leaf_access_panel', '_leaf_access_fastener', '_leaf_access_type', '_leaf_pull_back', '_leaf_pull_shadow', '_leaf_pull_rail', '_leaf_pull_end', '_leaf_serial_plate', '_leaf_serial_type', '_leaf_pressed_border', '_leaf_pressed_fastener', '_leaf_identity_mark', '_leaf_handling_wear')
...
def cap_member(obj):
    ...
    return any(obj.name.startswith(owner.name + suffix) for suffix in CAP_SUFFIXES)
...
def camera_near_surface(scene, obj):
    """Reject a camera's central view blocked immediately by physical geometry."""
    origin=obj.matrix_world.translation
    direction=obj.matrix_world.to_3x3()@Vector((0,0,-1))
    hit,location,normal,index,other,matrix=scene.ray_cast(bpy.context.evaluated_depsgraph_get(),origin,direction,distance=.12)
    return {'status':'FAIL' if hit else 'PASS','minimum_central_view_clearance_m':.12,
            'blocking_object':other.name if hit else None,'distance_m':(location-origin).length if hit else None,
            'scope':'Near-camera central ray only; composition and complete image visibility still require pixel review.'}


def camera_audit(scene, expected_samples):
    issues, results = [], []
    actual = sorted(o.name for o in scene.objects if o.type == 'CAMERA')
    expected = sorted(c[0] for c in CAMERAS+DIAGNOSTIC_CAMERAS)
    if actual != expected:
        issues.append({'camera_inventory': {'expected': expected, 'actual': actual}})
    for name, loc, target, lens in CAMERAS+DIAGNOSTIC_CAMERAS:
        ...
        obstruction=camera_near_surface(scene,obj)
        if obstruction['status']=='FAIL':errors['near_camera_obstruction']=obstruction
        results.append({'camera': name, 'status': 'FAIL' if errors else 'PASS', 'errors': errors,'near_surface_check':obstruction})
        if errors:
...
def stretcher_turn(surfaces, corner):
    findings, stats = [], {'rays': 0, 'blocked_rays': 0, 'blocking_objects': set()}
    center = Vector((corner[0], corner[1], 0))
...
def routes_audit(scene, geometries, interface):
    ...
    checks = [straight_route(surfaces, freight, 2.4, 2.2, 'freight_sampled_straight_clearance'),
              ...
              stretcher_turn(surfaces, bypass[1])]
...
def run_self_tests(output):
    ...
    data=bpy.data.cameras.new('Audit_camera'); camera=bpy.data.objects.new('Audit_camera',data);scene.collection.objects.link(camera)
    for height,expected in [(1.0,'PASS'),(.006,'FAIL')]:
        camera.location=(-2,0,height);bpy.context.view_layer.update()
        result=camera_near_surface(scene,camera)
        tests.append({'name':'clear_camera_passes' if height==1 else '6mm_camera_obstruction_rejected','expected':expected,'actual':result['status'],'pass':result['status']==expected})
...
def packed_material_audit(scene):
    rows=[]
    for key,filename in [('floor_albedo_sha256','T01-floor-albedo-r01.png'),('wall_albedo_sha256','T02-wall-albedo-r01.png'),('cloth_albedo_sha256','T03-cloth-albedo-r01.png')]:
        path=ROOT/'art/materials'/filename
        expected=hashlib.sha256(path.read_bytes()).hexdigest() if path.is_file() else None
        images=[im for im in bpy.data.images if im.name==filename]
        packed=hashlib.sha256(images[0].packed_file.data).hexdigest() if len(images)==1 and images[0].packed_file else None
        recorded=scene.get(key)
        rows.append({'file':filename,'source_sha256':expected,'packed_sha256':packed,'saved_sha256':recorded,'status':'PASS' if expected and expected==packed==recorded else 'FAIL'})
    return {'status':'PASS' if all(r['status']=='PASS' for r in rows) else 'FAIL','assets':rows,'scope':'Exact packed generated albedo bytes and saved/source identity; not a claim of pixel quality or seamless tiling'}
...
def main():
    ...
    report = {
        ...
        'packed_materials': packed_material_audit(scene),
    }
    ...
    body_parts=[g for n,g in geometries.items() if n=='Current_reactor_cartridge' or n.startswith('Current_reactor_cartridge.')]
    closure_parts=[g for n,g in geometries.items() if n=='Current_cask_end_closure' or n.startswith('Current_cask_end_closure.')]
    if len(core)==3 and len(body_parts)==1 and len(closure_parts)==2:
    ...
    gates = ['camera_and_render_settings', 'geometry', 'dependencies', 'packed_materials', 'support_contact', 'internal_routes','current_cartridge_core']
