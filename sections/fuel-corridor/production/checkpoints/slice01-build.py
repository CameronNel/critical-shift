...
"""
import argparse, bpy, math, json, sys, random, hashlib
...
def mat(name,color,rough=.6,metal=0,variation=.05,bump=.02,emission=0):
    ...
    M[name]=m;return m
mat('mineral',(0.48,.46,.40),.87,variation=.055,bump=.018)
mat('floor',(.26,.28,.275),.83,variation=.06,bump=.015)
mat('repair',(.30,.31,.29),.88,variation=.035,bump=.012)
mat('teal',(.055,.16,.17),.54,.28,.06,.007)
mat('teal_light',(.115,.245,.24),.62,.18,.055,.005)
mat('steel',(.26,.29,.285),.42,.78,.08,.003)
mat('darksteel',(.07,.087,.088),.63,.6,.055,.004)
mat('rubber',(.025,.03,.029),.93,.0,.04,.008)
mat('ochre',(.67,.39,.08),.66,.18,.045,.005)
mat('ivory',(.70,.69,.59),.68,.1,.025,.003)
mat('ink',(.075,.083,.075),.86,variation=0,bump=0)
mat('paper',(.67,.63,.48),.94,variation=.04,bump=.003)
mat('cloth',(.22,.25,.20),.98,variation=.07,bump=.018)
...
def rod(name,a,b,r,material='steel',verts=32):
    ...
    be=o.modifiers.new('Machined rim','BEVEL');be.width=min(.004,r*.1);be.segments=2
...
def wall(name,a,b,h=4.4):
    ...
    slab('_washable_lower',-.014,.64,.028,1.28,'teal_light')
...
def pipework():
    root('Slice_utility_header','wall',[(-2.2,12.85,3.22)],(-1,0,0))
    # Intentional single pressure-air service with flanges and isolator, entering a wall sleeve.
    tube('Air_header',[(-2.26,12.85,3.22),(-1.5,12.85,3.22),(-.9,12.85,3.22),(3.3,12.85,3.22),(3.55,12.6,3.22),(3.55,11.9,3.22)],.067,'teal')
    for x in [-1.35,2.8]:
    ...
    rod('Valve_stem',(.3,12.83,3.22),(.3,12.61,3.22),.02)
    ...
    # One descending conduit terminates at an enclosure with real wall brackets.
    tube('Control_conduit',[(3.55,11.9,3.22),(3.55,11.4,3.22),(3.7,11.25,3.1),(3.7,11.25,1.3)],.022,'darksteel')

def trolley(x=.2,y=12.32):
    ...
    # Saddle cradles are contour-cut plate profiles, not stacked cubes.
    for sx in [-1,1]:
        xx=x+sx*.61
        outline=[(y-.37,.46),(y-.37,.73),(y-.29,.77),(y-.21,.61),(y+.21,.61),(y+.29,.77),(y+.37,.73),(y+.37,.46)]
        ...
        for yy in [-.32,.32]:bolt('Saddle_fastener',(xx-.055,y+yy,.53),(-1,0,0))
    z=.88
    rod('Shielded_cartridge',(x-.90,y,z),(x+.90,y,z),.23,'ivory',64)
    for xx in [-.93,.93]:rod('Cartridge_end_flange',(x+xx-.022,y,z),(x+xx+.022,y,z),.26,'steel',48)
    for xx in [-.965,.965]:rod('Cartridge_end_cap',(x+xx-.01,y,z),(x+xx+.01,y,z),.22,'teal')
    for xx in [-.63,.63]:
        rod('Retaining_band',(x+xx-.034,y,z),(x+xx+.034,y,z),.245,'teal',48)
        box('Overcentre_latch',(x+xx,y-.262,z),(.11,.038,.17),'steel',.008)
        rod('Latch_handle',(x+xx,y-.288,z-.04),(x+xx,y-.288,z+.09),.012,'ochre')
    ...
    for sy in [-1,1]:
        tube('Push_handle',[(x-1.05,y+sy*.33,.43),(x-1.05,y+sy*.33,1.17),(x-1.03,y+sy*.29,1.21)],.025,'teal')
    rod('Grip',(x-1.03,y-.29,1.21),(x-1.03,y+.29,1.21),.031,'rubber')
    # Load card, wedge, rag and handled flask: four purposeful human details.
    box('Docket_board',(x-.33,y-.382,.48),(.36,.23,.016),'darksteel')
    box('Docket',(x-.33,y-.382,.491),(.29,.18,.004),'paper',0)
    text_obj('Docket_type','LOT 07 / RECEIVED',(x-.33,y-.399,.494),.022,'ink',(0,0,0))
    for i in range(4):box('Docket_rule',(x-.34,y-.375+i*.025,.494),(.22,.003,.001),'ink',0)
    # Flattened broad folds on the folded wiping rag.
    verts=[];faces=[]
    for j in range(7):
        for i in range(10):verts.append((x+.48+i*.031,y-.42+j*.031,.488+.009*math.sin(i*1.1+j*.38)))
    for j in range(6):
    ...
    box('Wrench_shank',(x+.06,y-.40,.50),(.23,.028,.017),'steel')
    torus('Wrench_ring',(x-.072,y-.40,.50),.030,.01,'steel')
...
def wall_station():
    root('Dispatch_paperwork_station','wall',[(3.1,13.2,1.52)],(0,1,0))
    ...
    box('Key_tag',(3.29,13.031,1.705),(.036,.008,.064),'ochre')
    for xx in [2.82,3.38]:
        for zz in [1.20,1.90]:bolt('Station_fixing',(xx,13.151,zz))
    root('Vacuum_flask_shelf','wall',[(2.28,13.2,1.02)],(0,1,0))
...
CAMERAS=[
 ...
 ('C03_HERO',(-1.55,8.10,1.70),(1.45,12.2,1.67),28,'cask staging / style slice'),
 ...
 ('C09_MATERIALS',(1.6,10.6,1.52),(.30,12.3,.85),46,'trolley material and support detail'),
...
def shell(stage):
    ...
    if stage=='slice':cells=[{'id':'style_bay','bounds':[-2.2,4.4,7,13.2],'height':4.4}]
    ...
    for (side,line,h),segs in edges.items():
        ...
        for k,(aa,bb) in enumerate(merged):
            ...
            if stage=='slice' and side=='E':cuts=[(8.3,11.7,3.4)]
            intervals=[(aa,bb)]
    ...
    for yy in [7.25,12.95]:
        ...
        for xx in [-2.13,4.33]:
            ...
            box('Bearing_plate',(xx,yy,3.64),(.12,.35,.5),'steel')
    pipework()
    if stage=='full':
        root('Longitudinal_tray')
        for yy in [8.0,8.28]:
...
if A.stage=='slice':door('FREIGHT_GATE',(4.4,10),3.4,3.4,-math.pi/2,True)
else:
...
collection('04_Freight_and_dressing');trolley();wall_station();floor_detail()
collection('05_Practical_lighting')
practical('Staging_warm_key',.15,11.78,4.4,430)
practical('Gate_cool_fill',3.32,8.45,4.4,190,True,math.pi/2)
if A.stage=='full':
...
else:area('Slice_low_fill',(-1.8,7.5,2.6),(.2,12,1),60,(.79,.86,1),2)
...
S.render.image_settings.file_format='PNG';S.render.image_settings.color_mode='RGB';S.view_settings.view_transform='AgX';S.view_settings.look='AgX - Medium High Contrast';S.view_settings.exposure=.45
