...
mat('teal',(.027,.076,.082),.59,.05,.075,.001)
mat('teal_light',(.07,.145,.145),.62,.05,.06,.001)
mat('steel',(.17,.19,.185),.35,.86,.045,.002)
mat('darksteel',(.07,.087,.088),.63,.6,.055,.004)
mat('rubber',(.025,.03,.029),.93,.0,.04,.008)
mat('ochre',(.67,.39,.08),.66,.18,.045,.005)
mat('ivory',(.40,.415,.36),.60,.05,.045,.0007)
...
mat('old_teal',(.058,.109,.113),.77,.02,.08,.009)
...
def trolley(x=.2,y=12.32):
    ...
    box('Carrier_tool_ledge',(x,y-.325,.478),(1.94,.28,.012),'teal')
    ...
    z=.88
    lathe_x('Cartridge_outer_sleeve',[(x-.90,.205),(x-.86,.23),(x-.16,.23),(x-.14,.226),(x+.14,.226),(x+.16,.23),(x+.86,.23),(x+.90,.205)],y,z,'ivory')
    ...
    for xx in [-.965,.965]:
        ...
        for a in [i*math.tau/8 for i in range(8)]:
            bolt('Cap_retention_bolt',(x+xx+( .013 if xx>0 else -.013),y+math.cos(a)*.196,z+math.sin(a)*.196),(1,0,0),.012)
        rod('Endplate_recess',(x+xx-.014,y,z),(x+xx+.014,y,z),.12,'darksteel')
        rod('Endplate_lock',(x+xx-.019,y,z),(x+xx+.019,y,z),.054,'steel',8)
    for xx in [-.63,.63]:
    ...
    for i in range(9):
        xx=x+random.uniform(-.78,.7);zz=z+random.uniform(-.09,.09)
        yy=y-math.sqrt(max(0,.2305**2-(zz-z)**2))
        mesh('Local_barrel_rub',[(xx,yy-.001,zz),(xx+.035,yy-.001,zz+.004),(xx+.07,yy-.001,zz+.011),(xx+.014,yy-.001,zz+.006)],[(0,1,2,3)],'repair')
    ...
    box('Docket_board',(x-.33,y-.342,.492),(.36,.23,.016),'darksteel')
    box('Docket',(x-.33,y-.342,.502),(.29,.18,.004),'paper',0)
    ...
    vv=[];ff=[]
    profile=[(-.070,.91),(-.062,1.05),(-.048,1.17),(-.030,1.227),(0,1.244),(.030,1.227),(.048,1.17),(.056,1.04),(.063,.95)]
    for i,(xx,zz) in enumerate(profile):
        for j in range(13):
            off=.009*math.sin(j*.9)*(abs(i-4)/4);vv.append((x-1.03+xx+off,y-.24+j*.02,zz+.004*math.sin(j*.7)*(abs(i-4)/4)))
    for i in range(len(profile)-1):
        for j in range(12):q=i*13+j;ff.append((q,q+1,q+14,q+13))
    ob=mesh('Draped_wiping_cloth',vv,ff,'cloth');so=ob.modifiers.new('Woven cloth thickness','SOLIDIFY');so.thickness=.002;so.offset=0
...
collection('04_Freight_and_dressing');trolley();wall_station();floor_detail();task_light()
collection('05_Practical_lighting')
practical('Staging_warm_key',.15,11.78,4.4,175)
practical('Gate_cool_fill',3.32,8.45,4.4,80,True,math.pi/2)
if A.stage=='full':
