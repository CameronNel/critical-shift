"""Independent frozen full01 branch audit. Read-only scene; CPU; no render/save."""
...
OUT=BASE/'astra-full01-branch-evidence.json'
...
for p in ports:
    ...
    section=[]
    for d in [-.75,-.63,-.56,-.50,-.37,-.28,-.15,-.08,0]:
        for z in [.20,.55,1.0,2.0,p['clear_height']-.10]:
    ...
    for g in near:
        lb=g['local'][p['id']]
        if lb[1][1]<=.00001 or lb[1][0]<=-p['clear_width']/2 or lb[0][0]>=p['clear_width']/2 or lb[1][2]<=.00001 or lb[0][2]>=p['clear_height']:continue
