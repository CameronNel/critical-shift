"""Independent frozen eng08 branch audit. Read-only scene; CPU; no render/save."""
...
SNAP=BASE.parent/'checkpoints/eng08'
OUT=BASE/'astra-eng08-branch-evidence.json'
contract=json.loads((SNAP/'interface.json').read_text())
previous_contract=json.loads((BASE.parent/'checkpoints/eng07/interface.json').read_text())
...
report['interface_semantic_changes_vs_eng07']=json_diff(previous_contract,contract)
report['unchanged_contract_fields_vs_eng07']={k:contract[k]==previous_contract[k] for k in ['ports','floor_cells','route_centerlines','handling','travel']}
