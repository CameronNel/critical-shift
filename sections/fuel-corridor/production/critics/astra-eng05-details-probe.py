"""Independent read-only evaluated wall fits and new mounting checks, eng05."""
from pathlib import Path
import runpy,json,hashlib,time
HERE=Path(__file__).resolve().parent
ctx=runpy.run_path(str(HERE/'astra-eng05-mesh-core.py'))
...
names=['Service_air_station','East_distribution','Bypass_isolation','Bypass_first_aid','Bypass_work_permit',
       ...
       'Clean_branch_blade_sign','Recess_task_light','East_wall_key']
...
report={'revision':'eng05','blend':str(SAVED),'blend_sha256':EXPECTED,'object_count':len(scene.objects),
...
(HERE/'astra-eng05-details-evidence.json').write_text(json.dumps(report,indent=2))
