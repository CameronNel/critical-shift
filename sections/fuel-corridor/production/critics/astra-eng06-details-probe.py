"""Independent read-only evaluated wall fits and new mounting checks, eng06."""
from pathlib import Path
import runpy,json,hashlib,time
HERE=Path(__file__).resolve().parent
ctx=runpy.run_path(str(HERE/'astra-eng06-mesh-core.py'))
...
report={'revision':'eng06','blend':str(SAVED),'blend_sha256':EXPECTED,'object_count':len(scene.objects),
...
(HERE/'astra-eng06-details-evidence.json').write_text(json.dumps(report,indent=2))
