...
SNAP=BASE.parent/'checkpoints/full01'
...
report={'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'blend_sha256':hashlib.sha256((SNAP/'Fuel_Corridor.blend').read_bytes()).hexdigest(),'scope':'Evaluated architecture shell mesh bounds in cold CPU process. Separate interface mating transforms only. No neighbor geometry or collision test. Nominal footprint means clear-width strip for the owned sill/stub length; it is not a full neighbor building envelope.','seams':[],'saved':False}
for pid,key,length in [('F01_REFINERY','F01_connector_to_refinery',1.1),('F02_REACTOR','F02_connector_to_reactor',3.7)]:
    ...
    result={'id':pid,'nominal_clear_width_height':[w,h],'neighbor_owned_length':length,'architecture_near_seam':[],'masonry_beyond_seam_in_nominal_plan_footprint':[],'other_shell_beyond_seam_in_nominal_plan_footprint':[]}
    for ob in bpy.data.collections['01_Architecture'].objects:
        if ob.type!='MESH':continue
        relevant=any(q in ob.name for q in ['_concrete','Port_upper_masonry','Height_step_masonry','Floor_','Ceiling_','Recessed_ceiling_panel'])
        ...
        try:
            ...
            if lb[1][0]<-w/2-.5 or lb[0][0]>w/2+.5 or lb[1][1]<-.5 or lb[0][1]>.5:continue
...
out=BASE/'astra-full01-branch-shell-evidence.json';out.write_text(json.dumps(report,indent=2),encoding='utf-8')
