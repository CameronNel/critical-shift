...
for o in s.objects:
 ...
 e.to_mesh_clear()
check('material_assignments',all(len(g['object'].data.materials)>0 for g in geometry.values()),[n for n,g in geometry.items() if not g['object'].data.materials])
...
expected={'shielded_cask':2,'residue_overpack':3,'dry_container':3,'cart':1,'handling_jib':1,'ventilation':1,'inventory':1,'repair_bench':1}
...
cams=[o for o in s.objects if o.type=='CAMERA'];check('fixed_cameras',len(cams)==14 and all(abs(o.location.z-1.68)<.001 for o in cams),[{'name':o.name,'location':list(o.location),'lens':o.data.lens} for o in cams])
roots=[]
for o in s.objects:
