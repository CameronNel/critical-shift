...
M={
 ...
 'cream':mat('Ivory enamel steel',(.44,.445,.38),.74,.25,.045,.004),
...
def architecture():
    ...
    for xx in [-3.82,3.82]:
        q=root('Receiving concrete panel reveal',target='Front_Wall_West' if xx<0 else 'Front_Wall_East',direction=(0,-1,0),anchors=[(xx,0,3.24)])
    ...
    q=root('Booth approach boot marks',anchors=[(-2.18,1.72,0)])
...
def cask_cart(cx,cy):
    ...
    for i in range(8):
        if i==6:continue # Registry plate replaces the front rib; text has its own unbroken field.
        a=2*pi*i/8;rr=.541;u=Vector((-sin(a),cos(a),0));v=Vector((cos(a),sin(a),0));c=Vector((cx,cy,1.14))
        outline=[(-.026,-.14),(.026,-.14),(.038,-.09),(.033,.11),(.019,.14),(-.019,.14),(-.033,.11),(-.038,-.09)]
        vv=[tuple(c+u*t+v*(rr-.004)+Vector((0,0,z))) for t,z in outline]
        vv += [tuple(c+v*(rr+.054)+Vector((0,0,-.105))),tuple(c+v*(rr+.046)+Vector((0,0,.11)))]
        ff=[tuple(reversed(range(8))),(0,1,8),(1,2,8),(2,3,9,8),(3,4,9),(4,5,9),(5,6,9),(6,7,8,9),(7,0,8)]
        mesh('Cast wedged heat spreader',vv,ff,'cast',q,.002)
...
def receiving_details():
    ...
    light('Receiving oblique inspection pool',(-1.93,.65,1.96),(.3,3,.60),90,.32,(1,.88,.69))
