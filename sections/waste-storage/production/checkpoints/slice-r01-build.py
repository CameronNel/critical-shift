...
"""
import bpy, math, json, argparse, sys, os
...
S['section']='waste-storage'; S['revision']=A.revision; S['build_stage']=A.stage; S['original_geometry']=True
COL={}
...
M={
 ...
 'plaster':mat('Painted mineral wall • warm cream',(.64,.62,.54),.83,noise=.016,bump=.002),
 'floor':mat('Sealed concrete • satin dry',(.32,.34,.31),.76,noise=.017,bump=.003),
 'teal':mat('Teal baked steel • maintained',(.065,.19,.205),.58,.35,.009,.001),
 'teallight':mat('Repainted teal panel',(.095,.24,.25),.68,.25,.006),
 'dark':mat('Graphite powder coated steel',(.045,.06,.06),.62,.45,.005),
 'steel':mat('Brushed zinc steel',(.36,.4,.4),.38,.82,.012),
 'cream':mat('Ivory enamel steel',(.61,.6,.51),.55,.25,.013),
 'orange':mat('Maintenance ochre enamel',(.71,.31,.07),.58,.25,.013),
 'rubber':mat('Carbon rubber • soft matte',(.019,.025,.026),.93,0,.003,.001),
 'plastic':mat('ABS instrument housing',(.10,.125,.12),.48,0,.004),
 'fabric':mat('Washed canvas',(.31,.34,.28),.96,0,.012,.002),
 ...
 'scuff':mat('Dry wheel abrasion',(.24,.26,.24),.87,0,.008)
}
M['glass']=mat('Wired safety glazing',(.26,.37,.36),.16,0,0); gb=M['glass'].node_tree.nodes.get('Principled BSDF'); gb.inputs['Transmission Weight'].default_value=.82; gb.inputs['IOR'].default_value=1.46
...
def box(name,co,dim,material,parent=None,bevel=.01,rot=None,solid=True):
    ...
    return finish(o,name,material,parent,bevel,solid)
...
def text(name,body,co,size,material,parent,rot=(pi/2,0,0),align='CENTER'):
    ...
    o=bpy.data.objects.new(name,cu); COL[ACTIVE].objects.link(o); o.location=co; o.rotation_euler=rot; return finish(o,name,material,parent,solid=False)
...
def architecture():
    ...
    for x in [-5.95,5.95]:
        box('Concrete wall plinth',(x,depth/2,.14),(.1,depth,.28),'concrete',bevel=.009)
    ...
    for x in [-1.57,1.57]:box('Door guide channel',(x,.095,1.6),(.12,.16,3.2),'dark',r,.006)
    ...
    box('Personnel frame header',(5.94,2.4,2.38),(.12,1.39,.17),'dark',r,.006)
    for y in [1.75,3.05]:box('Personnel frame jamb',(5.94,y,1.16),(.12,.12,2.32),'dark',r,.006)
...
def booth():
    ...
    for i,y in enumerate([.49,1.62,2.78,3.73]):box('Booth east stile %02d'%i,(-2.45,y,1.35),(.14,.07,2.7),'teal',r,.006)
    ...
    for z in [.28,1.99]:cyl('Booth hinge barrel',(-2.423,2.75,z),.025,.12,'steel',r)
    rod('Booth door lever',(-2.38,1.82,1.08),(-2.38,2.01,1.08),.013,'steel',r)
    box('Door lock escutcheon',(-2.447,1.82,1.075),(.025,.055,.18),'steel',r,.007)
    # Parent geometry to pivot while preserving source world transforms.
    d.location=(-2.495,2.75,0)
    for o in door_parts:
    ...
    box('Desk laminate worktop',(-4.30,1.3,.79),(2.35,.72,.06),'cream',q,.024)
    for x in [-5.34,-3.29]:
    ...
    light('Monitoring warm task pool',(-4.1,2.2,2.78),(-4.1,1.5,.8),125,1.1,(1,.84,.59),'RECTANGLE',.4)
    ...
    tube('Monitor supply conduit',[(-3.07,3.825,.68),(-3.24,3.83,.68),(-3.28,3.83,2.68),(-3.28,3.65,2.8)],.011,'steel',u)
...
def story_props(deskroot):
    ...
    rod('Resting pencil',(-3.78,1.15,.877),(-3.55,1.12,.877),.006,'orange',q)
...
def cask_cart(cx,cy):
    ...
    for xx in [cx-.53,cx+.53]:
        for yy in [cy-.58,cy+.58]:
            ...
            for x2 in [xx-.095,xx+.095]:box('Caster fork cheek',(x2,yy,.157),(.025,.21,.155),'teal',q,.018)
    ...
    for i in range(8):
        a=2*pi*i/8;rr=.546
        box('Cast vertical cooling rib',(cx+rr*cos(a),cy+rr*sin(a),1.13),(.065,.038,.24),'cream',q,.012,rot=(0,0,a+pi/2))
    ...
    for i in range(4):
        ...
        box('Cask latch bracket',(xx,yy,1.345),(.11,.045,.18),'teal',q,.012,rot=(0,0,a-pi/2))
        rod('Cask toggle clamp',(cx+.58*cos(a),cy+.58*sin(a),1.30),(cx+.59*cos(a),cy+.59*sin(a),1.465),.017,'orange',q)
    ...
    for xx in [cx-.39,cx+.29]:box('Cask bumper contact wear',(xx,cy-.542,.652),(.14,.008,.015),'steel',q,.005)
...
def receiving_details():
    ...
    ceiling_fixture('Receiving practical',.10,2.55,power=600,length=1.7)
    ceiling_fixture('Booth exterior practical',-3.3,3.99,power=330,length=1.25)
    light('Entry soft bounced fill',(1.3,-.6,2.3),(0,2,1),175,3,(.68,.78,1))

CAMERAS=[
 ('C01_ENTRY',(0,.28,1.65),(0,8,1.45),24),
 ('C02_HERO',(3.27,4.27,1.65),(-1.95,2.02,1.32),30),
...
def setup():
    ...
    S.world=bpy.data.worlds.new('Muted industrial ambient');S.world.use_nodes=True;S.world.node_tree.nodes['Background'].inputs[0].default_value=(.27,.34,.37,1);S.world.node_tree.nodes['Background'].inputs[1].default_value=.22
...
manifest={'stage':A.stage,'revision':A.revision,'camera_manifest':json.loads(S['camera_manifest']),'assemblies':[{'name':o.name,'target':o.get('support_target'),'direction':json.loads(o['support_direction']),'anchors':json.loads(o['support_anchors'])} for o in bpy.data.objects if o.get('support_required')]}
