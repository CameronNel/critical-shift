"""Original Waste Storage. Primitive/material utilities adapted from our Electrical authoring source; no geometry imported. Blender 5.2; factory-empty, no external assets.
...
SC['section']='waste-storage'
SC['revision']=args.revision
SC['stage']=args.stage
SC['source_sha256']=hashlib.sha256(SOURCE_BYTES).hexdigest()
COL={}
for name in ['Architecture','Casks','Containers','Cart','Monitoring','Ventilation','Workshop','Lighting','Cameras','Validation']:
...
exec(compile((Path(__file__).parent/'full_hall.py').read_text(),str(Path(__file__).parent/'full_hall.py'),'exec'))
