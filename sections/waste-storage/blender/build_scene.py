...
"""
import bpy, bmesh, math, json, argparse, sys, os, hashlib
...
M={
 ...
 'plaster':mat('Painted mineral wall • warm cream',(.50,.47,.40),.91,noise=.035,bump=.005),
 ...
 'cream':mat('Ivory enamel steel',(.44,.445,.38),.65,.30,.035,.0012),
 'cast':mat('Cast heat spreader metal',(.26,.285,.255),.81,.52,.023,.006),
 ...
 'chip':mat('Oxidised exposed primer',(.31,.325,.29),.78,.35,.011),
 'deck':mat('Dull galvanized load deck',(.29,.33,.32),.59,.74,.018,.001),
...
def cyl(name,co,r,depth,material,parent=None,axis='Z',vertices=48,bevel=.004):
    ...
    return finish(o,name,material,parent,min(bevel,r*.2,depth*.2))
...
def mesh(name,verts,faces,material,parent,bevel=0):
    ...
    o=bpy.data.objects.new(name,me); COL[ACTIVE].objects.link(o); return finish(o,name,material,parent,bevel)

def text(name,body,co,size,material,parent,rot=(pi/2,0,0),align='CENTER'):
    cu=bpy.data.curves.new(name,'FONT'); cu.body=body; cu.size=size; cu.align_x=align; cu.align_y='CENTER'; cu.extrude=.00005; cu.resolution_u=8
...
def cylinder_wear(name,cx,cy,r,a,z,w,h,parent,material='chip'):
    pts=[(-.5,-.24),(-.39,-.49),(-.12,-.34),(.15,-.5),(.5,-.20),(.38,.11),(.15,.49),(-.16,.31),(-.48,.43)]
    vv=[(cx+r*cos(a+x*w/r),cy+r*sin(a+x*w/r),z+y*h) for x,y in pts]
    o=mesh(name,vv,[tuple(range(len(vv)))],material,parent);md=o.modifiers.new('Thin exposed primer','SOLIDIFY');md.thickness=.0005;o['collision_role']='noncolliding'
    return o
...
def gauge(x,y,z,parent):
    ...
    cyl('Dose meter knob',(x+.082,y+.109,.904),.020,.030,'rubber',q,'Y')
...
def wheel_sweep(cx,cy,radius,a0,a1,width):
    q=root('Receiving wheel turn abrasion',anchors=[(cx+radius*cos(a0),cy+radius*sin(a0),0)])
    vv=[];N=34
    for i in range(N):
        t=i/(N-1);a=a0+(a1-a0)*t;w=width*(.13+.87*sin(pi*t))
        for rr in [radius-w/2,radius+w/2]:vv.append((cx+rr*cos(a),cy+rr*sin(a),.001))
    o=mesh('Curved rubber transfer on concrete',vv,[(2*i,2*i+1,2*i+3,2*i+2) for i in range(N-1)],'scuff',q)
    md=o.modifiers.new('Embedded abrasion film','SOLIDIFY');md.thickness=.001;o['collision_role']='route_marking'
...
def architecture():
    ...
    # Poured construction joints and a restrained serviced-wall patch establish material scale.
    q=root('Receiving wall repair',target='Front_Wall_East',direction=(0,-1,0),anchors=[(3.20,0,1.12)])
    box('Repaired mineral wall patch',(3.20,.001,1.12),(1.13,.002,.46),'concrete',q,.0003)
    for xx in [2.665,3.735]:rod('Repair feathered edge',(xx,.003,.90),(xx,.003,1.34),.002,'plaster',q)
    for xx in [-4.2,4.2]:
        q=root('Receiving wall joint',target='Front_Wall_West' if xx<0 else 'Front_Wall_East',direction=(0,-1,0),anchors=[(xx,0,2.1)])
        box('Architectural expansion joint',(xx,.002,2.1),(.009,.004,3.85),'concrete',q,.001)
    for xx in [-3.82,3.82]:
        q=root('Receiving concrete panel reveal',target='Front_Wall_West' if xx<0 else 'Front_Wall_East',direction=(0,-1,0),anchors=[(xx,0,3.24),(xx-.90,0,3.46),(xx+.90,0,3.46)])
        box('Horizontal mineral panel joint',(xx,.002,3.24),(4.6,.004,.012),'concrete',q,.001)
        for x2 in [xx-.90,xx+.90]:
            cyl('Recessed panel tie plug',(x2,.004,3.46),.024,.008,'concrete',q,'Y',32,.001)
    q=root('Booth approach boot marks',anchors=[(-2.18,1.72,0),(-2.03,1.91,0),(-2.09,2.29,0)])
    for x,y,w,h in [(-2.18,1.72,.08,.17),(-2.03,1.91,.06,.22),(-2.09,2.29,.08,.19)]:chip('Sparse rubbed floor patch',(x,y,.001),(1,0,0),(0,1,0),w,h,q,'scuff')
...
def booth():
    ...
    door_parts.append(chip('Door boot contact scuff',(-2.466,2.17,.16),(0,1,0),(0,0,1),.28,.03,r,'chip'))
    door_parts.append(chip('Door handled paint nick',(-2.466,1.83,.95),(0,1,0),(0,0,1),.07,.018,r,'steel'))
...
def story_props(deskroot):
    ...
    o=mesh('Folded woven wiping cloth',vv,ff,'fabric',q)
    for p in o.data.polygons:p.use_smooth=True
    md=o.modifiers.new('Cloth edge thickness','SOLIDIFY');md.thickness=.006
...
def cask_cart(cx,cy):
    ...
    q['function']='transfer'
    for xx in [cx-.45,cx+.45]:box('Cart channel rail',(xx,cy,.30),(.13,1.54,.13),'teallight',q,.006)
    for yy in [cy-.64,cy,cy+.64]:box('Cart cross bearer',(cx,yy,.30),(1.21,.11,.13),'teallight',q,.005)
    box('Cart inset deck',(cx,cy,.389),(1.15,1.35,.045),'deck',q,.007)
    for xx in [cx-.53,cx+.53]:
        for yy in [cy-.58,cy+.58]:
            ...
            for side in [-1,1]:
                cyl('Recessed wheel hub',(xx+side*.086,yy,.112),.063,.012,'steel',q,'X')
                cyl('Axle bolt',(xx+side*.099,yy,.112),.02,.018,'dark',q,'X',6,.002)
    # Cradle ring uses shaped pads and stops instead of a flat box plinth.
    lathe('Cask cradle seat',cx,cy,[(.50,.413),(.64,.43),(.66,.50),(.61,.545),(.51,.545)],'teallight',q)
    for xx in [cx-.49,cx+.49]:
        box('Cradle rubber saddle',(xx,cy,.55),(.12,.71,.07),'rubber',q,.018)
    lathe('Cask lower impact ring',cx,cy,[(.47,.55),(.56,.58),(.59,.63),(.59,.70),(.54,.73)],'cast',q)
    ...
    # Four folded protection stays transfer handling impacts between the two structural rings.
    # Open channel sections, flared end shoes and radial fasteners replace applied decorative ribs.
    for i in range(4):
        a=pi/8+i*pi/2;u=Vector((-sin(a),cos(a),0));v=Vector((cos(a),sin(a),0));c=Vector((cx,cy,0))
        cs=[(-.037,.545),(.037,.545),(.037,.611),(.024,.611),(.024,.562),(-.024,.562),(-.024,.611),(-.037,.611)]
        vv=[tuple(c+u*t+v*rr+Vector((0,0,z))) for z in [.715,1.395] for t,rr in cs];n=8
        ff=[tuple(reversed(range(n))),tuple(range(n,2*n))]+[(j,(j+1)%n,(j+1)%n+n,j+n) for j in range(n)]
        mesh('Folded cask impact stay',vv,ff,'teal',q,.0016)
        for z in [.73,1.385]:
            box('Cask stay flared shoe',(cx+.572*cos(a),cy+.572*sin(a),z),(.123,.080,.054),'cast',q,.004,rot=(0,0,a+pi/2))
            rod('Cask stay radial fastener',c+v*.606+Vector((0,0,z)),c+v*.632+Vector((0,0,z)),.018,'steel',q)
    ...
    lid=bpy.data.objects.new('Cask_Lid_LIFT_PIVOT',None); COL[ACTIVE].objects.link(lid);lid.parent=q;lid['support_role']='assembly_part';lid['support_root']=q.name;lid['interaction']='unseal_lift';lid['axis']='Z';lid.location=(cx,cy,1.48);bpy.context.view_layer.update()
    prior=set(bpy.data.objects)
    lathe('Shielded dished lid',cx,cy,[(.555,1.477),(.555,1.51),(.50,1.56),(.37,1.60),(.12,1.615)],'teal',q)
    for i in range(12):
    ...
    for o in set(bpy.data.objects)-prior:
        w=o.matrix_world.copy();o.parent=lid;o.matrix_world=w
    ...
    for a,z,w,h in [(-.68,1.195,.11,.013),(-.85,1.18,.041,.024),(-.91,1.198,.03,.008),(-1.65,1.10,.048,.022),(-.30,1.09,.022,.048)]:
        cylinder_wear('Cask localized handling abrasion',cx,cy,.5459,a,z,w,h,q)
    for a,z,w in [(-.82,.924,.14),(-1.8,.959,.08),(-.98,.923,.04)]:cylinder_wear('Restraint belt paint chips',cx,cy,.5606,a,z,w,.009,q,'steel')
...
def receiving_details():
    ...
    wheel_sweep(.43,2.3,.72,-2.1,-.6,.026)
    wheel_sweep(.43,2.3,.92,-2.0,-.9,.018)
    # A supported inspection lamp throws an oblique pool onto the cart and wheel contact zone.
    q=root('Receiving inspection task lamp',target='Booth east stile 00',direction=(-1,0,0),anchors=[(-2.38,.49,2.18)])
    box('Task lamp mounting pad',(-2.355,.49,2.18),(.05,.105,.21),'dark',q,.006)
    tube('Task lamp bent steel arm',[(-2.33,.49,2.18),(-2.10,.49,2.18),(-1.98,.59,2.08)],.021,'dark',q)
    sphere('Task lamp cast head',(-1.98,.59,2.045),(.12,.10,.09),'dark',q)
    cyl('Task lamp inset opal lens',(-1.98,.59,1.971),.084,.018,'light',q)
    light('Receiving oblique inspection pool',(-1.93,.65,1.96),(.3,3,.60),40,.45,(1,.88,.69))
    q=root('Receiving angled wall luminaire',target='Front_Wall_East',direction=(0,-1,0),anchors=[(3.2,0,2.63)])
    box('Wall luminaire steel mounting shoe',(3.2,.043,2.63),(.19,.086,.25),'dark',q,.008)
    box('Angled luminaire cast reflector',(3.2,.17,2.68),(.46,.25,.15),'teal',q,.012,rot=(-.35,0,0))
    box('Wall luminaire opal face',(3.2,.248,2.599),(.39,.095,.025),'light',q,.005,rot=(-.35,0,0))
    light('Receiving wall practical grazing pool',(3.2,.29,2.58),(.30,2.5,.45),155,.55,(1,.84,.62))
...
CAMERAS=[
 ...
 ('C07_MONITORING',(-3.00,2.53,1.65),(-4.32,1.27,1.04),28),
