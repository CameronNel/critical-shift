...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--review',default='pass-01');ap.add_argument('--validate-only',action='store_true');ap.add_argument('--finish-only',action='store_true');ap.add_argument('--no-bake',action='store_true');ap.add_argument('--cameras',default='REFERENCE_HALL,REFERENCE_BRIEFING,REFERENCE_LOCKER');a=ap.parse_args(sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [])
    ...
    if not a.validate_only:
        if not a.finish_only:materials();lockers();bathroom_floor();modern_door();briefing_dressing();hallway_dressing();locker_dressing();lighting();cameras()
        finishing_pass();rev.startup()
