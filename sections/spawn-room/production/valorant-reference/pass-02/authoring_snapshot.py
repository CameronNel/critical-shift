    ...
    if kind=='snake':
        ...
        for j in range(6):
            ...
            for i in range(6):
                ...
                leaf(name+'_broad_leaf',end,(cos(theta),sin(theta),.40+.50*(i/5)),.43+(i%2)*.12,.105+(i%3)*.018,(i+j)%3)
...
def briefing_dressing():
    ...
    root=k.group('V_BRIEF_wastebasket',b,(-6.63,1.38,.022));floor_contact(root,'BRIEFING_wood_floor')
...
def lighting():
    ...
    for ob in bpy.data.objects:
        ...
        if ob.name.startswith('EEVEE_floor_bounce'):
            ob.data.energy={'BRIEFING':35,'HALL':25,'LOCKER':54,'SERVICE':15}[ob.name.split('_')[-1]];ob.data.color=(.70,.77,.86)
...
def cameras():
    ...
    for name,eye,target,lens in [('HALL',(.55,8.65,1.60),(0,1.3,1.42),22),('BRIEFING',(-2.65,1.34,1.58),(-5.25,4.4,1.3),22),('LOCKER',(7.55,1.15,1.60),(3.65,4.6,1.3),21)]:
        cam=room.cam('REFERENCE_'+name,eye,target,lens);cam.data.sensor_width=46 if name=='HALL' else 52
