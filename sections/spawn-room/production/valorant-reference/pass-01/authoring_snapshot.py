...
def materials():
    ...
    for name in ['wall','dado']:
        ...
        for n in m.node_tree.nodes:
            if n.type=='BUMP':n.inputs['Distance'].default_value*=.62
    for name,c,rough,metal in [
        ('V_locker_steel',(.12,.18,.17),.80,.35),('V_graphite',(.073,.084,.082),.76,.5),
        ...
        ('V_stem',(.16,.17,.06),.9,0),('V_paper',(.57,.51,.38),.95,0),
        ('V_cardboard',(.25,.19,.108),.94,0),('V_rustred',(.34,.065,.029),.73,.2),
...
def modern_door():
    ...
    root=k.group('V_REAR_staff_door',b,(0,-.46,0),pi);root['asset_role']='modern_inaccessible_staff_door'
...
def leaf(name,base,direction,length,width,mi=0):
    ...
    for i in range(9):
        t=i/8;center=Vector(base)+axis*(t*length)+Vector((0,0,-.14*length*t*t))
        w=width*(sin(pi*t)**.75)
        for s in [-1,0,1]:v.append(tuple(center+side*s*w+normal*(.07*length*sin(pi*t)*(1-abs(s)))))
    f=[]
    for i in range(8):
    ...
    for p in ob.data.polygons:p.material_index=1 if p.index%2 else 0
    rib=[v[i*3+1] for i in range(1,8)]
...
def plant(name,p,size=1,kind='ficus',pot='V_potstone',target='FACILITY_floor'):
    ...
    k.lathe(name+'_pot',[(0,0),(.167,0),(.176,.025),(.220,.435),(.222,.468),(.217,.478),(.201,.478),(.199,.445),(.17,.055),(0,.055)],pot,48)
    k.lathe(name+'_base_band',[(.172,.035),(.186,.035),(.197,.145),(.187,.145),(.172,.035)],'V_ochre',48)
    ...
    if kind=='snake':
        ...
        for j in range(4):
            a=j*2.399;h=1.15+(j%3)*.26
            pts=[(0,0,.43),(.05*cos(a),.05*sin(a),.85),(.12*cos(a),.12*sin(a),h+.20)]
            k.tube(name+'_branch',pts,.011 if j else .017,'V_stem')
            for i in range(6):
                z=.68+i*(h-.58)/5;theta=a+i*2.3
                start=(.10*cos(a),.10*sin(a),z)
                end=Vector(start)+Vector((.12*cos(theta),.12*sin(theta),.06))
                k.rod(name+'_petiole',start,end,.004,'V_stem',10)
                leaf(name+'_broad_leaf',end,(cos(theta),sin(theta),.40+.50*(i/5)),.38+(i%2)*.10,.095+(i%3)*.015,(i+j)%3)
...
def briefing_dressing():
    plant('V_BRIEF_corner_ficus',(-2.48,5.70,.022),1.05,target='BRIEFING_wood_floor')
    # The original sideboard stays in place. Existing mug/clipboard remain useful.
    plant('V_BRIEF_table_pothos',(-6.17,1.19,.798),.32,'trailing','V_terracotta','BRIEFING_sideboard_laminate_top')
    ...
    for i in range(3):
        ...
        k.label('V_BRIEF_manual_title',['OPERATIONS','PEOPLE','A SAFER TOMORROW'][i],(-.097,-.161,.009),.018,'paper')
    ...
    root=k.group('V_BRIEF_wastebasket',b,(-5.40,1.14,.022));floor_contact(root,'BRIEFING_wood_floor')
...
def hallway_dressing():
    ...
    plantroot=plant('V_HALL_trailing_plant',(0,-.15,.017),.36,'trailing','V_terracotta',deck);plantroot.parent=shelf
    ...
    root=k.group('V_HALL_culture_poster',b,(1.7,7.32,1.96),-pi/2);life.supported(root,'HALL_right_ops',direction='LOCAL_+Y',kind='WALL')
    firstaid();trolley()
...
def firstaid():
    ...
    root=k.group('V_HALL_first_aid',b,(-1.7,6.87,1.83),pi/2);life.supported(root,'HALL_left_ops',direction='LOCAL_+Y',kind='WALL')
    ...
    root=k.group('V_HALL_extinguisher',b,(-1.692,6.84,.85),pi/2);life.supported(root,'HALL_left_ops_washable_dado',[(0,0,.15)],'LOCAL_+Y','WALL')
    wall_notice('V_EXT_location',(-1.7,6.84,1.35),pi/2,'HALL_left_ops',.16,.19,'FIRE','V_rustred')
...
def locker_dressing():
    ...
    p=plant('V_LOCKER_shelf_pothos',(-.20,-.17,.017),.33,'trailing','V_ochre',deck);p.parent=shelf
    for i in range(2):
    ...
    bag=life.bag('V_LOCKER_bench_bag',(3.62,5.15,.451),'canvas_rust',.12)
    floor_contact(bag,'LOCKER_changing_bench_02_laminate_slat.001',[(0,0,.005)])
...
def lighting():
    ...
    for ob in bpy.data.objects:
        ...
        for pref,en in energies.items():
            if ob.name.startswith(pref):ob.data.energy=en;ob.data.color=(1,.80,.57)
        if ob.name.startswith('EEVEE_floor_bounce'):
            ob.data.energy={'BRIEFING':72,'HALL':54,'LOCKER':100,'SERVICE':15}[ob.name.split('_')[-1]];ob.data.color=(.78,.83,.86)
    ...
    for name,eye,target,power,size in [
        ...
        d=bpy.data.lights.new('V_LIGHT_'+name,'AREA');d.energy=power;d.shape='DISK';d.size=size;d.color=(1,.81,.59)
...
def cameras():
    ...
    for name,eye,target,lens in [('HALL',(1.20,8.65,1.68),(-.15,1.3,1.42),22),('BRIEFING',(-2.65,1.34,1.68),(-5.25,4.4,1.3),22),('LOCKER',(7.55,1.15,1.68),(3.65,4.6,1.3),21)]:
        cam=room.cam('REFERENCE_'+name,eye,target,lens);cam.data.sensor_width=36

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--review',default='pass-01');ap.add_argument('--validate-only',action='store_true');ap.add_argument('--no-bake',action='store_true');ap.add_argument('--cameras',default='REFERENCE_HALL,REFERENCE_BRIEFING,REFERENCE_LOCKER');a=ap.parse_args(sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [])
    ...
    if not a.validate_only:
        materials();lockers();bathroom_floor();modern_door();briefing_dressing();hallway_dressing();locker_dressing();lighting();cameras();rev.startup()
        ...
        if not a.no_bake:print('BAKING_REFERENCE_LIGHTING',flush=True);bpy.ops.object.lightprobe_cache_bake(subset='ALL')
