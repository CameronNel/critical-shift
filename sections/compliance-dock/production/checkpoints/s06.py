...
def material(name,col,rough,metal=0,variation=.05,bump=.025):
    ...
    bs.inputs['Specular IOR Level'].default_value=0 if name in ['fabric','blue_fabric','paper','rubber','plaster','concrete'] else (.28 if metal<.2 else .4)
    ...
    if bump:
        ...
        if name in ['plaster','concrete','floor']:
            ...
            l.new(tc.outputs['Object'],broad.inputs['Vector']);b2=n.new('ShaderNodeBump');b2.inputs['Strength'].default_value=.15;b2.inputs['Distance'].default_value=.003
            l.new(broad.outputs['Fac'],b2.inputs['Height']);l.new(b.outputs[0],b2.inputs['Normal']);l.new(b2.outputs[0],bs.inputs['Normal'])
    if name=='steel' and 'Anisotropic IOR Level' in bs.inputs:bs.inputs['Anisotropic IOR Level'].default_value=.35
    MATERIALS[name]=m;return m

def palette():
    for args in [
        ('plaster',(.59,.535,.43),.88,0,.055,.002),('concrete',(.29,.30,.285),.85,0,.10,.0025),
        ('floor',(.32,.32,.29),.79,0,.08,.004),('teal',(.055,.15,.157),.48,.42,.085,.0008),
        ('teal_light',(.14,.27,.27),.53,.36,.075,.00065),('frame',(.065,.09,.093),.44,.72,.05,.0004),
        ...
        ('fabric',(.30,.29,.22),.95,0,.12,.002),('blue_fabric',(.075,.11,.12),.97,0,.09,.002),
...
def desk_slice():
    ...
    for i in range(7):box('Intercom speaker grille bar',(-3.14,4.771,.803+i*.009),(.184,.002,.004),'steel',.0004)
    for x in [-3.25,-3.05]:cyl('Intercom push key',(x,4.80,.863),.021,.014,'orange',w=.002)
    # Molded receiver: continuous curved handle with broad earpieces and parting line.
    vs=[];fs=[];nx=34;nr=20
    for i in range(nx+1):
        t=i/nx;xx=-3.285+.29*t
        zz=.911+.058*sin(pi*t)**.7;rr=.020+.017*(abs(2*t-1)**3)
        for j in range(nr):
            ang=2*pi*j/nr;vs.append((xx,5.0+rr*cos(ang),zz+.68*rr*sin(ang)))
    for i in range(nx):
        for j in range(nr):fs.append((i*nr+j,i*nr+(j+1)%nr,(i+1)*nr+(j+1)%nr,(i+1)*nr+j))
    fs.extend([tuple(range(nr-1,-1,-1)),tuple(range(nx*nr,(nx+1)*nr))])
    phone=mesh('Molded telephone receiver',vs,fs,'rubber')
    for f in phone.data.polygons:f.use_smooth=True
    for xx in [-3.269,-3.012]:
        box('Receiver rest',(xx,5.0,.889),(.043,.083,.05),'frame',.009)
    for xx in [-3.275,-3.007]:
        for j in range(4):cyl('Receiver acoustic hole',(xx,4.978,.902+j*.006),.0018,.002,'ink','Y',vertices=12,w=0)
...
def cloth(name,x,y,z,w=.27,h=.20,mat='blue_fabric',drape=None):
    vs=[];fs=[];nx=20;ny=16
    for j in range(ny+1):
        for i in range(nx+1):
    ...
    lowest=min(v[2] for v in vs if drape is None or v[1]>=drape+.01)
    vs=[(xx,yy,zz-lowest+z+.0015-(max(0,drape-yy)*.78 if drape else 0)) for xx,yy,zz in vs]
    ...
    if ASM:
        pt=min((v for v in vs if drape is None or v[1]>=drape+.01),key=lambda v:v[2])
...
def slice_architecture():
    ...
    door_front(-5.4,3.6)
    for xx in [-5.99,-4.81]:
        for dz,ww,hh in [(.115,.027,.014),(.167,.009,.032),(.299,.025,.008)]:
            scar_front('Jamb trolley wear',xx,3.449,dz,ww,hh,'steel')
    ...
    box('Hatch fixed glass',(-3.961,3.564,1.94),(.826,.012,.94),'glass',.001)
    box('Hatch sliding glass',(-3.106,3.542,1.94),(.906,.012,.94),'glass',.001)
    for xx,yy in [(-4.379,3.564),(-3.546,3.564),(-3.564,3.542),(-2.648,3.542)]:
        box('Glazing rubber bead',(xx,yy,1.94),(.012,.022,.94),'rubber',.002)
        box('Glazing extruded stile',(xx,yy-.018,1.94),(.019,.015,.96),'steel',.002)
    for xx in [-4.25,-3.68,-3.40,-2.79]:
        box('Glass roller saddle',(xx,3.513,2.407),(.085,.045,.068),'frame',.005)
        cyl('Glass carriage roller',(xx,3.484,2.411),.021,.018,'steel','Y',w=.002)
        bolt('Roller axle',(xx,3.471,2.411),r=.008)
    line('Glass sliding pull',[(-3.49,3.517,1.75),(-3.49,3.445,1.75),(-3.49,3.445,1.91),(-3.49,3.517,1.91)],.009,'steel')
    box('Glazing latch block',(-3.49,3.510,1.510),(.059,.040,.058),'teal',.004)
    cyl('Glazing latch pin',(-3.49,3.477,1.510),.011,.026,'steel','Y')
    for x in [-4.1,-3.0]:
    ...
    cloth('Folded cleaning cloth',-2.94,3.025,1.04,.24,.22,drape=2.977)
