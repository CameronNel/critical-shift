...
import bpy,json,math,os,bmesh
from pathlib import Path
from mathutils import Vector
ROOT=Path(__file__).resolve().parents[1]
sid=os.environ['EXTERIOR_SECTION'];rev=os.environ['EXTERIOR_REVISION'];out=ROOT/'exteriors'/sid
...
original=bpy.data.objects['READ_ONLY_ORIGINAL_'+sid].instance_collection
...
mesh_defects=[]
for ob in added:
 eo=ob.evaluated_get(dg);bm=bmesh.new();bm.from_mesh(eo.to_mesh());vol=bm.calc_volume(signed=True)
 if vol<=0 or any(not e.is_manifold for e in bm.edges):mesh_defects.append({'object':ob.name,'signed_volume':vol})
 bm.free();eo.to_mesh_clear()
report={'mesh_defects':mesh_defects,'revision':rev,'scope':'Evaluated world-AABB connectivity broadphase, not raycast/anchor/support-angle validation. Original render-visible mesh objects are root candidates; physical/collision suitability is not independently classified. AABB overlap can be empty space, contact does not imply load-bearing support, and deep penetration is accepted by this connectivity graph. Text/curves excluded. Model is not saved.','threshold_m':.005,'numerical_epsilon_m':.000001,'additive_mesh_count':n,'original_root_candidate_count':len(roots),'root_connected_additive_count':len(connected),'detached_components':components,'direct_original_candidates':[{'object':objs[i].name,'original_candidates':[objs[j].name for j in root_hits[i]]} for i in range(n) if root_hits[i]],'additive_bounds':[{'name':objs[i].name,'bounds':bs[i]} for i in range(n)]}
(out/f'support-connectivity-{rev}.json').write_text(json.dumps(report,indent=2))
