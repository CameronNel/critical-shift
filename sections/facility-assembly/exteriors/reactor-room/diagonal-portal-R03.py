...
bpy.ops.wm.open_mainfile(filepath=str(out/'exterior-R03.blend'),load_ui=False);bpy.context.view_layer.update();dg=bpy.context.evaluated_depsgraph_get()
...
for o in bpy.data.collections['EXTERIOR_reactor-room'].objects:
 ...
 eo.to_mesh_clear()
r={'revision':'R03','portal_frame':'T(8.4,-8.4,0) Rz(-135deg)','box':[lo,hi],'inset_tolerance_m':.003,'bound_candidates':candidates,'surface_intrusions':hits,'scope':'Evaluated polygon clipping to inset portal box; no solid-containment test. All additive meshes/fonts/curves included.','samples':[]}
...
(out/'diagonal-portal-R03.json').write_text(json.dumps(r,indent=2));print(json.dumps(r))
