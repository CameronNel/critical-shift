...
for sid in ['reactor-room','cooling-plant']:
 out=root/'exteriors'/sid;path=out/'exterior-R02.blend';row=next(r for r in manifest if r['id']==sid);accepted=root/row['frozen'];module=root/'sources'/sid/'module.blend';hashes={str(p):sha(p) for p in [path,accepted,module]}
 ...
 r['audit']=json.loads((out/'audit-R02.json').read_text())
 ...
 (out/'independent-technical-R02.json').write_text(json.dumps(r,indent=2));print(sid, 'binding issues',len(r['binding_issues']),'source differences',len(r['source_geometry_transform_differences']),flush=True)
