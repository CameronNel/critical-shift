...
for sid in ['reactor-room']:
 out=root/'exteriors'/sid;path=out/'exterior-R03.blend';row=next(r for r in manifest if r['id']==sid);accepted=root/row['frozen'];module=root/'sources'/sid/'module.blend';hashes={str(p):sha(p) for p in [path,accepted,module]}
 ...
 r['audit']=json.loads((out/'audit-R03.json').read_text())
 ...
 bpy.ops.wm.open_mainfile(filepath=str(accepted),load_ui=False);bpy.context.scene.frame_set(1);bpy.context.view_layer.update();a={o.name:sig(o) for o in bpy.data.objects};r['source_geometry_transform_differences']=[n for n,s in linked.items() if a.get(n)!=s];r['hashes_unchanged']=all(sha(p)==h for p,h in hashes.items());r['hashes']=hashes
 (out/'independent-technical-R03.json').write_text(json.dumps(r,indent=2));print(sid, 'binding issues',len(r['binding_issues']),'source differences',len(r['source_geometry_transform_differences']),flush=True)
