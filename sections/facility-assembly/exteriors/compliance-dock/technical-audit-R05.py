...
files={'exterior':out/'exterior-R05.blend','accepted':root/row['frozen'],'module':root/'sources/compliance-dock/module.blend','original':Path(row['original'])}
...
os.environ['EXTERIOR_SECTION']='compliance-dock';os.environ['EXTERIOR_REVISION']='R05'
runpy.run_path(str(root/'blender/audit_exterior.py'),run_name='__main__')
report['rerun_audit']=json.loads((out/'audit-R05.json').read_text())['status']
...
(out/'technical-audit-R05.json').write_text(json.dumps(report,indent=2))
