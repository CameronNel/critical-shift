...
for sid,rev in [('mine','R01')]:
 ...
 ext=bpy.data.collections['EXTERIOR_'+sid];meshes=list({o for o in list(ext.objects)+list(bpy.data.objects) if o.type=='MESH' and not o.hide_render and (o in list(ext.objects) or any(t in o.name.lower() for t in ['roof','ceiling','canopy']))})
 rows=[]
