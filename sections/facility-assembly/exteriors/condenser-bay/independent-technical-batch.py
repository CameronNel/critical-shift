...
for sid in ['condenser-bay','cooling-plant','reactor-room','fuel-corridor']:
 ...
 bpy.ops.wm.open_mainfile(filepath=str(accepted),load_ui=False);a={o.name:sig(o) for o in bpy.data.objects};r['source_geometry_transform_differences']=[n for n,s in linked.items() if a.get(n)!=s];r['hashes_unchanged']=all(sha(p)==h for p,h in hashes.items());r['hashes']=hashes
 (out/'independent-technical-R01.json').write_text(json.dumps(r,indent=2));print(sid,json.dumps({k:v for k,v in r.items() if k not in ['audit','hashes','images','libraries','portal_candidates']}),flush=True)
