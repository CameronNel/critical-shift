...
for w in warnings:
 ...
 reason=('Evaluated font tessellation; backed surface text, not architectural shell.' if kind=='FONT' else
         ...
         'Intentional surface wear/decal sheet, separately support-tested.' if base in {'Localized chipped paint','Wear at repeatedly handled edge','Sparse floor handling scuff','Subtle repaired wall patch','Power flow direction stencil'} else None)
...
for wr in wm['renders']:
 ...
 delta=np.abs(np.asarray(i).astype(float)-np.asarray(j).astype(float))
 metrics={'mean_absolute_error_255':float(delta.mean()),'maximum_channel_error_255':float(delta.max()),'changed_pixel_fraction':float(np.any(delta>0,axis=2).mean()),'over_two_levels_pixel_fraction':float(np.any(delta>2,axis=2).mean())}
 stable=metrics['mean_absolute_error_255']<=.01 and metrics['maximum_channel_error_255']<=8 and metrics['changed_pixel_fraction']<=.005 and metrics['over_two_levels_pixel_fraction']<=.0001
 rows.append({'camera':name,'decoded_pixels_equal':i.size==j.size and difference.getbbox() is None,'render_numerically_stable':stable,'pixel_metrics':metrics,'warm_sha256':sha(warm/(name+'.png')),'cold_sha256':sha(cold/(name+'.png')),'camera_equal':bool(cr) and all(wr[k]==cr[k] for k in ['location','rotation_euler','lens_mm'])})
...
r['render_comparison_policy']='Exact comparison failed and is retained separately. GPU render numerical stability requires mean error<=0.01/255, maximum<=8/255, changed pixels<=0.5%, pixels differing by >2 levels<=0.01%. Independent cold pixel review remains required. No exact pixel identity claimed.'
r['pass']=r['saved_file_unchanged'] and a['artifact_fingerprint']==b['artifact_fingerprint'] and all(source_checks.values()) and len(rows)==14 and all(x['render_numerically_stable'] and x['camera_equal'] for x in rows) and tech['pass'] and r['walkthrough_pass'] and r['apertures_pass'] and all(x['reason'] and x['nonmanifold_edges']==0 for x in classification)
