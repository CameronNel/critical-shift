...
def switchgear(name,y,kind='feeder',label='DISTRIBUTION',width=1.17):
    ...
    box('Meter compartment skin',(0,-.633,2.20),(w-.14,.055,.87),'pale',root,.005)
    fasteners('Captive panel screw',[-w/2+.13,w/2-.13],[1.81,2.59],-.669,root,r=.011)
    plaque(root,label,(0,-.672,2.48),w-.27,.12,size=.048)
    folded_panel(root,(0,-.705,2.16),w-.26,.39,'pale')
    gauge(root,-.21,-.685,2.18,.091,'SRC' if kind=='incoming' else 'LOAD',-.4 if kind=='incoming' else .5)
    if kind=='incoming':
        gauge(root,.20,-.68,2.18,.091,'LOAD',.8)
    else:
        ...
        text('Relay reading','READY',(.075,-.710,2.183),.039,'paper',root)
    ...
    if kind=='repair':
        ...
        box('Breaker access door',(0,-.64,1.12),(w-.14,.05,1.23),'pale',root,.006)
        for z in [.67,1.55]:
...
def door(name,x,y,rot=0,label='TURBINE'):
    ...
    box('Flush portal threshold',(0,0,-.006),(2.4,.50,.012),'metal',r,.001)
    plaque(r,label,(0,-.193,3.18),2.40,.28,size=.15)
    for x in [-.95,.95]:box('Portal sign wall spacer',(x,-.12,3.18),(.08,.14,.12),'metal',r,.003)
    return r
...
def light_fixture(name,loc,power=750,warm=False,rot=0):
    ...
    ld=bpy.data.lights.new(name+' photometric area','AREA');ld.energy=power*.72;ld.shape='RECTANGLE';ld.size=1.5;ld.size_y=.28;ld.color=(1,.83,.62) if warm else (1,.95,.86)
...
def architecture(slice=False):
    ...
    for side in [-1,1]:
        ...
        for cy,ln in ([(5.5,11),(15.9,1)] if not slice and side==1 else [(end/2,end)]):
            box('Damp-proof concrete curb',(side*5.43,cy,.10),(.14,ln,.2),'patch',bevel=.006)
        for zz in [1.25,3.1]:
            for cy,ln in ([(5.5,11),(15.9,1)] if not slice and side==1 else [(end/2,end)]):box('Precast horizontal reveal',(side*5.497,cy,zz),(.005,ln,.014),'joint',bevel=0)
        for yy in [2.5,5,7.5,10,12.5,15]:
            if yy<end and not (not slice and side==1 and 11<yy<15.4):box('Precast vertical reveal',(side*5.497,yy,2.4),(.005,.012,4.8),'joint',bevel=0)
    for y,is_back in [(0,False)]+([] if slice else [(end,True)]):
        ...
        door('D02_Waste' if is_back else 'D01_Turbine',0,y,math.pi if not is_back else 0,'WASTE / D02' if is_back else 'TURBINE / D01')
...
def work_cart(y=1.8):
    ...
    text('Work order imprint','SG06\nISOLATED\n________\n07:20  OK',(.15,.1,.8813),.018,'ink',paper,rot=(0,0,0))
    for x in [-.24,-.14]:
        text('Fuse ceramic marking','FUSE',(x-.014,-.01,.929),.015,'ink',r,rot=(0,0,0))
...
def setup():
    ...
    SC.world.node_tree.nodes['Background'].inputs[0].default_value=(.24,.23,.21,1);SC.world.node_tree.nodes['Background'].inputs[1].default_value=.12
...
busway(6.7 if args.stage=='slice' else 16.65)
...
if args.stage=='slice':
    ...
    from full_hall import build
    build(sys.modules[__name__])
setup()
SC.camera=bpy.data.objects.get('C01_Entry') or bpy.data.objects.get('S01_Validation') or next(o for o in bpy.data.objects if o.type=='CAMERA')
SC['authoring_sources']=json.dumps({p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in Path(__file__).parent.glob('*.py')})
...
for p in Path(__file__).parent.glob('*.py'):
    if p.name!='build_room.py':(md/p.name).write_bytes(p.read_bytes())
