...
def build(B):
 ...
 # Mesh infill and both signs travel with the actual service leaf.
 bpy.context.view_layer.update()
 for child in list(tx.children):
  if child.location.y<-.98 and child.name.startswith(('Guard vertical wire','Guard horizontal wire','Recessed engraved plate','Engraved ')):
   local=gate.matrix_local.inverted() @ child.matrix_local
   child.parent=gate;child.matrix_basis=local
