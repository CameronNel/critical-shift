...
def build(B):
 ...
 box('Waste seam slab',(0,16.685,-.12),(3,.57,.24),'floor',bevel=0)
 ...
 # Two 1.52m leaves open within the 1.85m service apron, clear of main aisle.
 gates=[]
 for side in [-1,1]:
  gate=B.assembly('TX_GUARD_SERVICE_PIVOT_'+('LEFT' if side<0 else 'RIGHT'),(side*1.54,-1.018,.36),parent=tx)
  gate['assembly_root']=False;gate['open_rotation_z_degrees']=90*side;gate['closed_rotation_z_degrees']=0;gates.append(gate)
  direction=-side
  for z in [0,2.50]:box('Gate leaf horizontal',(direction*.76,0,z),(1.52,.035,.04),'yellow',gate,.003)
  for x in [0,direction*1.52]:box('Gate leaf stile',(x,0,1.25),(.045,.035,2.5),'yellow',gate,.003)
  for x in [direction*(.06+i*.12) for i in range(13)]:beam('Gate vertical mesh',(x,0,.03),(x,0,2.47),.009,'metal',gate)
  for z in [i*.12+.09 for i in range(20)]:beam('Gate horizontal mesh',(direction*.02,0,z),(direction*1.50,0,z),.009,'metal',gate)
  for z in [.25,2.23]:cyl('Guard gate hinge',(0,0,z),.035,.14,'metal',gate)
  B.handle(gate,direction*1.34,-.06,1.05,height=.2)
  if side<0:
   panel(gate,'TX / 01',(.76,-.027,1.20),.75,.26,.11)
   panel(gate,'ISOLATE BEFORE ACCESS',(.76,-.03,.89),1.40,.15,.065)
 ...
 B.gauge(td,-.49,-.45,.99,.105,'SOC',.5)
 box('Reserve state inset',(.49,-.447,.995),(.27,.025,.19),'darkpaint',td,.003)
 for z in [.965,1.025]:box('Reserve state readback',(.49,-.463,z),(.18,.008,.025),'amber',td,.002)
 ...
 for i,yy in enumerate([12.18,13.2,14.22],1):
  rb=root('RB0'+str(i)+'_Reserve_module',(7.78,yy,0),-math.pi/2,target='Reserve floor',anchors=[[-.40,-.30,0],[.40,-.30,0]])
 ...
 cyl('Enamel mug body',(-.92,.51,1.083),.055,.126,'pale',wb)
 cyl('Mug dark interior',(-.92,.51,1.148),.047,.004,'black',wb)
 ring('Mug rolled rim',(-.92,.51,1.149),.051,.005,'metal',wb)
 ring('Mug handle',(-.858,.51,1.09),.033,.008,'pale',wb,'Y')
 ...
 for x in [2,4.5,7]:
  ...
  for y in [6.1,6.55]:cyl('TX branch hanger rod',(x,y,.35),.014,1.14,'metal',bus)
 ...
 for polygon in me.polygons:polygon.flip()
 # Apply exact wall penetrations; authoring source retains recoverable operands.
 for name,loc,dims,targets in [
  ...
  cutter=box(name,loc,dims,'wall',bevel=0)
 ...
 # Named construction contacts: inset thresholds and enclosed terminal entries.
 for name,contacts in {
  'Waste seam slab':['Flush portal threshold.001'],
  'Entry seam slab':['Flush portal threshold'],
  'Transformer terminal drop':['Transformer terminal enclosure','TX primary insulated lead.001','TX secondary enclosed lead.001'],
  'Transfer entry drop':['Transfer crown']}.items():
  bpy.data.objects[name]['allowed_intersections']=json.dumps(contacts)
 for gate in gates:
  for child in gate.children:child['assembly_member']=tx.name
 ...
 ('C06_Transformer',(.65,9.3,1.68),(4.15,6.4,1.55),26),
 ...
 ('C10_Transfer',(1.5,10.2,1.68),(4.8,10.3,1.25),26),
 ...
 B.light_fixture('Transformer task practical',(2.3,5.8,4.6),650)
