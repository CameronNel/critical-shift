...
evidence={'blend':Path(bpy.data.filepath).name,'revision':s.get('revision'),'source_sha256':s.get('source_sha256'),'settings':{'engine':'CYCLES','device':a.device,'samples':a.samples,'resolution':[a.width,a.height],'seed':17},'renders':[]}
for c in cams:
