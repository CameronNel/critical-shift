...
M={
 ...
 'paint':material('Petrol enamel - original','31585E',.43,.26,.09,.001),
...
def gauge(parent,x,y,z,r=.115,label='A',needle=.4):
    ...
    cyl('Rolled bezel rim',(x,y-.032,z),r*.94,.012,'metal',parent,'Y',48)
...
def switchgear(name,y,kind='feeder',label='DISTRIBUTION',width=1.17):
    ...
    gauge(root,-.21,-.68,2.18,.091,'kV' if kind=='incoming' else 'A',-.4 if kind=='incoming' else .5)
    if kind=='incoming':
...
def door(name,x,y,rot=0,label='TURBINE'):
    ...
    box('Sliding door rail hood',(0,.42,2.93),(5.05,.24,.16),'metal',r,.008)
    # Two open sliding leaves parked on either side, flush to outer wall.
    for side in [-1,1]:
        xx=side*1.86
        box('Parked sliding leaf',(xx,.36,1.34),(1.26,.07,2.65),'pale',r,.006)
        box('Door impact shoe',(xx,.308,.32),(1.16,.032,.38),'metal',r,.003)
        handle(r,xx-side*.38,.28,1.1)
...
def work_cart(y=1.8):
    ...
    m=box('Handheld multimeter',(0,-.06,.935),(.15,.24,.11),'yellow',meter,.017)
    box('Multimeter screen',(0,-.09,.996),(.105,.10,.01),'glass',meter,.005)
    cyl('Range selector',(0,.015,.996),.033,.019,'black',meter)
    tube('Red test lead',[(.04,.02,.99),(.25,.08,.889),(.24,-.18,.884),(.08,-.24,.884)],.004,'red',meter)
    tube('Black test lead',[(-.02,.02,.99),(-.16,.14,.889),(-.27,.07,.884),(-.2,-.15,.884)],.004,'rubber',meter)
    for x in [-.24,-.14]:
    ...
    text('Work order imprint','D-04\nISOLATED',(.15,.1,.8813),.027,'ink',paper,rot=(0,0,0))
    # Folded cotton rag uses an undulating stitched surface with thickness.
    verts=[];faces=[]
    for j in range(9):
        for i in range(13):
            x=-.28+i*.037;y=-.18+j*.038;z=.302+.016*(1+math.sin(i*.8))*(1+math.sin(j*.7))/4
            verts.append((x,y,z))
    for j in range(8):
        for i in range(12):a=j*13+i;faces.append((a,a+1,a+14,a+13))
    low=min(verts,key=lambda v:v[2]);rag=assembly('Rag on lower tray',support='Cart lower tray',anchors=[[low[0],low[1],low[2]-.002]],parent=r)
