...
def arrow(x0,y0,x1,y1,color="#17616c",w=2):
...
add('''<defs><marker id="arrow" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0 0 L8 4 L0 8 Z" fill="#17616c"/></marker><pattern id="hatch" width="8" height="8" patternUnits="userSpaceOnUse" patternTransform="rotate(45)"><line x1="0" y1="0" x2="0" y2="8" stroke="#df7882" stroke-width="2"/></pattern></defs>''')
...
text(1525,82,f"REV {contract['revision']}  ·  08 SEP 2026",15,anchor="end",weight=700)
...
for eq in contract["equipment_zones"]:
    ...
    if eq["id"]=="SG":
        for yy in [4.55,6.05,7.75,9.25]: line(X(a[0]),Y(yy),X(b[0]),Y(yy),"#344956",1)
    if eq["id"]=="RB":
...
for yy,label in [(0,"D01"),(16.4,"D02")]:
    line(X(-1.2),Y(yy),X(1.2),Y(yy),"#17616c",3)
    text(X(0),Y(yy)+(23 if yy==0 else -15),label+" · 2.40 × 2.70",13,"#17616c",anchor="middle",weight=700)
    py=yy+.30 if yy==0 else yy-.30
    for leaf_min,leaf_max in [(-2.49,-1.23),(1.23,2.49)]:
        line(X(leaf_min),Y(py),X(leaf_max),Y(py),"#5e8589",.07*S)
line(X(5.5),Y(12),X(5.5),Y(14.4),"#17616c",3)
text(X(6.43),Y(15.08),"RESERVE BAY",12,anchor="middle",weight=700)
text(X(6.40),Y(11.27),"P03 · 2.40 × 3.20",11,"#17616c",anchor="middle",weight=700)
arrow(0,.45,0,1.65)
...
add(f'<circle cx="{X(0)}" cy="{Y(0)}" r="5" fill="#17616c"/>')
text(X(-1.55),Y(-.55),"(0,0,0)",12,"#17616c",anchor="end")
arrow(-4.7,.6,-3.7,.6)
arrow(-4.7,.6,-4.7,1.6)
...
# Socket labels.
for p,label in [((-4.32,0),"U01"),((-4.32,16.4),"U02"),((8.3,14),"U03")]:
...
rows=[
 ...
 ("RB · RESERVE CABINETS", "0.75 × 3.00; front −X", "x 7.35…8.10 / y 11.70…14.70"),
...
lines(RX,824,["Main ceiling z=4.80; reserve ceiling z=3.60.", "Main bus CL: x=−4.32, z=3.88; y=0…16.40.", "Transformer branch crosses at y=6.40, CL z=3.88.", "Route-crossing duct/support underside ≥3.50.", "D01/D02: 2.70 H; P03: 3.20 H; thresholds flush.", "U03 essential inlet: (8.30, 14.00, 2.80), +X.", "Dashed bus lines record the built centrelines.", "Ratings, capacity and utility bindings unresolved."],13,22)
line(RX,1007,1515,1007,"#c4cdcb")
text(RX,1041,"04  CIRCULATION / REVIEW STATUS",19,weight=700)
status_lines=(["TD overlaps the direct P03 reserve access path.", "Hatched conflict must be corrected before acceptance."] if blocked else ["TD placement revised to clear the P03 branch.", "Final geometry and engine route checks remain pending."])
