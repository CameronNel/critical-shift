...
if($Mode -in @('slice','full')) {
    & $BlenderExe --background --factory-startup --python-exit-code 2 --python (Join-Path $PSScriptRoot 'build_room.py') -- --stage $Mode --revision $Revision
    if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}
}
$BlendFile=Join-Path $PSScriptRoot $(if($Mode -eq 'slice'){'electrical_slice.blend'}else{'electrical_room.blend'})
if($Mode -eq 'full') {
    & $BlenderExe --background $BlendFile --python-exit-code 2 --python (Join-Path $PSScriptRoot 'final_adjustments.py') -- --revision $Revision
    if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}
}
...
& $PythonExe $GpuGate --owner electrical-room -- $BlenderExe --background $BlendFile --python-exit-code 2 --python (Join-Path $PSScriptRoot 'render_room.py') -- --out $Out --cameras $Cameras --samples $Samples --width $Width --height $Height
exit $LASTEXITCODE
