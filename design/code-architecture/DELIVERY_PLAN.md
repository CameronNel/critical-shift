...
**Revision 2.0 | Planning only | No work package below is implemented by this revision**
...
| WP-01 Reproducible foundation | Minimal Unity project at agreed runtime root; pinned editor, package/compiler/backend profile; documented clean import/test/build invocation | WP-00 accepted; D-01 decided; Unity runner/license actually available | Clean checkout imports, discovers a real test, builds/starts/stops a minimal Player. Block if commands/configuration cannot be reproduced. |
| WP-02 Boundary and evidence checks | Explicit asmdefs, diagnostics baseline, resolved-graph checks, test-discovery failure controls and initial content-root inventory | WP-01 working | ARCH-01/02/03, CI-01/02 and DEAD-02. Deliberate violations fail; removal restores pass. Block gameplay expansion if safeguards only exist on paper. |
| WP-03 Small interaction/worker rules | Claim/lease/sequence rules, orthogonal worker state and session lifetime; one crate, basic feedback, cart/conveyor/lever fixture | WP-02; D-03 minimal input/feedback choice; D-09 initial fixtures ratified | CMD, HOLD and LIFE cases at the lowest suitable layer; single-host PHYS behaviour. No Materials/Reactor/whole-facility framework generated. |
...
| WP-05 Networked physics acceptance | Two-peer smoke followed by four-peer physics room and lifecycle tests | WP-04 chosen adapter; same command path for host and clients | Required N0/N1/N2 and Gate 1 soak evidence, clean restarts and independent feel review. Block content expansion on unstable claims, ragdolls or teardown. |
...
WP-01 and WP-02 jointly establish Roadmap Gate 0. WP-03 through WP-05 establish Gate 1. Later packages map directly to the existing roadmap. A package is not complete because its files exist; all relevant exit conditions require actual evidence. A later feature can be designed in advance without implementing it or declaring its gate passed.
...
A serious integrity/authority/reference issue blocks the affected subsystem's gate. A missing runner blocks validation rather than justifying invented results. Unrelated authorized planning/art tasks may proceed, but no stage gets marked passed by assumption. The immediate deliverable from this revision is a reviewable plan, not an engine project or a promise of future unattended work.
