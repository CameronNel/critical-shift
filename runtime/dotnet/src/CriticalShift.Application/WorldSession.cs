...
namespace CriticalShift.Application
{
    ...
    public sealed class WorldSession
    {
        ...
        public WorldSession(WorldSessionConfiguration configuration, IInteractionAccessPolicy access)
        {
            ...
            _interaction = new InteractionWorld(Epoch, new RunningAccess(_timeline, access),
        ...
        {
            RequireIdle(); long next = NextRevision();
            _interaction.RegisterConnection(connectionId, actorId); _revision = next;
        }
        ...
        public WorldAdvanceResult AdvanceTo(long hostMilliseconds)
        {
            ...
            try
            {
                ...
                if (timeChanged || released.Count != 0 || due.Count != 0) _revision = next;
                return new WorldAdvanceResult(View, signals.AsReadOnly(), released, false);
            }
            catch { FailClosed(); throw; }
        }
        ...
        public ObjectClaimView? Disconnect(Guid epoch, Guid connectionId)
        {
            ...
            try
            {
                ...
                if (count != _interaction.ConnectedCount) _revision = next;
                return released;
            }
            catch { FailClosed(); throw; }
        }
        ...
        /// </summary>
        public WorldSession Restart(IInteractionAccessPolicy nextWorldAccess)
        {
            RequireIdle();
            if (_restartIssued) throw new InvalidOperationException("A successor was already created from this world.");
            var next = new WorldSession(Configuration, nextWorldAccess);
        ...
        private void StopOwnedResources() { _timers.Stop(); _interaction.Stop(); }
        ...
        private sealed class RunningAccess : IInteractionAccessPolicy
        {
            ...
            internal RunningAccess(SessionTimeline timeline, IInteractionAccessPolicy inner)
            { _timeline = timeline; _inner = inner; }
            public AccessDecision Evaluate(Guid actorId, Guid entityId, InteractionKind kind)
            {
                ...
                return kind == InteractionKind.Grab && _timeline.Phase != TimelinePhase.Running ?
